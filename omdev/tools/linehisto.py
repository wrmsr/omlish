...
def with_timer(interval=5):
    def outer(fn):
        def inner(*args, **kwargs):
            inner.now = time.time()
            if (inner.now - inner.last) < interval:
                return None
...
class KeyedHistoRenderer:
    ...
    def render_status(self, entries):
        status = f'{self.histo.total_seen} total seen'

        total_tracked_percent = calc_percent(self.histo.total_tracked, self.histo.total_seen)

        status += f', {len(self.histo)} tracked, {self.histo.total_tracked} / {total_tracked_percent:.2f} % total tracked'  # noqa

        status += f', {self.histo.num_evicted} evicted, {self.histo.total_evicted} / {100.0 - total_tracked_percent:.2f} % total evicted'  # noqa

        duplicate_evictions = self.histo.total_evicted - self.histo.num_evicted

        status += f', {duplicate_evictions} / {calc_percent(duplicate_evictions, self.histo.total_evicted):.2f} % duplicate evictions'  # noqa

        # tracked % + duplicate evicted %
        status += f', {calc_percent(self.histo.total_tracked - duplicate_evictions, self.histo.total_seen):.2f} % correct'  # noqa
...
class CursesKeyedHistoRenderer(KeyedHistoRenderer):
    ...
    def __init__(self, window, histo, redraw_interval=.1):
        self.window = window
