...
def test_meet_same_type_returns_left() -> None:
...
def test_meet_any_returns_other_type() -> None:
    any_type = make_any()
    int_type = make_instance(make_info('int'))

    assert meet_types(any_type, int_type) is int_type
    assert meet_types(int_type, any_type) is int_type
...
def test_meet_nominal_subtype_returns_subtype() -> None:
    ...
    assert meet_types(child, base) is child
    assert meet_types(base, child) is child
...
def test_meet_reflected_generic_subclass_returns_subclass() -> None:
    ...
    assert meet_types(child, int_box) is child
    assert meet_types(int_box, child) is child
...
def test_meet_reflected_generic_subclass_with_different_base_arg_returns_uninhabited() -> None:
    ...
    typ = meet_types(child, str_box)

    assert isinstance(typ, types.UninhabitedType)


def test_meet_unrelated_supported_instances_returns_uninhabited() -> None:
...
def test_meet_union_distributes_and_drops_uninhabited_items() -> None:
    base_info = make_info('Base')
    child_info = make_info('Child')
    ...
    typ = meet_types(types.UnionType([child, other]), base)

    assert typ is child


def test_meet_union_with_union_distributes_pairwise() -> None:
    base_info = make_info('Base')
    child_info = make_info('Child')
    ...
    typ = meet_types(types.UnionType([child, left]), types.UnionType([base, right]))

    assert typ is child


def test_meet_union_returns_uninhabited_when_all_pairs_are_uninhabited() -> None:
    left = make_instance(make_info('Left'))
    right = make_instance(make_info('Right'))
    other = make_instance(make_info('Other'))

    typ = meet_types(types.UnionType([left, right]), other)

    assert isinstance(typ, types.UninhabitedType)


def test_meet_typed_dict_subtype_returns_narrower_type() -> None:
    int_type = make_instance(make_info('int'))
    source = make_typed_dict({'x': int_type, 'extra': int_type}, {'x', 'extra'})
    target = make_typed_dict({'x': int_type}, {'x'})

    assert meet_types(source, target) is source


def test_meet_typed_dicts_combines_compatible_keys() -> None:
    int_type = make_instance(make_info('int'))
...
def test_meet_typed_dicts_meets_readonly_key_covariantly() -> None:
    ...
    assert isinstance(typ, types.TypedDictType)
    assert type_str(typ) == "TypedDict({'x'=: Child})"


def test_meet_typed_dicts_raises_for_incompatible_mutable_key() -> None:
...
def test_meet_union_propagates_unsupported_relations() -> None:
    fallback = make_instance(make_info('function'))
    callable_type = types.CallableType([], [], [], make_any(), fallback)

    with pytest.raises(UnsupportedTypeOperationError):
        meet_types(types.UnionType([callable_type]), make_instance(make_info('object')))


def test_meet_raises_for_unsupported_subtype_relation() -> None:
    fallback = make_instance(make_info('function'))
    callable_type = types.CallableType([], [], [], make_any(), fallback)

    with pytest.raises(UnsupportedTypeOperationError):
        meet_types(callable_type, make_instance(make_info('object')))
...
def test_meet_strips_type_guarded_and_annotated_wrappers() -> None:
    ...
    assert meet_types(types.TypeGuardedType(child), base) is child
    assert meet_types(types.AnnotatedType(child, ('cfg',)), base) is child
    assert meet_types(child, types.AnnotatedType(base, ('cfg',))) is child


def test_meet_required_readonly_unpack_and_type_type_fail_closed_for_now() -> None:
...
def test_meet_type_type_returns_narrower_class_object_type() -> None:
    ...
    assert type_str(meet_types(child_type, base_type)) == 'type[Child]'
    assert type_str(meet_types(base_type, child_type)) == 'type[Child]'


def test_meet_type_type_with_unsupported_item_relation_fails_closed() -> None:
    fallback = make_instance(make_info('function'))
    callable_type = types.CallableType([], [], [], make_any(), fallback)
    item = make_instance(make_info('Item'))

    with pytest.raises(UnsupportedTypeOperationError, match='Unsupported meet'):
        meet_types(types.TypeType(callable_type), types.TypeType(item))


def test_meet_unrelated_type_types_returns_class_object_never() -> None:
    ...
    assert isinstance(typ.item, types.UninhabitedType)


def test_meet_union_of_type_types_with_type_type_drops_uninhabited_items() -> None:
    base_info = make_info('Base')
    child_info = make_info('Child')
    other_info = make_info('Other')
    child_info._mro = (child_info, base_info)
    ...
    typ = meet_types(types.UnionType([child_type, other_type]), base_type)

    assert type_str(typ) == 'type[Child]'


def test_meet_fixed_tuples_synthesizes_itemwise_meet() -> None:
...
def test_meet_fixed_tuples_with_different_lengths_returns_uninhabited() -> None:
    fallback = make_instance(make_info('builtins.tuple'), [make_any()])
    ...
    assert isinstance(meet_types(left, right), types.UninhabitedType)


def test_structural_meet_expands_variadic_alias_to_fixed_tuple() -> None:
    fallback = make_instance(make_info('builtins.tuple'), [make_any()])
    int_type = make_instance(make_info('builtins.int'))
    str_type = make_instance(make_info('builtins.str'))
    ...
    assert structural_meet_types(alias_type, target) is alias_type
    assert structural_meet_types(target, alias_type) is target


def test_meet_fails_closed_for_variadic_tuple_shapes() -> None:
    fallback = make_instance(make_info('builtins.tuple'), [make_any()])
    int_type = make_instance(make_info('builtins.int'))
    type_var_tuple = make_type_var_tuple('Ts', 1)
    ...
    with pytest.raises(UnsupportedTypeOperationError, match='variadic Tuple meet'):
        meet_types(variadic, concrete)

    with pytest.raises(UnsupportedTypeOperationError, match='variadic Tuple meet'):
        structural_meet_types(concrete, variadic)
...
def test_meet_simple_callables_returns_more_specific_callable() -> None:
    ...
    assert meet_types(left, right) is left
    assert meet_types(right, left) is left
...
def test_meet_callables_with_incompatible_shape_returns_uninhabited() -> None:
    item = make_instance(make_info('Item'))
...
def test_meet_generic_callable_fails_closed_for_now() -> None:
    fallback = make_instance(make_info('function'))
...
def test_meet_same_overload_returns_left() -> None:
    fallback = make_instance(make_info('function'))
    int_type = make_instance(make_info('int'))
...
def test_meet_different_overload_synthesizes_pairwise_items() -> None:
    fallback = make_instance(make_info('function'))
    int_type = make_instance(make_info('int'))
    str_type = make_instance(make_info('str'))
    left = types.Overloaded([
    ...
    assert type_str(typ) == 'Overload(def (Union[int, str]) -> Never, def (Union[str, int]) -> Never)'


def test_meet_overload_and_callable_fails_closed_for_now() -> None:
    fallback = make_instance(make_info('function'))
    int_type = make_instance(make_info('int'))
...
def test_meet_type_list_empty_returns_special_form_any() -> None:
    typ = meet_type_list([])

    assert isinstance(typ, types.AnyType)
    assert typ.type_of_any == types.TypeOfAny.SPECIAL_FORM


def test_meet_type_list_singleton_returns_item() -> None:
    item = make_instance(make_info('Item'))

    assert meet_type_list([item]) is item


def test_meet_type_list_folds_multiple_items() -> None:
    base_info = make_info('Base')
    child_info = make_info('Child')
    other_info = make_info('Other')
    child_info._mro = (child_info, base_info)
    ...
    typ = meet_type_list([base, child, other])

    assert isinstance(typ, types.UninhabitedType)


def test_structural_meet_literal_alias_with_fallback_returns_alias() -> None:
    str_type = make_instance(make_info('builtins.str'))
    literal = types.LiteralType('debug', str_type)
    alias = symbols.TypeAlias('Mode', literal)
    alias_type = types.TypeAliasType(alias, [])

    assert structural_meet_types(alias_type, str_type) is alias_type
    assert structural_meet_types(types.AnnotatedType(alias_type, ('cfg',)), str_type) is alias_type


def test_structural_meet_preserves_newtype_identity_through_aliases() -> None:
    int_type = make_instance(make_info('builtins.int'))
    ...
    assert structural_meet_types(user_alias_type, user_id) is user_alias_type

    typ = structural_meet_types(user_alias_type, account_alias_type)

    assert isinstance(typ, types.UninhabitedType)


def test_structural_meet_recursive_alias_matches_one_unrolling() -> None:
    ...
    assert structural_meet_types(alias_type, alias.target) is alias_type
    with pytest.raises(UnsupportedTypeOperationError):
        meet_types(alias_type, alias.target)


def test_structural_meet_recursive_alias_with_object_bound_returns_alias() -> None:
    object_info = make_info('builtins.object')
    int_info = make_info('builtins.int')
    list_info = make_info('builtins.list')
    int_info._mro = (int_info, object_info)
    ...
    assert structural_meet_types(alias_type, object_type) is alias_type
    assert structural_meet_types(object_type, alias_type) is alias_type


def test_structural_meet_json_like_recursive_alias_matches_one_unrolling_and_equivalent_alias() -> None:
    ...
    assert structural_meet_types(left.alias_type, left.one_unrolling) is left.alias_type
    assert structural_meet_types(left.one_unrolling, left.alias_type) is left.one_unrolling
    assert structural_meet_types(left.alias_type, right.alias_type) is left.alias_type
    assert structural_meet_type_list([left.alias_type, left.one_unrolling, right.alias_type]) is left.alias_type


def test_structural_meet_recursive_variadic_tuple_alias_matches_one_unrolling() -> None:
    case = make_recursive_variadic_tuple_alias_case('TupleNode', 1)

    assert structural_meet_types(case.concrete_alias_type, case.one_unrolling) is case.concrete_alias_type
    assert structural_meet_type_list([case.concrete_alias_type, case.one_unrolling]) is case.concrete_alias_type


def test_structural_meet_recursive_aliases_with_different_identity_same_structure() -> None:
    list_info = make_info('builtins.list')
    int_type = make_instance(make_info('builtins.int'))
    ...
    assert structural_meet_types(left_alias_type, right_alias_type) is left_alias_type


def test_structural_meet_incompatible_recursive_aliases_returns_uninhabited() -> None:
    list_info = make_info('builtins.list')
    int_type = make_instance(make_info('builtins.int'))
    str_type = make_instance(make_info('builtins.str'))
    left_alias = symbols.TypeAlias('Left', make_any())
    ...
    typ = structural_meet_types(left_alias_type, right_alias_type)

    assert isinstance(typ, types.UninhabitedType)


def test_structural_meet_recursive_alias_with_unsupported_callable_target_fails_closed() -> None:
    fallback = make_instance(make_info('function'))
    t_var = make_type_var('T', 1)
    other = make_instance(make_info('Other'))
    ...
    with pytest.raises(UnsupportedTypeOperationError):
        structural_meet_types(alias_type, target)


def test_structural_meet_type_list_folds_recursive_aliases() -> None:
    list_info = make_info('builtins.list')
    int_type = make_instance(make_info('builtins.int'))
    alias = symbols.TypeAlias('Node', make_any())
    alias_type = types.TypeAliasType(alias, [])
    ...
    assert structural_meet_type_list([alias_type, alias.target]) is alias_type
