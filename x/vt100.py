...
def _generate_c():
    ...
    # Writing vtparse_table.h
    with open("vtparse_table.h", "w") as f:
    ...
    print("Wrote vtparse_table.h")

    # Writing vtparse_table.c
    with open("vtparse_table.c", "w") as f:
        ...
        for i, state in enumerate(states_in_order):
            ...
            for j, state_change in enumerate(state_tables[state]):
                if not state_change:
                    ...
                    f.write(
                        f"/*{str(j).rjust(3)}*/  {StringPad.pad(action_str, 33)} | ({StringPad.pad(state_str, 33)} << 4),\n")
            f.write("  },\n")
        f.write("};\n\n")
