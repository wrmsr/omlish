...
class BlueletListener:
    ...
    def accept(self) -> BlueletFuture['AcceptBlueletEvent', None]:
        """
        ...
        return BlueletFuture(AcceptBlueletEvent(self))
    ...
    async def readline(self, terminator: bytes = b'\n', bufsize: int = 1024) -> None:
        ...
        while True:
            if terminator in self._buf:
                ...
                line += terminator
                await BlueletFuture(ReturnBlueletEvent(bytes(line)))
                break

            if (data := (await BlueletFuture(ReceiveBlueletEvent(self, bufsize)))):
                self._buf += data
            else:
                line = self._buf
                self._buf = bytearray()
...
async def bluelet_server(host: str, port: int, func) -> None:
    """
    ...
    try:
        while True:
            ...
            await bluelet_spawn(handler(conn))
