...
def bluelet_read(fd: ta.IO, bufsize: ta.Optional[int] = None) -> BlueletFuture[BlueletEvent, bytes]:
    ...
    if bufsize is None:
        ...
        async def reader():
            buf = []
            while True:
                data = await bluelet_read(fd, 1024)
                if not data:
                    break
                buf.append(data)
            await BlueletFuture(ReturnBlueletEvent(''.join(buf)))

        return BlueletFuture(DelegationBlueletEvent(reader()))

    else:
        return BlueletFuture(ReadBlueletEvent(fd, bufsize))
...
def bluelet_write(fd: ta.IO, data: bytes) -> BlueletFuture[BlueletEvent, None]:
    """Event: write to a file descriptor asynchronously."""

    return BlueletFuture(WriteBlueletEvent(fd, data))
