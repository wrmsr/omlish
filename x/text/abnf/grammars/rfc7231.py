...
Note that this RFC is obsolete as of June 2022, replaced by https://www.rfc-editor.org/rfc/rfc9110.
"""
import typing as ta
...
class Rule(_Rule):
    ...
    GRAMMAR: ta.ClassVar[list[str] | str] = [
        'Accept = [ ( "," / ( media-range [ accept-params ] ) ) *( OWS "," [ OWS ( media-range [ accept-params ] ) ] ) ]',  # noqa
        'Accept-Charset = *( "," OWS ) ( ( charset / "*" ) [ weight ] ) *( OWS "," [ OWS ( ( charset / "*" ) [ weight ] ) ] )',  # noqa
        'Accept-Encoding = [ ( "," / ( codings [ weight ] ) ) *( OWS "," [ OWS ( codings [ weight ] ) ] ) ]',
        'Accept-Language = *( "," OWS ) ( language-range [ weight ] ) *( OWS "," [ OWS ( language-range [ weight ] ) ] )',  # noqa
