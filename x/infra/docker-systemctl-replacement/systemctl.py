...
DefaultUnit = os.environ.get(
    "SYSTEMD_DEFAULT_UNIT", "default.target"
)  # systemd.exe --unit=default.target
DefaultTarget = os.environ.get(
    "SYSTEMD_DEFAULT_TARGET", "multi-user.target"
)  # DefaultUnit fallback
...
DefaultStandardOutput = os.environ.get(
    "SYSTEMD_STANDARD_OUTPUT", "journal"
)  # systemd.exe --default-standard-output
DefaultStandardError = os.environ.get(
    "SYSTEMD_STANDARD_ERROR", "inherit"
)  # systemd.exe --default-standard-error
...
def get_home():
    if False:  # pragma: no cover
        explicit = os.environ.get(
            "HOME", ""
        )  # >> On Unix, an initial ~ (tilde) is replaced by the
        if explicit:
            return explicit  # environment variable HOME if it is set; otherwise
...
def get_VARTMP(root=False):
    ...
    return os.environ.get(
        "TMPDIR", os.environ.get("TEMP", os.environ.get("TMP", VARTMP))
    )
    ...
    if user:
        ...
        if xgroups:
            groups += [
                ...
                if g.gr_name in xgroups and g.gr_gid not in groups
            ]
        if not groups:
            if group:
                gid = grp.getgrnam(group).gr_gid
...
ExecMode = collections.namedtuple(
    "ExecMode", ["mode", "check", "nouser", "noexpand", "argv0"]
)
...
class SystemctlConfData:
    ...
    def __init__(
        self, defaults=None, dict_type=None, conf_type=None, allow_no_value=False
    ):
    ...
    def get(self, section, option, default=None, allow_no_value=False):
        ...
        if option not in self._conf[section]:
            ...
            raise AttributeError(
                "option {} in {} does not exist".format(option, section)
            )
    ...
    def read_sysd(self, filename):
        ...
        for orig_line in open(filename):
            ...
            if line.startswith(".include"):
                logg.error(
                    "the '.include' syntax is deprecated. Use x.service.d/ drop-in files!"
                )
                includefile = re.sub(r"^\.include[ ]*", "", line).rstrip()
                if not os.path.isfile(includefile):
                    raise Exception(
                        "tried to include file that doesn't exist: %s" % includefile
                    )
        ...
        for line in self._lines:
            ...
            if m:
                ...
                if fnmatch.fnmatchcase(unit, pattern):
                    logg.debug(
                        "%s %s => %s %s", status, pattern, unit, strQ(self.filename())
                    )
                    return status
        return None
    ...
    def __enter__(self):
        try:
            ...
            for attempt in xrange(int(MaxLockWait or DefaultMaximumTimeout)):
                try:
                    logg_debug_flock(
                        "[%s] %s. trying %s _______ ", os.getpid(), attempt, lockname
                    )
                    fcntl.flock(self.opened, fcntl.LOCK_EX | fcntl.LOCK_NB)
                    st = os.fstat(self.opened)
                    if not st.st_nlink:
                        logg_debug_flock(
                            "[%s] %s. %s got deleted, trying again",
                            os.getpid(),
                            attempt,
                            lockname,
                        )
                    ...
                    logg_debug_flock(
                        "[%s] %s. holding lock on %s", os.getpid(), attempt, lockname
                    )
                    return True
                except IOError as e:
    ...
    def __exit__(self, type, value, traceback):
        try:
            ...
            if REMOVE_LOCK_FILE:  # an optional implementation
                ...
                os.unlink(
                    lockfile
                )  # ino is kept allocated because opened by this process
                logg.debug("[%s] lockfile removed for %s", os.getpid(), lockname)
            fcntl.flock(self.opened, fcntl.LOCK_UN)
            os.close(
                self.opened
            )  # implies an unlock but that has happend like 6 seconds later
            self.opened = -1
        except Exception as e:
            logg.warning("oops, %s", e)


SystemctlWaitPID = collections.namedtuple(
    "SystemctlWaitPID", ["pid", "returncode", "signal"]
)
    ...
    if cmd and cmd[0] == "/bin/kill":
        ...
        if pid is None:  # unknown $MAINPID
            if not waitpid.returncode:
                logg.error(
                    "waitpid %s did return %s => correcting as 11",
                    cmd,
                    waitpid.returncode,
                )
...
def subprocess_testpid(pid):
    ...
    if run_pid:
        return SystemctlWaitPID(
            run_pid, os.WEXITSTATUS(run_stat), os.WTERMSIG(run_stat)
        )
    else:
        return SystemctlWaitPID(pid, None, 0)


SystemctlUnitName = collections.namedtuple(
    "SystemctlUnitName",
    ["fullname", "name", "prefix", "instance", "suffix", "component"],
)
            ...
            for B in xrange(len(sortlist)):
                if A != B:
                    ...
                    if before > 0 and itemA.rank <= itemB.rank:
                        logg_debug_after(
                            "  %-30s before %s", itemA.conf.name(), itemB.conf.name()
                        )
                        itemA.rank = itemB.rank + 1
                        changed += 1
                    if before < 0 and itemB.rank <= itemA.rank:
                        logg_debug_after(
                            "  %-30s before %s", itemB.conf.name(), itemA.conf.name()
                        )
                        ...
                        if sock.fileno() == sock_fileno:
                            if not self.stopped.is_set():
                                if self.systemctl.loop.acquire():
                                    ...
                                    self.systemctl.do_accept_socket_from(
                                        sock.conf, sock.sock
                                    )
            except Exception as e:
...
class Systemctl:
    def __init__(self):
        ...
        self._file_for_unit_sysd = (
            None  # name.service => /etc/systemd/system/name.service
        )
    ...
    def match_sysd_templates(
        self, modules=None, suffix=".service"
    ):  # -> generate[ unit ]
    ...
    def list_service_units(
        self, *modules
    ):  # -> [ (unit,loaded+active+substate,description) ]
        ...
        return [
            (
                unit,
                result[unit] + " " + active[unit] + " " + substate[unit],
                description[unit],
            )
    ...
    def get_boottime_from_proc(self):
        ...
        if DEBUG_BOOTTIME:
            logg.debug(
                " boottime from the oldest entry in /proc [nothing in %s..%s]",
                pid1,
                pid_max,
            )
    ...
    def path_proc_started(self, proc):
        ...
        if DEBUG_BOOTTIME:
            logg.debug(
                "  BOOT 1. System uptime secs: %.3f (%s)", uptime_secs, system_uptime
            )
        ...
        if DEBUG_BOOTTIME:
            logg.debug(
                "  BOOT 1. Proc has been running since: %s"
        ...
        if DEBUG_BOOTTIME:
            logg.debug(
                "  BOOT 2. System btime secs: %.3f (%s)", system_btime, system_stat
            )

        started_btime = system_btime + started_secs
        if DEBUG_BOOTTIME:
            logg.debug(
                "  BOOT 2. Proc has been running since: %s"
                % (datetime.datetime.fromtimestamp(started_btime))
            )
    ...
    def truncate_old(self, filename):
        ...
        if filetime >= boottime:
            if DEBUG_BOOTTIME:
                logg.debug(
                    "  file time: %s (%s)",
                    datetime.datetime.fromtimestamp(filetime),
                    o22(filename),
                )
                logg.debug(
                    "  boot time: %s (%s)",
                    datetime.datetime.fromtimestamp(boottime),
                    "status modified later",
                )
            return False  # OK
        if DEBUG_BOOTTIME:
            logg.info(
    ...
    def get_ConfigurationDirectory(self, conf, section=Service):
        return self.expand_special(
            conf.get(section, "ConfigurationDirectory", ""), conf
        )
    ...
    def chown_service_directory(self, path, user, group):
        ...
        if user or group:
            ...
            if user and (user.strip() != st_user and user.strip() != str(st.st_uid)):
                change = True
            if group and (
                group.strip() != st_group and group.strip() != str(st.st_gid)
            ):
                change = True
            if change:
    ...
    def get_notify_socket_from(self, conf, socketfile=None, debug=False):
        ...
        if len(socketfile) > 100:
            ...
            if debug:
                logg.debug(
                    "https://unix.stackexchange.com/questions/367008/%s",
                    "why-is-socket-path-length-limited-to-a-hundred-chars",
                )
                logg.debug(
                    "old notify socketfile (%s) = %s", len(socketfile), socketfile
                )
            ...
            pref = "zz.%i.%s" % (
                get_USER_ID(),
                o22(os.path.basename(notify_socket_folder)),
            )
            if len(socketfile) > 100:
            ...
            if debug:
                logg.info(
                    "new notify socketfile (%s) = %s", len(socketfile), socketfile
                )
        return socketfile
    ...
    def wait_notify_socket(self, notify, timeout, pid=None, pid_file=None):
        ...
        if "READY" not in results:
            logg.info(
                ".... timeout while waiting for 'READY=1' status on $NOTIFY_SOCKET"
            )
        elif "MAINPID" not in results:
            logg.info(
                ".... seen 'READY=1' but no MAINPID update status on $NOTIFY_SOCKET"
            )
    ...
    def do_start_service_from(self, conf):
        ...
        if True:
            ...
            for cmd in conf.getlist(Service, "ExecStartPre", []):
                ...
                logg.debug(
                    " pre-start done (%s) <-%s>",
                    run.returncode or "OK",
                    run.signal or "",
                )
                if run.returncode and exe.check:
                    logg.error(
                        "the ExecStartPre control process exited with error code"
                    )
                    active = "failed"
            ...
            if service_result in ["success"] and mainpid:
                ...
                if "MAINPID" in results:
                    ...
                    if new_pid and new_pid != mainpid:
                        logg.info(
                            "NEW PID %s from sd_notify (was PID %s)", new_pid, mainpid
                        )
    ...
    def create_socket(self, conf):
        ...
        if re.match("@.*", address):
            logg.warning(
                "%s: abstract namespace socket not implemented (%s)",
                conf.name(),
                address,
            )
            return None
        if re.match("vsock:.*", address):
            logg.warning(
                "%s: virtual machine socket not implemented (%s)", conf.name(), address
            )
            return None
    ...
    def create_unix_socket(self, conf, path, dgram):
        ...
        try:
            ...
            if symlinks:
                logg.warning(
                    "%s: symlinks for socket not implemented (%s)", conf.name(), path
                )
        except Exception as e:
    ...
    def create_port_socket(self, conf, port, dgram):
        ...
        try:
            sock.bind(("", int(port)))
            logg.info(
                "%s: bound socket at %s %s:%s", conf.name(), strINET(inet), "*", port
            )
        except Exception as e:
            logg.error(
                "%s: create socket failed (%s:%s): %s", conf.name(), "*", port, e
            )
            sock.close()
            return None
        return sock
    ...
    def create_port_ipv4_socket(self, conf, addr, port, dgram):
        ...
        try:
            ...
            logg.info(
                "%s: bound socket at %s %s:%s", conf.name(), strINET(inet), addr, port
            )
        except Exception as e:
            logg.error(
                "%s: create socket failed (%s:%s): %s", conf.name(), addr, port, e
            )
            sock.close()
            return None
        return sock
    ...
    def execve_from(self, conf, cmd, env):
        ...
        try:
            if EXEC_SPAWN:
                ...
                sys.exit(
                    11
                )  # pragma: no cover (can not be reached / bug like mypy#8401)
        except Exception as e:
    ...
    def do_reload_service_from(self, conf):
        ...
        if runs in ["simple", "exec", "notify", "forking", "idle"]:
            ...
            for cmd in conf.getlist(Service, "ExecReload", []):
                ...
                if run.returncode and exe.check:
                    logg.error(
                        "Job for %s failed because the control process exited with error code. (%s)",
                        conf.name(),
                        run.returncode,
                    )
                    return False
    ...
    def restart_unit_from(self, conf):
        ...
        with waitlock(conf):
            if conf.name().endswith(".service"):
                logg.info(
                    " restart service %s => %s", conf.name(), strQ(conf.filename())
                )
                if not self.is_active_from(conf):
                    return self.do_start_unit_from(conf)
                else:
    ...
    def reload_or_restart_unit_from(self, conf):
        ...
        with waitlock(conf):
            logg.info(
                " reload-or-restart unit %s => %s", conf.name(), strQ(conf.filename())
            )
    ...
    def reload_or_try_restart_unit_from(self, conf):
        with waitlock(conf):
            logg.info(
                " reload-or-try-restart unit %s => %s",
                conf.name(),
                strQ(conf.filename()),
            )
    ...
    def status_unit(self, unit):
        ...
        if loaded:
            ...
            result += "\n    Loaded: {loaded} ({filename}, {enabled})".format(
                **locals()
            )
    ...
    def syntax_check_service(self, conf, section=Service):
        ...
        if haveType not in [
            ...
            logg.error(
                " %s: Failed to parse service type, ignoring: %s", unit, haveType
            )
            errors += 100
        for line in haveExecStart:
            ...
            if not exe.startswith("/"):
                if mode.check:
                    logg.error(
                        "  %s: %s Executable path is not absolute.", unit, section
                    )
                else:
                    logg.warning(
                        "%s: %s Executable path is not absolute.", unit, section
        ...
        if haveType in ["simple", "exec", "notify", "forking", "idle"]:
            if not usedExecStart and not usedExecStop:
                logg.error(
                    " %s: %s lacks both ExecStart and ExecStop= setting. Refusing.",
                    unit,
                    section,
                )
                errors += 101
                ...
                logg.error(
                    " %s: %s has no ExecStart= setting, which is only allowed for Type=oneshot services. Refusing.",
                    unit,
                    section,
                )
                errors += 101
        if len(usedExecStart) > 1 and haveType != "oneshot":
            logg.error(
                " %s: there may be only one %s ExecStart statement (unless for 'oneshot' services)."
                + "\n\t\t\tYou can use ExecStartPre / ExecStartPost to add additional commands.",
                unit,
                section,
            )
            errors += 1
        if len(usedExecStop) > 1 and haveType != "oneshot":
            logg.info(
                " %s: there should be only one %s ExecStop statement (unless for 'oneshot' services)."
                + "\n\t\t\tYou can use ExecStopPost to add additional commands (also executed on failed Start)",
                unit,
                section,
            )
        if len(usedExecReload) > 1:
            logg.info(
                " %s: there should be only one %s ExecReload statement."
                + "\n\t\t\tUse ' ; ' for multiple commands (ExecReloadPost or ExedReloadPre do not exist)",
                unit,
                section,
            )
        if len(usedExecReload) > 0 and "/bin/kill " in usedExecReload[0]:
            logg.warning(
                " %s: the use of /bin/kill is not recommended for %s ExecReload as it is asynchronous."
        ...
        if conf.getlist(Service, "ExecRestart", []):  # pragma: no cover
            logg.error(
                " %s: there no such thing as an %s ExecRestart (ignored)", unit, section
            )
        if conf.getlist(Service, "ExecRestartPre", []):  # pragma: no cover
            logg.error(
                " %s: there no such thing as an %s ExecRestartPre (ignored)",
                unit,
                section,
            )
        if conf.getlist(Service, "ExecRestartPost", []):  # pragma: no cover
            logg.error(
                " %s: there no such thing as an %s ExecRestartPost (ignored)",
                unit,
                section,
            )
        if conf.getlist(Service, "ExecReloadPre", []):  # pragma: no cover
            logg.error(
                " %s: there no such thing as an %s ExecReloadPre (ignored)",
                unit,
                section,
            )
        if conf.getlist(Service, "ExecReloadPost", []):  # pragma: no cover
            logg.error(
                " %s: there no such thing as an %s ExecReloadPost (ignored)",
                unit,
                section,
            )
        if conf.getlist(Service, "ExecStopPre", []):  # pragma: no cover
            logg.error(
                " %s: there no such thing as an %s ExecStopPre (ignored)", unit, section
            )
        for env_file in conf.getlist(Service, "EnvironmentFile", []):
            if env_file.startswith("-"):
                continue
            if not os.path.isfile(
                os_path(self._root, self.expand_special(env_file, conf))
            ):
    ...
    def exec_check_unit(self, conf, env, section=Service, exectype=""):
        ...
        for execs in [
            ...
            for cmd in conf.getlist(section, execs, []):
                ...
                if exe[0] != "/":
                    logg.error(
                        " %s: Exec is not an absolute path:  %s=%s", unit, execs, cmd
                    )
                    abspath += 1
                if not os.path.isfile(exe):
                    logg.error(
                        " %s: Exec command does not exist: (%s) %s", unit, execs, exe
                    )
                    if mode.check:
                        notexists += 1
        ...
        groups = [
            conf.get(section, "Group", ""),
            conf.get(section, "SocketGroup", ""),
        ] + conf.getlist(section, "SupplementaryGroups")
        for user in users:
            if user:
                try:
                    pwd.getpwnam(self.expand_special(user, conf))
                except Exception as e:
                    logg.error(
                        " %s: User does not exist: %s (%s)",
                        unit,
                        user,
                        getattr(e, "__doc__", ""),
                    )
        ...
        for group in groups:
            if group:
                try:
                    grp.getgrnam(self.expand_special(group, conf))
                except Exception as e:
                    logg.error(
                        " %s: Group does not exist: %s (%s)",
                        unit,
                        group,
                        getattr(e, "__doc__", ""),
                    )
        ...
        for setting in (
            ...
            if setting_value:
                logg.info(
                    "%s: %s private directory remounts ignored: %s=%s",
                    unit,
                    section,
                    setting,
                    setting_value,
        ...
        for setting in (
            ...
            if setting_yes:
                logg.info(
                    "%s: %s private directory option is ignored: %s=yes",
                    unit,
                    section,
                    setting,
                )
        ...
        if True:
            ...
            if abspath:
                logg.error(
                    " The SystemD ExecXY commands must always be absolute paths by definition."
                )
                time.sleep(1)
            if notexists:
                logg.error(
                    " Oops, %s executable paths were not found in the current environment. Refusing.",
                    notexists,
                )
                time.sleep(1)
            if badusers or badgroups:
                logg.error(
                    " Oops, %s user names and %s group names were not found. Refusing.",
                    badusers,
                    badgroups,
                )
                time.sleep(1)
            if tmpproblems:
                logg.info(
                    "  Note, %s private directory settings are ignored. The application should not depend on it.",
                    tmpproblems,
                )
                time.sleep(1)
            logg.error(
    ...
    def each_unit_items(self, unit, conf):
        ...
        yield "Description", self.get_description_from(
            conf
        )  # conf.get(Unit, "Description")
        yield "PIDFile", self.get_pid_file(
            conf
        )  # not self.pid_file_from w/o default location
        yield "PIDFilePath", self.pid_file_from(conf)
        yield "MainPID", strE(
            self.active_pid_from(conf)
        )  # status["MainPID"] or PIDFile-read
        yield "SubState", self.get_substate_from(
            conf
        ) or "unknown"  # status["SubState"] or notify-result
        yield "ActiveState", self.get_active_from(
            conf
        ) or "unknown"  # status["ActiveState"]
        ...
        yield "StartLimitIntervalSec", seconds_to_time(
            self.get_StartLimitIntervalSec(conf)
        )
    ...
    def target_default_services(self, target=None, sysv="S"):
        ...
        """
        igno = (
            self.igno_centos + self.igno_opensuse + self.igno_ubuntu + self.igno_always
        )
        if self._show_all:
    ...
    def enabled_target_services(self, target, sysv="S", igno=[]):
        ...
        if self.user_mode():
            ...
            for targets in targetlist:
                for unit in self.enabled_target_user_local_units(
                    targets, ".target", igno
                ):
                    if unit not in units:
                        units.append(unit)
            ...
            for targets in targetlist:
                for unit in self.enabled_target_user_local_units(
                    targets, ".service", igno
                ):
                    if unit not in units:
                        units.append(unit)
            for targets in targetlist:
                for unit in self.enabled_target_user_system_units(
                    targets, ".service", igno
                ):
                    if unit not in units:
                        units.append(unit)
            ...
            for targets in targetlist:
                for unit in self.enabled_target_configured_system_units(
                    targets, ".target", igno + self.igno_targets
                ):
                    if unit not in units:
                        units.append(unit)
            for targets in targetlist:
                for unit in self.required_target_units(targets, ".socket", igno):
                    if unit not in units:
                        units.append(unit)
            for targets in targetlist:
                for unit in self.enabled_target_installed_system_units(
                    targets, ".socket", igno
                ):
                    if unit not in units:
                        units.append(unit)
    ...
    def enabled_target_installed_system_units(
        self, target, unit_type=".service", igno=[]
    ):
        units = []
        for basefolder in self.system_folders():
            if not basefolder:
    ...
    def enabled_target_configured_system_units(
        self, target, unit_type=".service", igno=[]
    ):
        units = []
        if True:
    ...
    def get_StartLimitBurst(self, conf):
        ...
        return to_int(
            conf.get(Service, "StartLimitBurst", strE(defaults)), defaults
        )  # 5
    ...
    def restart_failed_units(self, units, maximum=None):
        ...
        for unit in units:
            ...
            try:
                ...
                if restartSec == 0:
                    if InitLoopSleep > 1:
                        logg.warning(
                            "[%s] set InitLoopSleep from %ss to 1 (caused by RestartSec=0!)",
                            unit,
                            InitLoopSleep,
                        )
                        InitLoopSleep = 1
                    ...
                    if restartSleep < InitLoopSleep:
                        logg.warning(
                            "[%s] set InitLoopSleep from %ss to %s (caused by RestartSec=%.3fs)",
                            unit,
                            InitLoopSleep,
                            restartSleep,
                            restartSec,
                ...
                if limitBurst > 1 and limitSecs >= 1:
                    try:
                        ...
                        logg.debug(
                            "[%s] [%s] Current limitSecs=%ss limitBurst=%sx (restarted %sx)",
                            me,
                            unit,
                            limitSecs,
                            limitBurst,
                        ...
                        if len(restarted) >= limitBurst:
                            logg.info(
                                "[%s] [%s] Blocking Restart - oldest %s is %s ago (allowed %s)",
                                me,
                                unit,
                                oldest,
                                interval,
                ...
                if unit:  # not dropped out
                    if unit not in self._restart_failed_units:
                        ...
                        logg.debug(
                            "[%s] [%s] restart scheduled in %+.3fs",
                            me,
                            unit,
                            (self._restart_failed_units[unit] - now),
                        )
            except Exception as e:
                logg.error(
                    "[%s] [%s] An error occurred while restart checking: %s",
                    me,
                    unit,
                    e,
                )
        ...
        restart_done = []
        logg.debug(
            "[%s] Restart checking  %s",
            me,
            ["%+.3fs" % (t - now) for t in self._restart_failed_units.values()],
        )
        for unit in sorted(self._restart_failed_units):
            ...
            try:
                ...
                logg.error(
                    "[%s] [%s] An error occurred while restarting: %s", me, unit, e
                )
        for unit in restart_done:
            if unit in self._restart_failed_units:
                del self._restart_failed_units[unit]
        logg.debug(
            "[%s] Restart remaining %s",
            me,
            ["%+.3fs" % (t - now) for t in self._restart_failed_units.values()],
        )
        return restart_done
    ...
    def init_loop_until_stop(self, units):
        ...
        signal.signal(
            signal.SIGQUIT,
            lambda signum, frame: ignore_signals_and_raise_keyboard_interrupt(
                "SIGQUIT"
            ),
        )
        signal.signal(
        ...
        while True:
            try:
                ...
                if DEBUG_INITLOOP:  # pragma: no cover
                    logg.debug(
                        "reap zombies - init-loop found %s running procs", running
                    )
    ...
    def sysinit_target(self):
        if not self._sysinit_target:
            self._sysinit_target = self.default_unit_conf(
                SysInitTarget, "System Initialization"
            )
                ...
                if pid:
                    try:
                        ...
                        if len(cmd) > 1 and cmd_exe.startswith("python"):
                            X = 1
                            while cmd[X].startswith("-"):
                                X += 1  # atleast '-u' unbuffered
                            cmd_arg = os.path.basename(cmd[X])
                            if DEBUG_KILLALL:
                                logg.debug("cmd.arg '%s'", cmd_arg)
                            if fnmatch.fnmatchcase(cmd_arg, target):
                                found = "arg"
                            if cmd_exe.startswith("coverage") or cmd_arg.startswith(
                                "coverage"
                            ):
                                x = cmd.index("--")
                        ...
                        if found:
                            if DEBUG_KILLALL:
                                logg.debug(
                                    "%s found %s %s", found, pid, [c for c in cmd]
                                )
                            if pid != os.getpid():
    ...
    def force_ipv4(self, *args):
        ...
        for line in open(sysconf_hosts):
            if "::1" in line:
                ...
                if line != newline:
                    logg.info(
                        "%s: '%s' => '%s'", _etc_hosts, line.rstrip(), newline.rstrip()
                    )
                    line = newline
            lines.append(line)
...
def print_begin(argv, args):
    ...
    if _root and not is_good_root(_root):
        root44 = path44(_root)
        logg.warning(
            "the --root=%s should have atleast three levels /tmp/test_123/root", root44
        )
...
if __name__ == "__main__":
    ...
    _o.add_option("--version", action="store_true", help="Show package version")
    _o.add_option(
        "--system",
        action="store_true",
        default=False,
    ...
    _o.add_option("--kill-who", metavar="WHO", help="Who to send signal to (ignored)")
    _o.add_option(
        "-s", "--signal", metavar="SIG", help="Which signal to send (ignored)"
    )
    _o.add_option(
        "--now",
        action="store_true",
    ...
    _o.add_option(
        "-q", "--quiet", action="store_true", default=_quiet, help="Suppress output"
    )
    _o.add_option(
        "--no-block",
        action="store_true",
    ...
    _o.add_option(
        "-n", "--lines", metavar="NUM", help="Number of journal entries to show"
    )
    _o.add_option(
        "-o",
        "--output",
    ...
    for setting in opt.config:
        ...
        if nam in globals():
            ...
            if old is False or old is True:
                ...
                logg.warning(
                    "(ignored) unknown target type -c '%s' : %s", nam, type(old)
                )
        else:
            logg.warning(
                "(ignored) unknown target config -c '%s' : no such variable", nam
            )
    #
    systemctl_debug_log = os_path(
        _root, expand_path(SYSTEMCTL_DEBUG_LOG, not _user_mode)
    )
    systemctl_extra_log = os_path(
        _root, expand_path(SYSTEMCTL_EXTRA_LOG, not _user_mode)
    )
    if os.access(systemctl_extra_log, os.W_OK):
        loggfile = logging.FileHandler(systemctl_extra_log)
        loggfile.setFormatter(
