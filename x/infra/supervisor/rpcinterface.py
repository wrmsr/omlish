...
class SupervisorNamespaceRPCInterface:
    ...
    def _update(self, text):
        ...
        if (
                isinstance(self.supervisord.options.mood, int) and
    ...
    def _getAllProcesses(self, lexical=False):
        # if lexical is true, return processes sorted in lexical order, otherwise, sort in priority order
        all_processes = []
    ...
    def startProcess(self, name, wait=True):
        ...
        # test filespec, don't bother trying to spawn if we know it will eventually fail
        try:
        ...
        # We call reap() in order to more quickly obtain the side effects of process.finish(), which reap() eventually
        # ends up calling.  This might be the case if the spawn() was successful but then the process died before its
        # startsecs elapsed or it exited with an unexpected exit code. In particular, finish() may set spawnerr, which
        # we can check and immediately raise an RPCError, avoiding the need to defer by returning a callback.
        ...
        # We call process.transition() in order to more quickly obtain its side effects.  In particular, it might set
        # the process' state from STARTING->RUNNING if the process has a startsecs==0.
        process.transition()

        if wait and process.get_state() != ProcessStates.RUNNING:
            # by default, this branch will almost always be hit for processes with default startsecs configurations,
            # because the default number of startsecs for a process is "1", and the process will not have entered the
            # RUNNING state yet even though we've called transition() on it.  This is because a process is not
            # considered RUNNING until it has stayed up > startsecs.
    ...
    def stopProcess(self, name, wait=True):
        ...
        # We'll try to reap any killed child. FWIW, reap calls waitpid, and then, if waitpid returns a pid, calls
        # finish() on the process with that pid, which drains any I/O from the process' dispatchers and changes the
        # process' state.  I chose to call reap without once=True because we don't really care if we reap more than one
        # child.  Even if we only reap one child. we may not even be reaping the child that we just stopped (this is all
        # async, and process.stop() may not work, and we'll need to wait for SIGKILL during process.transition() as the
        ...
        if wait and process.get_state() not in STOPPED_STATES:
            def onwait():
                # process will eventually enter a stopped state by virtue of the supervisord.reap() method being called
    ...
    def signalProcessGroup(self, name, signal):
        ...
        sendall = make_allfunc(processes, isSignallable, self.signalProcess, signal=signal)
        result = sendall()
        self._update('signalProcessGroup')
...
def make_allfunc(processes, predicate, func, **extra_kwargs):
    ...
    # XXX the above implementation has a weakness inasmuch as the first call into each individual process callback will
    # always return NOT_DONE_YET, so they need to be called twice.  The symptom of this is that calling this method
    # causes the client to block for much longer than it actually requires to kill all of the running processes.  After
    # the first call to the killit callback, the process is actually dead, but the above killall method processes the
    # callbacks one at a time during the select loop, which, because there is no output from child processes after e.g.
    # stopAllProcesses is called, is not busy, so hits the timeout for each callback.  I attempted to make this better,
    # but the only way to make it better assumes totally synchronous reaping of child processes, which requires
    # infrastructure changes to supervisord that are scary at the moment as it could take a while to pin down all of the
    # platform differences and might require a C extension to the Python signal module to allow the setting of ignore
    # flags to signals.
    return allfunc
