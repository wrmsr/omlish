...
# Obtain possible configuration from the environment, assuming 1 (on) by default, upon 0 set to None. Should
# instructively fail if some non 0/1 value is set.
...
if mp is not None:
    try:
        # try to create a named semaphore using SemLock to make sure they are available on this platform. We use the low
        # level object _multiprocessing.SemLock to avoid spawning a resource tracker on Unix system or changing the
        # default backend.
        import tempfile
