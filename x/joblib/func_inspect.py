...
def get_func_code(func):
    """ Attempts to retrieve a reliable function code hash.
        ...
        The reason we don't use inspect.getsource is that it caches the source, whereas we want this to be modified on
        the fly when the function is modified.
        ...
        Notes
        ------
        This function does a bit more magic than inspect, and is thus more robust.
    """
    source_file = None
    try:
        code = func.__code__
        ...
        if not os.path.exists(source_file):
            # Use inspect for lambda functions and functions defined in an interactive shell, or in doctests
...
def get_func_name(func, resolv_alias=True, win_characters=True):
    ...
    if module == '__main__':
        ...
        if filename is not None:
            ...
            if parts[-1].startswith('<ipython-input'):
                # We're in a IPython (or notebook) session. parts[-1] comes from func.__code__.co_filename and is of the
                # form <ipython-input-N-XYZ>, where:
                # - N is the cell number where the function was defined
                # - XYZ is a hash representing the function's code (and name). It will be consistent across sessions and
                #   kernel restarts, and will change if the function's code/name changes
                # We remove N so that cache is properly hit if the cell where the func is defined is re-exectuted. The
                # XYZ hash should avoid collisions between functions with the same name, both within the same notebook
                # but also across notebooks
                splitted = parts[-1].split('-')
                parts[-1] = '-'.join(splitted[:2] + splitted[3:])
            elif len(parts) > 2 and parts[-2].startswith('ipykernel_'):
                # In a notebook session (ipykernel). Filename seems to be 'xyz' of above. parts[-2] has the structure
                # ipykernel_XXXXXX where XXXXXX is a six-digit number identifying the current run (?). If we split it
                # off, the function again has the same identifier across runs.
                parts[-2] = 'ipykernel'
            filename = '-'.join(parts)
    ...
    if hasattr(func, '__qualname__') and func.__qualname__ != name:
        # Extend the module name in case of nested functions to avoid (module, name) collisions
...
def filter_args(func, ignore_lst, args=(), kwargs=dict()):
    """ Filters the given args and kwargs using a list of arguments to ignore, and a function specification.
        ...
        ignore_lst: list of strings
            List of arguments to ignore (either a name of an argument in the function spec, or '*', or '**')
        *args: list
    ...
    if inspect.ismethod(func):
        # First argument is 'self', it has been removed by Python we need to add it back:
        args = [func.__self__, ] + args
        # func is an instance method, inspect.signature(func) does not include self, we need to fetch it from the class
        # method, i.e func.__func__
    ...
    # XXX: Maybe I need an inspect.isbuiltin to detect C-level methods, such as on ndarrays.
    ...
    for item in ignore_lst:
        if item in arg_dict:
            ...
            raise ValueError("Ignore list: argument '%s' is not defined for function %s"
