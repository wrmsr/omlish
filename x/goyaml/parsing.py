...
def create_grouped_tokens(tokens: tokens.Tokens) -> tuple[list[Token] | None, str | None]:
    err: str | None = None
...
def create_line_comment_token_groups(tokens: list[Token]) -> list[Token]:
    ret: list[Token] = []
...
def create_literal_and_folded_token_groups(tokens: list[Token]) -> tuple[list[Token], str | None]:
    ret: list[Token] = []
    i = -1
    while True:
        i += 1
...
def create_anchor_and_alias_token_groups(tokens: list[Token]) -> tuple[list[Token] | None, str | None]:
    ret: list[Token] = []
    i = -1
    while True:
        i += 1
...
def create_scalar_tag_token_groups(tokens: list[Token]) -> tuple[list[Token] | None, str | None]:
    ret: list[Token] = []
    i = -1
    while True:
        i += 1
...
def create_anchor_with_scalar_tag_token_groups(tokens: list[Token]) -> tuple[list[Token] | None, str | None]:
    ret: list[Token] = []
    i = -1
    while True:
        i += 1
...
def create_map_key_token_groups(tokens: list[Token]) -> tuple[list[Token] | None, str | None]:
...
def create_map_key_by_mapping_key(tokens: list[Token]) -> tuple[list[Token] | None, str | None]:
    ret: list[Token] = []
    i = -1
    while True:
        i += 1
...
def create_map_key_by_mapping_value(tokens: list[Token]) -> tuple[list[Token] | None, str | None]:
    ret: list[Token] = []
    i = -1
    while True:
        i += 1
...
def create_map_key_value_token_groups(tokens: list[Token]) -> list[Token]:
    ret: list[Token] = []
    i = -1
    while True:
        i += 1
...
def create_directive_token_groups(tokens: list[Token]) -> tuple[list[Token] | None, str | None]:
    ret: list[Token] = []
    i = -1
    while True:
        i += 1
...
def create_document_tokens(tokens: list[Token]) -> tuple[list[Token] | None, str | None]:
    ret: list[Token] = []
    i = -1
    while True:
        i += 1
...
) -> tuple[ast.MappingNode | None, str | None]:
...
def new_mapping_key_node(ctx: Context, tk: Token) -> tuple[ast.MappingKeyNode | None, str | None]:
...
def new_anchor_node(ctx: Context, tk: Token) -> tuple[ast.AnchorNode | None, str | None]:
...
def new_alias_node(ctx: Context, tk: Token) -> tuple[ast.AliasNode | None, str | None]:
...
def new_directive_node(ctx: Context, tk: Token) -> tuple[ast.DirectiveNode | None, str | None]:
...
def new_merge_key_node(ctx: Context, tk: Token) -> tuple[ast.MergeKeyNode | None, str | None]:
...
def new_null_node(ctx: Context, tk: Token) -> tuple[ast.NullNode | None, str | None]:
...
def new_bool_node(ctx: Context, tk: Token) -> tuple[ast.BoolNode | None, str | None]:
...
def new_integer_node(ctx: Context, tk: Token) -> tuple[ast.IntegerNode | None, str | None]:
...
def new_float_node(ctx: Context, tk: Token) -> tuple[ast.FloatNode | None, str | None]:
...
def new_infinity_node(ctx: Context, tk: Token) -> tuple[ast.InfinityNode | None, str | None]:
...
def new_nan_node(ctx: Context, tk: Token) -> tuple[ast.NanNode | None, str | None]:
...
def new_string_node(ctx: Context, tk: Token) -> tuple[ast.StringNode | None, str | None]:
...
def new_literal_node(ctx: Context, tk: Token) -> tuple[ast.LiteralNode | None, str | None]:
...
def new_tag_node(ctx: Context, tk: Token) -> tuple[ast.TagNode | None, str | None]:
...
def new_sequence_node(ctx: Context, tk: Token, is_flow: bool) -> tuple[ast.SequenceNode | None, str | None]:
...
def new_tag_default_scalar_value_node(ctx: Context, tag: tokens_.Token) -> tuple[ast.ScalarNode | None, str | None]:
...
def set_line_comment(ctx: Context, node: ast.Node, tk: Token | None) -> str | None:
...
def set_head_comment(cm: ast.CommentGroupNode | None, value: ast.Node) -> str | None:
    if cm is None:
        return None
    n = value
...
class Parser:
    ...
    ) -> tuple[ta.Optional['Parser'], str | None]:
        filtered_tokens: list[tokens_.Token] = []
    ...
    def parse(self, ctx: Context) -> tuple[ast.File | None, str | None]:
        file = ast.File(docs=[])
    ...
    def parse_document(self, ctx: Context, doc_group: TokenGroup) -> tuple[ast.DocumentNode | None, str | None]:
    ...
    def parse_scalar_value(self, ctx: Context, tk: Token) -> tuple[ast.ScalarNode | None, str | None]:
    ...
    def validate_map_key_value_next_token(self, ctx: Context, key_tk, tk: Token) -> str | None:
        if tk is None:
            return None
    ...
    def validate_map_key(self, ctx: Context, tk: tokens_.Token, key_path: str, colon_tk: Token) -> str | None:
    ...
    def parse_map_value(self, ctx: Context, key: ast.MapKeyNode, colon_tk: Token) -> tuple[ast.Node | None, str | None]:
        tk = ctx.current_token()
        if tk is None:
    ...
    def validate_anchor_value_in_map_or_seq(self, value: ast.Node, col: int) -> str | None:
    ...
    def parse_anchor_name(self, ctx: Context) -> tuple[ast.AnchorNode | None, str | None]:
    ...
    def parse_alias(self, ctx: Context) -> tuple[ast.AliasNode | None, str | None]:
    ...
    def parse_scalar_tag(self, ctx: Context) -> tuple[ast.TagNode | None, str | None]:
    ...
    def parse_flow_sequence(self, ctx: Context) -> tuple[ast.SequenceNode | None, str | None]:
    ...
    def parse_sequence(self, ctx: Context) -> tuple[ast.SequenceNode | None, str | None]:
    ...
    def parse_sequence_value(self, ctx: Context, seq_tk: Token) -> tuple[ast.Node | None, str | None]:
        tk = ctx.current_token()
        if tk is None:
    ...
    def parse_directive(self, ctx: Context, g: TokenGroup) -> tuple[ast.DirectiveNode | None, str | None]:
    ...
    def parse_comment(self, ctx: Context) -> tuple[ast.Node | None, str | None]:
        cm = self.parse_head_comment(ctx)
        if ctx.is_token_not_found():
            return cm, None
        node, err = self.parse_token(ctx, ctx.current_token())
    ...
    def parse_head_comment(self, ctx: Context) -> ast.CommentGroupNode | None:
        tks: list[tokens_.Token] = []
    ...
    def parse_foot_comment(self, ctx: Context, col: int) -> ast.CommentGroupNode | None:
        tks: list[tokens_.Token] = []
