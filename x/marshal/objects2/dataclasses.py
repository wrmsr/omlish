"""
Dataclass marshaling/unmarshaling support.

Provides factories for creating marshalers/unmarshalers for dataclasses, with support for
field metadata, naming conventions, embedded fields, and more.
"""
import typing as ta
...
from omlish.lite import marshal as lm
...
from omlish.marshal.base.errors import UnhandledTypeError
from omlish.marshal.base.options import Option
...
from omlish.marshal.naming import translate_name

from .marshal import ObjectMarshaler
from .metadata import DEFAULT_FIELD_METADATA
...
def get_dataclass_field_infos(
    ...
    obj_md = get_dataclass_metadata(ty)
        ...
        def inner() -> Unmarshaler:
            ...
            def add_field(fi: FieldInfo, *, prefixes: ta.Iterable[str] = ('',)) -> ta.Iterable[str]:
                ...
                if fi.metadata.embed:
                    ...
                    if fi.metadata.default.present:
                        defaults[fi.name] = fi.metadata.default.must()
