...
def ggml_data_to_tensor(t: Tensor, n: int, ggml_type: int) -> Tensor:
    ...
    def q_to_uint8(t: Tensor, b: int) -> Tensor:
        ...
        return (
            t.unsqueeze(-1)
            .expand((*t.shape, 8 // b))
            .div(shift_tensor, rounding_mode='trunc')
            .bitwise_and(bitmask)
            .transpose(-1, -2)
            .flatten(-2)
        )
    ...
    if (nelements_nbytes := _GGML_QUANT.get(ggml_type)) is not None:
        ...
        blocks = (
            t[: (n // nelements_nbytes[0]) * nelements_nbytes[1]]
            .reshape((-1, nelements_nbytes[1]))
            .contiguous()
        )

        if ggml_type == 2:
            return (
                    (q_to_uint8(blocks[:, 2:], 4).bitcast(dtypes.int8) - 8)
        ...
        if ggml_type in (6, 7):
            ...
            qh = (
                q_to_uint8(blocks[:, qh_off : qh_off + 4], 1)
                .reshape((-1, 8, 4))
                .transpose(-1, -2)
                .flatten(-2)
                .bitcast(dtypes.int8)
            ...
            return q * d + (
                blocks[:, 2:4].bitcast(dtypes.float16).cast(dtypes.float32)
                if ggml_type == 7
                else -16 * d
            )
        ...
        if ggml_type in (12, 13):
            ...
            s = blocks[
                :, 4:16,
            ]  # 12 bytes: 6-bit scales[0-3], 6-bit mins[0-3], high bits[4-7]
            sc = (
                s[:, 0:4]
                .bitwise_and(63)
                .cat(
                    s[:, 8:12]
                    .bitwise_and(0xF)
                    .bitwise_or(s[:, 0:4].rshift(6).lshift(4)),
                    dim=-1,
                )
            )
            mn = (
                ...
                .bitwise_and(63)
                .cat(
                    s[:, 8:12].rshift(4).bitwise_or(s[:, 4:8].rshift(6).lshift(4)),
                    dim=-1,
                )
            )
            qs_off = 48 if ggml_type == 13 else 16
            q = Tensor.stack(
                (qs := blocks[:, qs_off : qs_off + 128].reshape(-1, 4, 32)).bitwise_and(0xF),
                qs.rshift(4),
                dim=2,
            ).reshape(-1, 8, 32)
            if ggml_type == 13:
                q = q + q_to_uint8(blocks[:, 16:48], 1).reshape(-1, 8, 32) * 16
        ...
        if ggml_type == 18:
            ...
            db = (
                d
                * (scale_words.rshift(28).cast(dtypes.float32) + 0.5).reshape((-1, 8, 1, 1))
                * 0.5
            )
            sign_idx = (
                scale_words.unsqueeze(-1)
                .rshift(Tensor([0, 7, 14, 21], device=t.device, dtype=dtypes.uint32))
                .bitwise_and(0x7F)
                .reshape((-1, 32))
                .cast(dtypes.int32)
            )
            even_signs = Tensor(
                [i | (0x80 if i.bit_count() % 2 else 0) for i in range(128)],
                dtype=dtypes.uint8,
                device=t.device,
            )
            signs = (
                (q_to_uint8(even_signs[sign_idx].reshape((-1, 32, 1)), 1) == 0)
                .where(1.0, -1.0)
                .reshape((-1, 8, 4, 8))
            )
            grid = _ggml_iq_grid(t.device, _ggml.iq3xxs_grid, (256, 4))[
                blocks[:, 2:66]
            ].reshape((-1, 8, 4, 8))
        ...
        if ggml_type == 21:
            ...
            scales = (
                (1 + 2 * q_to_uint8(blocks[:, 106:110].reshape((-1, 4, 1)), 4).reshape((-1, 8)))
                .cast(dtypes.float32)
                .reshape((-1, 8, 1, 1))
            )
            qh = (
                q_to_uint8(blocks[:, 66:74].reshape((-1, 8, 1)), 1)
                .reshape((-1, 64))
                .cast(dtypes.uint16)
            )
            signs = (
                (q_to_uint8(blocks[:, 74:106].reshape((-1, 32, 1)), 1).reshape((-1, 256)) == 0)
                .where(1.0, -1.0)
                .reshape((-1, 8, 4, 8))
            )
            ...
            return (
                d
                * scales
                * _ggml_iq_grid(t.device, _ggml.iq3s_grid, (512, 4))[q].reshape((-1, 8, 4, 8))
                * signs
            ).flatten(-3)
        ...
        if ggml_type == 22:
            ...
            db = (
                d
                * (
                    q_to_uint8(blocks[:, 74:82].reshape((-1, 8, 1)), 4)
                    .reshape((-1, 16))
                    .cast(dtypes.float32)
                    + 0.5
                ).reshape((-1, 16, 1, 1))
            ...
            signs = (
                (q_to_uint8(blocks[:, 34:66].reshape((-1, 32, 1)), 1) == 0)
                .where(1.0, -1.0)
                .reshape((-1, 16, 2, 8))
            )
            qh = (
                q_to_uint8(blocks[:, 66:74].reshape((-1, 8, 1)), 2)
                .reshape((-1, 32))
                .cast(dtypes.uint16)
            )
            ...
            return (
                db
                * _ggml_iq_grid(t.device, _ggml.iq2s_grid, (1024, 8))[q].reshape(
                    (-1, 16, 2, 8),
                )
                * signs
            ).flatten(-3)
        ...
        if ggml_type == 23:
            ...
            scale_shifts = Tensor(
                [0, 2, 4, 6, 8, 10, 12, 14], device=t.device, dtype=dtypes.uint16,
            )
            iq4_xs_lut = Tensor(
                list(_ggml.kvalues_iq4nl), dtype=dtypes.float32, device=t.device,
            )
            scales_l = Tensor.stack(
                (sl := blocks[:, 4:8]).bitwise_and(0xF), sl.rshift(4), dim=2,
            ).reshape((-1, 8))
            scales_h = (
                blocks[:, 2:4]
            ...
            scales = (
                (scales_l.bitwise_or(scales_h.lshift(4)).bitcast(dtypes.int8) - 32)
                .cast(dtypes.float32)
                .reshape((-1, 8, 1))
            )
            q = (
                (qs := blocks[:, 8:].reshape((-1, 8, 16)))
                .bitwise_and(0xF)
                .cat(qs.rshift(4), dim=2)
            )
        ...
        if ggml_type == 39:
            ...
            small_bits = Tensor(
                [0x00200000, 0x00400000], dtype=dtypes.uint32, device=t.device,
            )[
                e.clip(0, 1).cast(dtypes.int32)
            ]  # e = 0 or e = 1 case
            d = (
                (e < 2)
                .where(small_bits, ((e - 1) * 0x00800000).cast(dtypes.uint32))
...
def _read_unpack(fmt: str, n: int, r: io.BufferedIOBase):
    return struct.unpack(fmt, r.read(n))[0]


def read_str(r: io.BufferedIOBase):
    return str(r.read(read_uint64(r)), 'utf-8')
...
readers: dict[int, ta.Callable[[io.BufferedIOBase], ta.Any]] = {
    8: read_str,
    9: read_arr,
    **{
        t: functools.partial(_read_unpack, '<' + f, nb)
        for t, f, nb in [
            (0, 'c', 1),
            (1, 'b', 1),
            (2, 'H', 2),
...
def _gguf_parse(tensor: Tensor) -> tuple[dict, dict[str, Tensor]]:
    ...
    t_infos = [
        (
            read_str(r),
            tuple(read_uint64(r) for _ in range(read_uint32(r))),
            read_int32(r),
            read_uint64(r),
        )
    ...
    state_dict = {
        name: ggml_data_to_tensor(tensor[data_start + off :], prod(dims), typ).reshape(
            *reversed(dims),
        )
...
def _gguf_split_paths(path: pathlib.Path, kv: dict) -> list[pathlib.Path]:
    if (total := kv.get('split.count', 1)) <= 1:
        return [path]
    if kv.get('split.no', 0) != 0:
        raise ValueError(f"multi-part GGUF must be loaded from the first split, got split.no={kv['split.no']}")
    if not (m := re.match(r'^(.*)-00001-of-\d{5}\.gguf$', str(path))):
        raise ValueError(f'first split path must end with -00001-of-NNNNN.gguf: {path}')
    return [
        pathlib.Path(f'{m.group(1)}-{i:05d}-of-{total:05d}.gguf')
...
def gguf_load(fn: Tensor | str | pathlib.Path) -> tuple[dict, dict[str, Tensor]]:
    ...
    for pp in _gguf_split_paths(pathlib.Path(fn), kv)[1:]:
        sd.update(_gguf_parse(Tensor(pp))[1])
    return kv, sd
