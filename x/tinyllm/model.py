...
def precompute_freqs_cis(
    ...
    freqs = 1.0 / (
        theta ** (Tensor.arange(0, dim, 2, device=device)[: (dim // 2)] / dim)
    )
...
class ExpertWeights:
    ...
    def __call__(self, sel: Tensor, x: Tensor) -> Tensor:
        ...
        return (
            (x.unsqueeze(-2) @ self.weight[sel].transpose(-1, -2))
            .contiguous()
            .squeeze(-2)
        )
...
def pairwise_topk(x: Tensor, k: int) -> tuple[Tensor, Tensor]:
    n = x.shape[-1]
    vals = (
        Tensor.arange(n, device=x.device).reshape(1, 1, n).cast(x.dtype).expand(x.shape)
    )
    cmp = (
        (x.unsqueeze(-1) > x.unsqueeze(-2))
        | (
            (x.unsqueeze(-1) == x.unsqueeze(-2))
            & (
                Tensor.arange(n, device=x.device).reshape(1, 1, n, 1)
                < Tensor.arange(n, device=x.device).reshape(1, 1, 1, n)
            )
        )
    )
    sel = (
        x.const_like(0)
        .scatter(-1, cmp.sum(axis=-1).cast('int32'), vals)[:, :, n - k:]
        .cast('int32')
    )
    ...
    def __init__(self, config: TransformerConfig) -> None:
        ...
        if config.num_experts > 0:
            ...
            if config.expert_bias:
                self.exp_probs_b = {'bias': Tensor.zeros(config.num_experts)}
        ...
        if hasattr(self, 'ffn_gate_exps'):
            ...
            if hasattr(self, 'exp_probs_b'):
                ...
                probs = (
                    vals.softmax(-1)
                    if self.config.norm_topk_prob
                    else logits.softmax(-1).gather(-1, sel)
                )

            probs = probs * self.config.routed_scaling_factor
            x_down = self.ffn_down_exps(
                sel,
            ...
            if hasattr(self, 'ffn_gate_shexp'):
                shexp = self.ffn_down_shexp(
                    self.ffn_gate_shexp(x).silu().contiguous() * self.ffn_up_shexp(x),
                )
                if hasattr(self, 'ffn_gate_inp_shexp'):
                    shexp = (
                        shexp
                        * (x * self.ffn_gate_inp_shexp['weight'])
...
class TransformerBlock(FFNBlock):
    def __init__(self, config: TransformerConfig) -> None:
        ...
        check.arg(config.v_head_dim == config.head_dim, 'TransformerBlock requires v_head_dim == head_dim')
        ...
        if config.qk_norm:
            self.attn_q_norm, self.attn_k_norm = (
                nn.RMSNorm(config.qk_norm, config.norm_eps),
                nn.RMSNorm(config.qk_norm, config.norm_eps),
            )
    ...
    def _attention(self, x: Tensor, start_pos: int | UOp) -> Tensor:
        ...
        if self.config.attn_output_gate:
            ...
            q, gate = (
                qg[:, :, :, 0, :],
                qg[:, :, :, 1, :].reshape(B, T, self.config.n_heads * self.config.head_dim),
            )
        ...
        q = apply_rope(
            q[..., : self.config.rope_dim],
            self.freqs_cis[start_pos : start_pos + T],
        ).cat(q[..., self.config.rope_dim :], dim=-1)
        k = apply_rope(
            k[..., : self.config.rope_dim],
            self.freqs_cis[start_pos : start_pos + T],
        ).cat(k[..., self.config.rope_dim :], dim=-1)

        # NOTE: we don't want to change self.cache_kv, the function API doesn't support this well
        assigned_kv = Tensor(
            self.cache_kv.uop.after(
        ...
        return self.attn_output(
            attn if not self.config.attn_output_gate else (attn * gate.sigmoid()),
        )
...
class MLATransformerBlock(FFNBlock):
    def __init__(self, config: TransformerConfig) -> None:
        ...
        self.attn_k_b = {
            'weight': Tensor.zeros(
                config.n_heads, config.kv_lora_rank, qk_nope_head_dim,
            ),
        }
        self.attn_v_b = {
            'weight': Tensor.zeros(
                config.n_heads, config.v_head_dim, config.kv_lora_rank,
            ),
        }
    ...
    def _attention(self, x: Tensor, start_pos: int | UOp) -> Tensor:
        ...
        q_proj = (
            self.attn_q_b(self.attn_q_a_norm(self.attn_q_a(x)))
            if self.config.q_lora_rank > 0
            else self.attn_q(x)
        )
        q = q_proj.reshape(B, T, self.config.n_heads, self.config.head_dim).transpose(1, 2)
        q_nope, q_rope = q[..., :q_nope_head_dim], q[..., q_nope_head_dim:]
        q = (q_nope @ self.attn_k_b['weight'].transpose(-1, -2)).cat(
            apply_rope(q_rope, self.freqs_cis[start_pos : start_pos + T]), dim=-1,
        )

        kv_a = self.attn_kv_a_mqa(x)
        c_kv = self.attn_kv_a_norm(kv_a[..., : self.config.kv_lora_rank])
        k_rope = apply_rope(
            kv_a[..., self.config.kv_lora_rank :]
            .reshape(B, T, 1, self.config.rope_dim)
            .transpose(1, 2),
            self.freqs_cis[start_pos : start_pos + T],
        )

        k_store = c_kv.reshape(B, 1, T, self.config.kv_lora_rank).cat(
            k_rope.reshape(B, 1, T, self.config.rope_dim), dim=-1,
        )
        k = Tensor(
            self.cache_k.uop.after(
                self.cache_k[:, :, start_pos : start_pos + T, :].uop.store(k_store.uop),
            ),
        )[:, :, 0 : start_pos + T, :]
        ...
        attn = attn.softmax(-1)
        attn = (
            ((attn @ v) @ self.attn_v_b['weight'].transpose(-1, -2))
            .transpose(1, 2)
            .reshape(B, T, -1)
        )
    ...
    def _init_state(self, x: Tensor) -> None:
        if not hasattr(self, 'cache_k'):
            self.cache_k = Tensor.empty(
                x.shape[0],
                1,
                self.config.max_context,
                self.config.kv_lora_rank + self.config.rope_dim,
                device=x.device,
            )
            self.freqs_cis = precompute_freqs_cis(
                self.config.rope_dim,
...
class GatedDeltaNetBlock(FFNBlock):
    def __init__(self, config: TransformerConfig, ssm: SSMConfig) -> None:
        super().__init__(config)

        self.head_k_dim, self.num_k_heads, self.num_v_heads = (
            ssm.state_size,
            ssm.group_count,
            ssm.time_step_rank,
        )
        check.arg(self.num_v_heads % self.num_k_heads == 0)
        self.head_v_dim, self.ssm_conv_kernel = (
            ssm.inner_size // ssm.time_step_rank,
            ssm.conv_kernel,
        )
        self.conv_channels, self.q_dim = (
            ssm.inner_size + 2 * ssm.group_count * ssm.state_size,
            ssm.state_size * ssm.group_count,
        )
        self.attn_qkv, self.attn_gate = nn.Linear(
            config.dim, self.conv_channels, bias=False,
        ), nn.Linear(config.dim, ssm.inner_size, bias=False)
        self.ssm_alpha, self.ssm_beta = nn.Linear(
            config.dim, self.num_v_heads, bias=False,
        ), nn.Linear(config.dim, self.num_v_heads, bias=False)
        self.ssm_conv1d = {
            'weight': Tensor.zeros(self.conv_channels, self.ssm_conv_kernel),
        }
        self.ssm_dt = {'bias': Tensor.zeros(self.num_v_heads)}
        self.ssm_a = Tensor.zeros(self.num_v_heads)
        self.ssm_norm, self.ssm_out = nn.RMSNorm(
            self.head_v_dim, config.norm_eps,
        ), nn.Linear(ssm.inner_size, config.dim, bias=False)

    def _attention(self, x: Tensor, start_pos: int | UOp) -> Tensor:
        B, T, _ = x.shape
        check.arg(T == 1, 'GatedDeltaNetBlock currently only supports T=1')
        ...
        alpha = (
            ((self.ssm_alpha(x).float() + self.ssm_dt['bias']).softplus() * self.ssm_a)
            .reshape(B, self.num_v_heads, 1, 1)
            .exp()
        )
        ...
        conv_out = (
            (conv_window * self.ssm_conv1d['weight'].T.unsqueeze(0)).sum(1).silu()
        )
        q, k, v = conv_out.split(
            [self.q_dim, self.q_dim, self.conv_channels - 2 * self.q_dim], dim=-1,
        )
        q = (
            q.reshape(B, self.num_k_heads, self.head_k_dim)
            .normalize(dim=-1)
            .repeat(1, self.num_v_heads // self.num_k_heads, 1)
        )
        k = (
            k.reshape(B, self.num_k_heads, self.head_k_dim)
            .normalize(dim=-1)
            .repeat(1, self.num_v_heads // self.num_k_heads, 1)
        )
        v = v.reshape(B, self.num_v_heads, self.head_v_dim)
        q, k, v = (
            q.mul(self.head_k_dim**-0.5).unsqueeze(-1),
            k.unsqueeze(-1),
            v.unsqueeze(-1),
        )
        ...
        core_attn_out = self.ssm_norm(
            (recurrent_state @ q)
            .squeeze(-1)
            .reshape(B, 1, self.num_v_heads, self.head_v_dim),
        )
        return self.ssm_out(
            (core_attn_out * out_gate.silu()).reshape(B, 1, -1).cast(x.dtype),
        )
    ...
    def _state_reset_ops(self):
        return (
            [
                self.conv_state.assign(self.conv_state.const_like(0)),
                self.recurrent_state.assign(self.recurrent_state.const_like(0)),
            ]
            if hasattr(self, 'conv_state')
            else []
        )

    def _reusable_prefix_len(self, prefix_len: int, cached_len: int) -> int:
        return 0 if prefix_len != cached_len else prefix_len

    def _init_state(self, x):
        if not hasattr(self, 'conv_state'):
            self.conv_state = Tensor.zeros(
                x.shape[0],
                self.ssm_conv_kernel - 1,
                self.conv_channels,
                device=x.device,
            ).clone()
...
class Transformer:
    def __init__(self, config: TransformerConfig) -> None:
        ...
        if config.ssm:
            config = dc.replace(config, qk_norm=config.head_dim)
        block_cls = MLATransformerBlock if config.kv_lora_rank > 0 else TransformerBlock
        self.blk: list[FFNBlock] = [
            (
                GatedDeltaNetBlock(config, config.ssm)
                if config.ssm and (i + 1) % config.full_attention_interval != 0
                else block_cls(
                    dense_config if i < config.leading_dense_blocks else config,
                )
            )
        ...
        self.has_recurrent_block = any(
            isinstance(b, GatedDeltaNetBlock) for b in self.blk
        )
    ...
    def forward(
        ...
        return (
                logits / temperature.maximum(1e-12)
                - (Tensor.rand_like(logits).maximum(1e-12).log().neg()).log()
    ...
    def __call__(
        ...
        return (
            self.prefill_jit if resolve(tokens.shape[1] != 1) else self.rollout_jit
        )(tokens.contiguous(), start_pos, temperature)

    @staticmethod
    def from_gguf(
            gguf: Tensor | str | pathlib.Path,
            max_context: int | None = None,
            realize=bool(getenv('REALIZE', 0)),
    ) -> tuple[Transformer, dict]:
        # TODO: remove the need for copy to default device
        kv, state_dict = gguf_load(
            gguf.to(None).realize() if isinstance(gguf, Tensor) else gguf,
        )
        ...
        n_heads, n_kv_heads = (
            kv[f'{arch}.attention.head_count'],
            kv[f'{arch}.attention.head_count_kv'],
        )
        ...
        if arch in ('qwen35', 'qwen35moe', 'glm4moe'):
            state_dict = {
                k.replace('post_attention_norm', 'ffn_norm'): v
                for k, v in state_dict.items()
            }
        ...
        for name in state_dict:
            if ('attn_q.weight' in name or 'attn_q_b.weight' in name) and (arch == 'llama' or kv_lora_rank):
                w = state_dict[name].reshape(
                    n_heads, state_dict[name].shape[0] // n_heads, -1,
                )
                prefix = head_dim - rope_dim
                state_dict[name] = (
                    w[:, :prefix]
                    .cat(
                        w[:, prefix:].rearrange('n (h two) d -> n (two h) d', two=2),
                        dim=1,
                    )
                    .reshape(-1, w.shape[-1])
                )
                ...
                w = state_dict[name].reshape(
                    n_kv_heads, state_dict[name].shape[0] // n_kv_heads, -1,
                )
                state_dict[name] = w.rearrange(
                    'n (h two) d -> n (two h) d', two=2,
                ).reshape(-1, w.shape[-1])

            elif kv_lora_rank and 'attn_kv_a_mqa.weight' in name:
                state_dict[name] = state_dict[name][:kv_lora_rank].cat(
                    state_dict[name][kv_lora_rank:].rearrange(
                        '(h two) d -> (two h) d', two=2,
                    ),
                    dim=0,
                )
        ...
        config = TransformerConfig(
            ...
            hidden_dim=kv.get(
                ...
                kv.get(f'{arch}.feed_forward_length', 0),
            ),
            n_heads=n_heads,
            ...
            v_head_dim=kv.get(
                ...
                kv.get(f'{arch}.attention.value_length', head_dim),
            ),
            max_context=max_context,
            qk_norm=(
                int(state_dict['blk.0.attn_q_norm.weight'].shape[0])
            ...
            norm_topk_prob=kv.get(
                f'{arch}.expert_weights_norm', arch in ('qwen3moe', 'qwen35moe'),
            ),
            ...
            shared_expert_dim=kv.get(
                ...
                kv.get(f'{arch}.expert_shared_count', 0)
                * kv.get(f'{arch}.expert_feed_forward_length', 0),
            ),
            shared_expert_gate=(
                f"blk.{kv.get(f'{arch}.leading_dense_block_count', 0)}.ffn_gate_inp_shexp.weight" in state_dict  # noqa
            ),
            dense_hidden_dim=(
                kv.get(f'{arch}.feed_forward_length', 0)
                if kv.get(f'{arch}.leading_dense_block_count', 0)
                else 0
            ),
            ...
            expert_bias=(
                f"blk.{kv.get(f'{arch}.leading_dense_block_count', 0)}.exp_probs_b.bias" in state_dict
            ),
        )
    ...
    def get_start_pos(self, tokens: list[int]) -> int:
        prefix_len = sum(
            ...
            for _ in itertools.takewhile(
                lambda ab: ab[0] == ab[1],
                zip(tokens[:-1], self._cached_tokens),
            )
        )

        return min(
            block._reusable_prefix_len(prefix_len, len(self._cached_tokens))
            for block in self.blk
        )
    ...
    def generate(
        ...
        t = Tensor(
            tokens + [0] * (self.max_context - len(tokens)), dtype='int32',
        ).reshape(1, self.max_context)

        # recompute start_pos from what's currently valid in the caches
        start_pos = self.get_start_pos(tokens)
        if (
                start_pos < len(self._cached_tokens) and
                (resets := [r for b in self.blk for r in b._state_reset_ops()])
        ):
        ...
        while len(tokens) < self.max_context:
            ...
            out = self(
                t[:, sp: sp + nt] if start_pos < prompt_len or out is None else out,
                sp,
                temp,
            ).realize()
            start_pos += nt.val
