...
class SimpleTokenizer:
    ...
    @staticmethod
    def from_gguf_kv(kv: dict) -> SimpleTokenizer:
        # https://github.com/ggml-org/llama.cpp/blob/94933c8c2eeaa9a7983e3f6c08af76bd86724094/src/llama-vocab.cpp#L1818-L1820
        vocab: ta.Iterable[tuple[str, int]] = (
            (tok, idx) for idx, tok in enumerate(kv['tokenizer.ggml.tokens'])
        )
        normal_tokens, special_tokens = partition(
        ...
        return SimpleTokenizer(
            ...
            bos_id=(
                kv.get('tokenizer.ggml.bos_token_id')
                if kv.get('tokenizer.ggml.add_bos_token', True)
                else None
            ),
    ...
    def stream_decoder(self) -> ta.Callable[..., str]:
        dec = codecs.getincrementaldecoder('utf-8')('replace')

        def _decode(tid: int | None = None) -> str:
            return (
                dec.decode(self._tok2bytes[tid])
                if tid is not None
                else dec.decode(b'', final=True)
            )
    ...
    def role(self, role: str):
        ...
        if self.preset == 'kimi-k2':
            return self.encode('<|im_' + role + '|>' + role + '<|im_middle|>')
        ...
        if self.preset == 'tekken':
            if role == 'user':
                return self.encode('[INST]')
            if role == 'assistant':
                return []
            raise ValueError(
                f"Unsupported role '{role}' for tokenizer preset '{self.preset}'",
            )
    ...
    def end_turn(self):
        ...
        if self.preset == 'kimi-k2':
            return [self.eos_id]
        if self.preset == 'qwen2':
            return [self.eos_id] + self.encode('\n')
        if self.preset == 'glm4':
            return []
        if self.preset == 'tekken':
            return self.encode('[/INST]')
    ...
    def prefix(self) -> list[int]:
        return ([] if self.bos_id is None else [self.bos_id]) + (
            self.encode('<sop>') if self.preset == 'glm4' else []
        )

    def is_end(self, token_id: int) -> bool:
        return token_id in (self.eos_id, self.eot_id)
...
class Handler(HTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == '/v1/models':
            ...
            self.send_data(
                (pathlib.Path(__file__).parent / 'chat.html').read_bytes(),
    ...
    def run_model(
        ...
        for next_id in model.generate(ids, temperature=temperature):
            if len(out) == 0:
                stderr_log(
                    f"prefill:{(len(ids) - cache_start_pos) / ((pt := time.perf_counter()) - st):4.0f} tok/s  {colored('--', 'BLACK')}  ",
                )
        ...
        stderr_log(
            f"gen:{len(out) / (et - pt) if len(out) > 1 else 0:4.0f} tok/s  {colored('--', 'BLACK')}  "
    ...
    def do_POST(self):
        ...
        body: dict[str, ta.Any] = json.loads(raw_body.decode('utf-8'))
        if DEBUG >= 1:
                ...
                if isinstance(content, str):
                    ...
                    for c in content:
                        if c['type'] == 'text':
                            ...
                            raise RuntimeError(f"unhandled type: {c['type']}")
                else:
                    raise RuntimeError(f'unknown content type: {type(content)}')
                if msg['role'] == 'assistant' and i == len(body['messages']) - 1:
                    break
                ids += tok.end_turn()
            else:
...
def main():
    ...
    parser.add_argument(
        ...
        help=f"Model choice ({', '.join(models.keys())}) or path to a local GGUF file",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        const=8000,
        metavar='PORT',
        help='Run OpenAI compatible API (optional port, default 8000)',
    )
    ...
    parser.add_argument(
        ...
        const=20,
        metavar='COUNT',
        help='Benchmark tok/s (optional count, default 20)',
    )
    args = parser.parse_args()

    # load the model
    model, kv = Transformer.from_gguf(
        fetch(models.get(args.model, args.model)), args.max_context,
    )
    model_name = kv.get('general.name') or kv.get('general.basename') or args.model
    file_sizes = [
        y.nbytes()
        for y in UOp.sink(*[x.uop for x in nn.state.get_parameters(model)]).toposort()
        if y.op is Ops.BUFFER
    ]
    print(
        f'using model "{model_name}" with {sum(file_sizes):,} bytes and {sum(x.numel() for x in nn.state.get_parameters(model)):,} params',
    )
    ...
    if args.benchmark is not None:
        ...
        for i in range(args.benchmark):
            ...
            with Timing(
                on_exit=lambda x: f', {1e9/x:6.2f} tok/s, {GlobalCounters.global_mem/x:7.2f} GB/s,'
    ...
    while 1:
        ...
        for next_id in model.generate(ids):
            sys.stdout.write(
                dec(next_id) if not tok.is_end(next_id) else dec() + '\n\n',
            )
            sys.stdout.flush()
            if tok.is_end(next_id):
                break
