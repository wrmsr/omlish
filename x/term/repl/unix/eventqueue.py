...
TERMINAL_KEYNAMES: ta.Mapping[str, str] = {
    'delete': 'kdch1',
    'down': 'kcud1',
    'end': 'kend',
    'enter': 'kent',
    ...
    **{f'f{i:d}': f'kf{i:d}' for i in range(1, 21)},
}


# Known CTRL-arrow keycodes
CTRL_ARROW_KEYCODES: ta.Mapping[bytes, str] = {
...
def get_terminal_keycodes(ti: terminfo.TermInfo) -> dict[bytes, str]:
...
class UnixEventQueue(BaseEventQueue):
    def __init__(self, fd: int, encoding: str, ti: terminfo.TermInfo) -> None:
        keycodes = get_terminal_keycodes(ti)
        if os.isatty(fd):
            backspace = termios.tcgetattr(fd)[6][termios.VERASE]
            keycodes[backspace] = 'backspace'

        super().__init__(encoding, keycodes)
