...
"""
Keymap contains functions for parsing keyspecs and turning keyspecs into appropriate sequences.

A keyspec is a string representing a sequence of key presses that can be bound to a command. All characters other than
the backslash represent themselves. In the traditional manner, a backslash introduces an escape sequence.

pyrepl uses its own keyspec format that is meant to be a strict superset of readline's KEYSEQ format. This means that if
a spec is found that readline accepts that this doesn't, it should be logged as a bug. Note that this means we're using
the '\\C-o' style of readline's keyspec, not the 'Control-o' sort.

The extension to readline is that the sequence \\<KEY> denotes the sequence of characters produced by hitting KEY.
...
'\\C-o', '\\c-o'  - control-o
'\\<tab>', '\\<TAB>', '\\t', '\\011', '\\x09', '\\X09', '\\C-i', '\\C-I' - all of these are the tab character.
"""
import typing as ta
    ...
    while not ret and s < len(key):
        if key[s] == '\\':
            ...
            if c in _ESCAPES:
                ...
                if key[s + 2] != '-':
                    raise KeySpecError(f"\\C must be followed by `-' (char {s + 2:d} of {key!r})")
                if ctrl:
                    raise KeySpecError(f'doubled \\C- (char {s + 1:d} of {key!r})')
                ctrl = 1
                s += 3

            elif c == 'm':
                if key[s + 2] != '-':
                    raise KeySpecError(f"\\M must be followed by `-' (char {s + 2:d} of {key!r})")
                if meta:
                    raise KeySpecError(f'doubled \\M- (char {s + 1:d} of {key!r})')
                meta = 1
                s += 3
                ...
                raise KeySpecError(f'unknown backslash escape {c!r} at char {s + 2:d} of {key!r}')
