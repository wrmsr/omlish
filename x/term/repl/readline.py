...
"""
A compatibility wrapper reimplementing the 'readline' standard module on top of pyrepl. Not all functionalities are
supported. Contains extensions for multiline input.
"""
...
class ReadlineAlikeReader(
    ...
    def __init__(
        ...
        self.assume_immutable_completions = False
        self.use_brackets = False
        self.sort_in_column = True

        # Instance fields
        self.more_lines: MoreLinesCallable | None = None
        self.last_used_indentation: str | None = None
    ...
    def after_command(self, cmd: Command) -> None:
        ...
        if self.more_lines is None:
            # Force single-line input if we are in raw_input() mode. Although there is no direct way to add a \n in this
            # mode, multiline buffers can still show up using various commands, e.g. navigating the history.
            try:
...
def set_auto_history(_should_auto_add_history: bool) -> None:
    ...
    HistoricalReader.should_auto_add_history = bool(_should_auto_add_history)
...
def _should_auto_indent(buffer: list[str], pos: int) -> bool:
    # check if last character before "pos" is a colon, ignoring whitespaces and comments.
    last_char = None
    while pos > 0:
        pos -= 1
        if last_char is None:
            ...
            # even if we found a non-whitespace character before original pos, we keep going back until newline is
            # reached to make sure we ignore comments
            if buffer[pos] == '\n':
                break
            if buffer[pos] == '#':
                last_char = None
...
class maybe_accept(Command):  # noqa
    def do(self) -> None:
...
class backspace_dedent(Command):  # noqa
    def do(self) -> None:
        r = self.reader
        b = r.buffer
...
class _ReadlineWrapper:
    f_in: int = -1
    f_out: int = -1
    reader: ReadlineAlikeReader | None = dc.field(default=None, repr=False)
    saved_history_length: int = -1
    startup_hook: ta.Callable[[], None] | None = None
    config: ReadlineConfig = dc.field(default_factory=ReadlineConfig, repr=False)
    raw_input: ta.Callable[[object], str] | None = None
    ...
    def input(self, prompt: object = '') -> str:
        ...
        except CONSOLE_ERROR_TYPES:
            return check.not_none(self.raw_input)(prompt)

        prompt_str = str(prompt)
        reader.prompts.ps1 = prompt_str
    ...
    def multiline_input(self, more_lines: MoreLinesCallable, ps1: str, ps2: str) -> str:
        """
        Read an input on possibly multiple lines, asking for more lines as long as 'more_lines(unicodetext)' returns an
        object whose boolean value is true.
        """
        ...
        try:
            ...
            reader.prompts.ps1 = ps1
            reader.prompts.ps2 = ps1
            reader.prompts.ps3 = ps2
            reader.prompts.ps4 = ''
    ...
    def set_completer_delims(self, delimiters: ta.Collection[str]) -> None:
    ...
    def read_history_file(self, filename: str = site.gethistoryfile()) -> None:
        # multiline extension (really a hack) for the end of lines that are actually continuations inside a single
        # multiline_input() history item: we use \r\n instead of just \n. If the history file is passed to GNU readline,
        # the extra \r are just ignored.
    ...
    def write_history_file(self, filename: str = site.gethistoryfile()) -> None:
    ...
    def append_history_file(self, filename: str = site.gethistoryfile()) -> None:
        reader = self.get_reader()
    ...
    def remove_history_item(self, index: int) -> None:
        ...
        if 0 <= index < len(history):
            ...
            raise ValueError(f'No history item at position {index:d}')
            # like readline.c
    ...
    def set_startup_hook(self, function: ta.Callable[[], None] | None = None) -> None:
