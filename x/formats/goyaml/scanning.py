...
class InvalidTokenError(Exception):
    ...
    def __str__(self) -> str:
        return check.not_none(self.token.error)
...
class Context:
    ...
    def get_multi_line_state(self) -> ta.Optional['MultiLineState']:
        return self.mstate
    ...
    def buffered_token(self, pos: tokens_.Position) -> tokens_.Token | None:
        ...
        if self.is_multi_line():
            ...
            tk = tokens.new(source, self.obuf, pos)
...
class Scanner:
    ...
    def is_changed_to_indent_state_down(self) -> bool:
        return self.indent_state == IndentState.DOWN
    ...
    def scan_single_quote(self, ctx: Context) -> ta.Tuple[ta.Optional[tokens.Token], ta.Optional[str]]:
        ...
        while True:
            ...
            self.progress_column(ctx, 1)
            return tokens.new_single_quote(value, ctx.obuf, srcpos), None
    ...
    def scan_double_quote(self, ctx: Context) -> ta.Tuple[ta.Optional[tokens.Token], ta.Optional[str]]:
        ...
        while True:
            ...
            self.progress_column(ctx, 1)
            return tokens.new_double_quote(value, ctx.obuf, srcpos), None
    ...
    def scan_tag(self, ctx: Context) -> ta.Tuple[bool, ta.Optional[str]]:
        ...
        for idx, c in enumerate(ctx.src[ctx.idx:]):
            ...
            if c == ' ':
                ...
                ctx.add_token(tokens.new_tag(value, ctx.obuf, self.pos()))
    ...
    def scan_comment(self, ctx: Context) -> bool:
        ...
        for idx, c in enumerate(ctx.src[ctx.idx:]):
            ...
            progress = len(value)
            ctx.add_token(tokens.new_comment(value, ctx.obuf, self.pos()))
            self.progress_column(ctx, progress)
            self.progress_line(ctx)
            ctx.clear()
            return True
    ...
    def scan_multi_line(self, ctx: Context, c: str) -> ta.Optional[str]:
        ...
        if ctx.is_eos():
            ...
            if (err := state.validate_indent_column()) is not None:
                invalid_tk = tokens.new_invalid(str(err), ctx.obuf, self.pos())
                self.progress_column(ctx, 1)
                return repr(err_invalid_token(invalid_tk))

            value = ctx.buffered_src()
            ctx.add_token(tokens.new_string(value, ctx.obuf, self.pos()))
            ctx.clear()
            self.progress_column(ctx, 1)
    ...
    def scan_flow_map_start(self, ctx: Context) -> bool:
        ...
        self.add_buffered_token_if_exists(ctx)
        ctx.add_origin_buf('{')
        ctx.add_token(tokens.new_mapping_start(ctx.obuf, self.pos()))
    ...
    def scan_flow_map_end(self, ctx: Context) -> bool:
        ...
        self.add_buffered_token_if_exists(ctx)
        ctx.add_origin_buf('}')
        ctx.add_token(tokens.new_mapping_end(ctx.obuf, self.pos()))
        self.started_flow_map_num -= 1
        self.progress_column(ctx, 1)
        ctx.clear()
        return True
    ...
    def scan_flow_array_start(self, ctx: Context) -> bool:
        ...
        self.add_buffered_token_if_exists(ctx)
        ctx.add_origin_buf('[')
        ctx.add_token(tokens.new_sequence_start(ctx.obuf, self.pos()))
    ...
    def scan_flow_array_end(self, ctx: Context) -> bool:
        ...
        self.add_buffered_token_if_exists(ctx)
        ctx.add_origin_buf(']')
        ctx.add_token(tokens.new_sequence_end(ctx.obuf, self.pos()))
        self.started_flow_sequence_num -= 1
        self.progress_column(ctx, 1)
        ctx.clear()
        return True
    ...
    def scan_flow_entry(self, ctx: Context, c: str) -> bool:
        ...
        self.add_buffered_token_if_exists(ctx)
        ctx.add_origin_buf(c)
        ctx.add_token(tokens.new_collect_entry(ctx.obuf, self.pos()))
        self.progress_column(ctx, 1)
        ctx.clear()
        return True
    ...
    def scan_sequence(self, ctx: Context) -> ta.Tuple[bool, ta.Optional[str]]:
        ...
        self.add_buffered_token_if_exists(ctx)
        ctx.add_origin_buf('-')
        tk = tokens.new_sequence_entry(ctx.obuf, self.pos())
        self.last_delim_column = tk.position.column
        ctx.add_token(tk)
        self.progress_column(ctx, 1)
        ctx.clear()
    ...
    def scan_multi_line_header_option(self, ctx: Context) -> ta.Optional[str]:
        ...
        if comment_index > 0:
            ...
            ctx.add_token(tokens.new_comment(comment, ctx.obuf[len(header_buf):], self.pos()))
    ...
    def scan_map_key(self, ctx: Context) -> bool:
        ...
        tk = tokens.new_mapping_key(self.pos())
        self.last_delim_column = tk.position.column
        ctx.add_token(tk)
        self.progress_column(ctx, 1)
        ctx.clear()
    ...
    def scan_directive(self, ctx: Context) -> bool:
        ...
        self.add_buffered_token_if_exists(ctx)
        ctx.add_origin_buf('%')
        ctx.add_token(tokens.new_directive(ctx.obuf, self.pos()))
        self.progress_column(ctx, 1)
        ctx.clear()
    ...
    def scan_anchor(self, ctx: Context) -> bool:
        ...
        self.add_buffered_token_if_exists(ctx)
        ctx.add_origin_buf('&')
        ctx.add_token(tokens.new_anchor(ctx.obuf, self.pos()))
        self.progress_column(ctx, 1)
        self.is_anchor = True
        ctx.clear()
        return True
    ...
    def scan_alias(self, ctx: Context) -> bool:
        ...
        self.add_buffered_token_if_exists(ctx)
        ctx.add_origin_buf('*')
        ctx.add_token(tokens.new_alias(ctx.obuf, self.pos()))
        self.progress_column(ctx, 1)
        self.is_alias = True
        ctx.clear()
        return True
