...
from .incompatibility_cause import PlatformCauseError
from .incompatibility_cause import PythonCauseError
from .incompatibility_cause import RootCauseError


if ta.TYPE_CHECKING:
    from .incompatibility_cause import IncompatibilityCauseError
...
class Incompatibility:
    def __init__(self, terms: list[Term], cause: 'IncompatibilityCauseError') -> None:
        ...
        if len(terms) != 1 and (
            # Short-circuit in the common case of a two-term incompatibility with two different packages (for example, a
            # dependency).
            len(terms) != 2
            ...
            for term in terms:
                ...
                if ref in by_ref:
                    ...
                    # If we have two terms that refer to the same package but have a null intersection, they're mutually
                    # exclusive, making this incompatibility irrelevant, since we already know that mutually exclusive
                    # version ranges are incompatible. We should never derive an irrelevant incompatibility.
                    err_msg = f"Package '{ref}' is listed as a dependency of itself."
                    value = check.not_none(value, err_msg)
                    by_ref[ref] = value
                else:
                    by_ref[ref] = term
    ...
    @property
    def external_incompatibilities(self) -> ta.Iterator[Incompatibility]:
    ...
    def __str__(self) -> str:
        if isinstance(self._cause, DependencyCauseError):
            ...
            check.state(len(self._terms) == 1)
            check.state(self._terms[0].is_positive())
            ...
            check.state(len(self._terms) == 1)
            check.state(not self._terms[0].is_positive())
            check.state(self._terms[0].dependency.is_root)
