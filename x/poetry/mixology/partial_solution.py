...
class PartialSolution:
    """
    A list of Assignments that represent the solver's current best guess about what's true for the eventual set of
    package versions that will comprise the total solution.
    ...
    def decide(self, package: Package) -> None:
        ...
        # When we make a new decision after backtracking, count an additional attempted solution. If we backtrack
        # multiple times in a row, though, we only want to count one, since we haven't actually started attempting a new
        # solution.
    ...
    def _register(self, assignment: Assignment) -> None:
        ...
        if old_positive is not None:
            value = check.not_none(old_positive.intersect(assignment))
            self._positive[name] = value

            return

        old_negative = self._negative.get(name)
        term = check.not_none(assignment if old_negative is None else assignment.intersect(old_negative))
    ...
    def satisfier(self, term: Term) -> Assignment:
        """
        Returns the first Assignment in this solution such that the sublist of assignments up to and including that
        ...
        for assignment in self._assignments:
            ...
            if (
                ...
                check.state(not term.is_positive())
            ...
            assigned_term = check.not_none(assigned_term)
