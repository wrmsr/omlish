...
from .incompatibility_cause import NoVersionsCauseError
from .incompatibility_cause import RootCauseError
...
class Preference(enum.IntEnum):
    """
    Preference is one of the criteria for choosing which dependency to solve first. A higher value means that there are
    "more options" to satisfy a dependency. A lower value takes precedence.
    """
...
class DependencyCache:
    ...
    The key observation here is that during the search - except at backtracking - once we have decided that a dependency
    is invalid, we never need check it again.
    """

    def __init__(self, provider: Provider) -> None:
        self._provider = provider

        # self._cache maps a package name to a stack of cached package lists, ordered by the decision level which added
        # them to the cache. This is done so that when backtracking we can maintain cache entries from previous decision
        # levels, while clearing cache entries from only the rolled back levels.
        #
    ...
    def _search_for(
        ...
        # provider.search_for() normally does not include pre-release packages (unless requested), but will include them
        # if there are no other eligible package versions for a version constraint.
        #
        # Therefore, if the eligible versions have been filtered down to nothing, we need to call provider.search_for()
        # again as it may return additional results this time.
        if not packages:
    ...
    def search_for(
        ...
        # We could always use dependency.without_features() here, but for performance reasons we only do it if
        # necessary.
        ...
        if dependency.features and packages:
            ...
            return PackageCollection(packages[0].dependency.with_features(dependency.features), packages)
...
class VersionSolver:
    """
    The version solver that finds a set of package versions that satisfy the root package's dependencies.

    See https://github.com/dart-lang/pub/tree/master/doc/solver.md for details on how this solver works.
    """
    ...
    def solve(self) -> SolverResult:
        """
        Finds a set of dependencies that match the root package's constraints, or raises an error if no such set is
        available.
        """
    ...
    def _propagate(self, package: str) -> None:
        """
        Performs unit propagation on incompatibilities transitively related to package to derive new assignments for
        _solution.
        """
        ...
        while changed:
            ...
            # Iterate in reverse because conflict resolution tends to produce more general incompatibilities as time
            # goes on. If we look at those first, we can derive stronger assignments sooner and more eagerly find
            # conflicts.
            for incompatibility in reversed(self._incompatibilities[package]):
                ...
                if result is _conflict:
                    # If the incompatibility is satisfied by the solution, we use _resolve_conflict() to determine the
                    # root cause of the conflict as a new incompatibility.
                    #
                    # It also backjumps to a point in the solution where that incompatibility will allow us to derive
                    # new assignments that avoid the conflict.
                    root_cause = self._resolve_conflict(incompatibility)

                    # Back jumping erases all the assignments we did at the previous decision level, so we clear
                    # [changed] and refill it with the newly-propagated assignment.
                    changed.clear()
                    result = check.not_none(self._propagate_incompatibility(root_cause))
                    check.not_equal(result, _conflict)
    ...
    def _propagate_incompatibility(self, incompatibility: Incompatibility) -> ta.Union[str, object, None]:
        ...
        If incompatibility is satisfied by _solution, returns _conflict. If incompatibility is almost satisfied by
        _solution, returns the unsatisfied term's package name.

        Otherwise, returns None.
        """

        # The first entry in incompatibility.terms that's not yet satisfied by _solution, if one exists. If we find more
        # than one, _solution is inconclusive for incompatibility and we can't deduce anything.
        unsatisfied = None
        ...
        for term in incompatibility.terms:
            ...
            if relation == SetRelation.DISJOINT:
                # If term is already contradicted by _solution, then incompatibility is contradicted as well and there's
                ...
                # If more than one term is inconclusive, we can't deduce anything about incompatibility.
                if unsatisfied is not None:
                    return None

                # If exactly one term in incompatibility is inconclusive, then it's almost satisfied and [term] is the
                # unsatisfied term. We can add the inverse of the term to _solution.
                unsatisfied = term

        # If *all* terms in incompatibility are satisfied by _solution, then incompatibility is satisfied and we have a
        # conflict.
    ...
    def _resolve_conflict(self, incompatibility: Incompatibility) -> Incompatibility:
        """
        Given an incompatibility that's satisfied by _solution, The `conflict resolution`_ constructs a new
        incompatibility that encapsulates the root cause of the conflict and backtracks _solution until the new
        ...
        while not incompatibility.is_failure():
            # The term in incompatibility.terms that was most recently satisfied by _solution.
            most_recent_term = None

            # The earliest assignment in _solution such that incompatibility is satisfied by _solution up to and
            # including this assignment.
            most_recent_satisfier = None

            # The difference between most_recent_satisfier and most_recent_term; that is, the versions that are allowed
            # by most_recent_satisfier and not by most_recent_term. This is None if most_recent_satisfier totally
            # satisfies most_recent_term.
            difference = None

            # The decision level of the earliest assignment in _solution *before* most_recent_satisfier such that
            # incompatibility is satisfied by _solution up to and including this assignment plus most_recent_satisfier.
            #
            # Decision level 1 is the level where the root package was selected. It's safe to go back to decision level
            # 0, but stopping at 1 tends to produce better error messages, because references to the root package end up
            ...
            for term in incompatibility.terms:
                ...
                if most_recent_term == term:
                    # If most_recent_satisfier doesn't satisfy most_recent_term on its own, then the next-most-recent
                    # satisfier may be the one that satisfies the remainder.
            ...
            # If most_recent_identifier is the only satisfier left at its decision level, or if it has no cause
            # (indicating that it's a decision rather than a derivation), then incompatibility is the root cause. We
            # then backjump to previous_satisfier_level, where incompatibility is guaranteed to allow _propagate to
            # produce more assignments.

            most_recent_satisfier = check.not_none(most_recent_satisfier)
            if (
                    previous_satisfier_level < most_recent_satisfier.decision_level or
                    most_recent_satisfier.cause is None
            ):
            ...
            # Create a new incompatibility by combining incompatibility with the incompatibility that caused
            # most_recent_satisfier to be assigned. Doing this iteratively constructs an incompatibility that's
            # guaranteed to be true (that is, we know for sure no solution will satisfy the incompatibility) while also
            # approximating the intuitive notion of the "root cause" of the conflict.
            ...
            # The most_recent_satisfier may not satisfy most_recent_term on its own if there are a collection of
            # constraints on most_recent_term that only satisfy it together. For example, if most_recent_term is `foo
            # ^1.0.0` and _solution contains `[foo >=1.0.0, foo <2.0.0]`, then most_recent_satisfier will be `foo
            # <2.0.0` even though it doesn't totally satisfy `foo ^1.0.0`.
            #
            # In this case, we add `not (most_recent_satisfier \ most_recent_term)` to the incompatibility as well, See
            # the `algorithm documentation`_ for details.
            #
    ...
    def _get_comp_key(self, dependency: Dependency) -> CompKey:
        ...
        - has_deps
        - num_packages
        that serves as priority for choosing the next package to resolve. (A lower value takes precedence.)
        ...
        A dependency with an upper bound is more likely to cause conflicts. Therefore, a package with more dependencies
        with upper bounds should be chosen first.

        ## has_deps

        A package with dependencies should be chosen first because a package without dependencies is less likely to
        cause conflicts.

        ## num_packages

        The original algorithm proposes to prefer packages with as few remaining versions as possible, so that if a
        conflict is necessary it's forced quickly.
        https://github.com/dart-lang/pub/blob/master/doc/solver.md#decision-making However, this leads to the famous
        boto3 vs. urllib3 issue, so we prefer packages with more remaining versions (see
        https://github.com/python-poetry/poetry/pull/8255#issuecomment-1657198242 for more details).
        """

        preference = Preference.DEFAULT

        # Direct origin dependencies must be handled first: we don't want to resolve a regular dependency for some
        # package only to find later that we had a direct-origin dependency.
        ...
        if packages:
            ...
            if package.is_root():
                ...
                if preference != Preference.LOCKED and not package.is_direct_origin():
                    # We have to get the package from the pool, otherwise `requires` will be empty.
                    #
                    # We might need `package.source_reference` as fallback for transitive dependencies without a source
                    # if there is a top-level dependency for the same package with an explicit source.
            ...
            num_deps_upper_bound = sum(1 for d in relevant_dependencies if d.constraint.has_upper_bound())
    ...
    def _choose_package_version(self) -> ta.Optional[str]:
        ...
        Returns the name of the package whose incompatibilities should be propagated by _propagate(), or None indicating
        that version solving is complete and a solution has been found.
        """
        ...
        if locked is None:
            ...
            if package is None:
                ...
                self._add_incompatibility(Incompatibility(
                    [Term(dependency, True)], NoVersionsCauseError(),
                ))
        ...
        for incompatibility in self._provider.incompatibilities_for(package):
            ...
            # If an incompatibility is already satisfied, then selecting version would cause a conflict.
            #
            # We'll continue adding its dependencies, then go back to unit propagation which will guide us to choose a
            # better version.
