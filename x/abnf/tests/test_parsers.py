...
from ..core import CharValNodeVisitor
from ..core import GrammarNodeVisitor
from ..core import GrammarRule
from ..core import NumValVisitor
...
from ..parsers import ParseCache
...
def test_next_longest():
    ...
    match1 = Match([], 1)
    match2 = Match([], 2)
    assert list(next_longest_match({match0, match1, match2})) == [
        match2,
        match1,
        match0,
    ]
...
def test_parse_cache_bad_max_size():
    with pytest.raises(ValueError):  # noqa
        ParseCache(-12)


def test_parse_cache_miss():
    cache = ParseCache()
...
def test_literal():
    ...
    match = next(matches)
    assert match == Match([ta.cast('Node', LiteralNode('a', 0, 1))], 1)


def test_literal_bad_arg():
    with pytest.raises(TypeError):
        Literal(('x', 1))  # type: ignore[arg-type]
...
def test_rule_first_match_alternation_pass():
    # this test just exercises first_match_alternation setter in the case of a rule which is not an alternation.
...
def test_exclusion():
    ...
    with pytest.raises(ParseError):
        # this will match 'fo', perhaps because of backtrackery. so we want a different test to trigger the parse error.
        identifier.parse_all('foo')
...
def test_bin_val():
    src = 'b01111000'
    node = GrammarRule('bin-val').parse_all(src)
...
def test_prose_val():
    ...
    src = '<blah blah>'
    node = GrammarRule('prose-val').parse_all(src)
    visitor = GrammarNodeVisitor(ProseRule)
    parser = visitor.visit(node)
...
def test_repetition_3():
    ...
    matches = list(result)
    assert matches == [
        Match(nodes=[ta.cast('Node', LiteralNode('*', 0, 1))], start=1),
        Match(nodes=[], start=0),
    ]


def test_repetition_cached_parseerror():
    src = 'a'
...
def test_empty_charval_node():
    ...
    node = GrammarRule('char-val').parse_all('""')
