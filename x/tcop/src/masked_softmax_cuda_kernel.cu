...
__global__ void __launch_bounds__(32) masked_softmax_cuda_forward_kernel(
  ...
  const unsigned int ibase =
      blockIdx.x * gridDim.y * gridDim.z * hidden_size +
      blockIdx.y * gridDim.z * hidden_size +
      blockIdx.z * hidden_size;

  const unsigned int mask_offset =
      blockIdx.x * (m0 > 1 ? m1 : 0) +
      blockIdx.z * (m1 > 1 ? 1 : 0);
  unsigned int mask_size = min(static_cast<unsigned int>(mask[mask_offset]), hidden_size);
...
std::vector<at::Tensor> masked_softmax_cuda_forward(
  ...
  TORCH_CHECK(mask.size(0) == 1 || mask.size(0) == input.size(0), "mask dim #0 has an incorrect shape");
  TORCH_CHECK(mask.size(1) == 1 || mask.size(1) == input.size(2), "mask dim #2 has an incorrect shape");
