...
class SpatialTransformer:
    def __init__(
        ...
        self.transformer_blocks = [
            BasicTransformerBlock(channels, ctx_dim[d], n_heads, d_head)
    ...
    def __call__(self, x: Tensor, ctx: Tensor | None = None) -> Tensor:
        ...
        ops = [
            ...
            (lambda z: self.proj_in(z)),
        ]
        x = x.sequential(ops if self.use_linear else ops[::-1])
        for block in self.transformer_blocks:
            x = block(x, ctx=ctx)
        ops = [
            (lambda z: self.proj_out(z)),
            (lambda z: z.permute(0, 2, 1).reshape(b, c, h, w)),
        ]
        x = x.sequential(ops if self.use_linear else ops[::-1])
        return x + x_in
...
class UNetModel:
    def __init__(
        ...
        self.middle_block: list = [
            ...
            SpatialTransformer(
                ch, n_heads, d_head, ctx_dim, use_linear, depth=transformer_depth[-1],
            ),
            ResBlock(ch, time_embed_dim, ch),
        ]
