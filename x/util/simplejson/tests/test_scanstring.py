...
class TestScanString(TestCase):
    ...
    def _test_scanstring(self, scanstring):
        if sys.maxunicode == 65535:
            self.assertEqual(
                scanstring('"z\U0001d120x"', 1, None, True), ('z\U0001d120x', 6),
            )
        else:
            self.assertEqual(
                scanstring('"z\U0001d120x"', 1, None, True), ('z\U0001d120x', 5),
            )
        ...
        self.assertEqual(
            scanstring('["Unclosed array"', 2, None, True), ('Unclosed array', 17),
        )

        self.assertEqual(
            scanstring('["extra comma",]', 2, None, True), ('extra comma', 14),
        )
        ...
        self.assertEqual(
            scanstring('["Extra close"]]', 2, None, True), ('Extra close', 14),
        )

        self.assertEqual(
            scanstring('{"Extra comma": true,}', 2, None, True), ('Extra comma', 14),
        )
        ...
        self.assertEqual(
            scanstring('{"Missing colon" null}', 2, None, True), ('Missing colon', 16),
        )

        self.assertEqual(
            scanstring('{"Double colon":: null}', 2, None, True), ('Double colon', 15),
        )
        ...
        self.assertEqual(
            scanstring('["Bad value", truth]', 2, None, True), ('Bad value', 12),
        )
    ...
    def test_issue3623(self):
        ...
        self.assertRaises(
            UnicodeDecodeError, json.encoder.encode_basestring_ascii, b('xx\xff'),
        )
