...
class Vt100Input(Input):
    ...
    def __init__(self, stdin: ta.TextIO) -> None:
        ...
        # into an application. For convenience, we print an error message and go on.
        isatty = stdin.isatty()
        fd = stdin.fileno()
        ...
        # Create a backup of the fileno(). We want this to work even if the underlying file is closed, so that
        # `typeahead_hash()` keeps working.
    ...
    def flush_keys(self) -> list[KeyPress]:
        ...
        # Flush all pending keys. (This is most important to flush the vt100 'Escape' key early when nothing else
        # follows.)
...
def _attached_input(
    ...
    def callback_wrapper() -> None:
        """
        Wrapper around the callback that already removes the reader when the input is closed. Otherwise, we keep
        continuously calling this callback, until we leave the context manager (which can happen a bit later). This
        fixes issues when piping /dev/null into a prompt_toolkit application.
        """
    ...
    try:
        ...
        # For `EPollSelector`, adding /dev/null to the event loop will raise `PermissionError` (that doesn't happen for
...
class raw_mode:
    ...
    #
    #    The result is that the eventloop will stop correctly, because it has to logic to quit when stdin is closed.
    #    However, we should not fail at this point. See:
    ...
    def __enter__(self) -> None:
        ...
        try:
            ...
            # VMIN defines the number of characters read at a time in non-canonical mode. It seems to default to 1 on
            # Linux, but on Solaris and derived operating systems it defaults to 4. (This is because the VMIN slot is
            # the same as the VEOF slot, which defaults to ASCII EOT = Ctrl-D = 4.)
    ...
    def _patch_iflag(cls, attrs: int) -> int:
        return attrs & ~(
            # Disable XON/XOFF flow control on output and input. (Don't capture Ctrl-S and Ctrl-Q.)
...
class cooked_mode(raw_mode):
    ...
    @classmethod
    def _patch_iflag(cls, attrs: int) -> int:
        # Turn the ICRNL flag back on. (Without this, calling `input()` in run_in_terminal doesn't work and displays ^M
        # instead. Ptpython evaluates commands using `run_in_terminal`, so it's important that they translate ^M back
        # into ^J.)
