...
ANSI_SEQUENCES: ta.Mapping[str, Keys | tuple[Keys, ...]] = {
    # Control keys.
    ...
    # ASCII Delete (0x7f)
    # Vt220 (and Linux terminal) send this when pressing backspace. We map this to ControlH, because that will make it
    # easier to create key bindings that work everywhere, with the trade-off that it's no longer possible to handle
    # backspace and control-h individually for the few terminals that support it. (Most terminals send ControlH when
    ...
    # These are currently unsupported, so just re-map some common ones to the unmodified versions
    ...
    # --
    # Sequences generated by numpad 5. Not sure what it means. (It doesn't appear in 'infocmp'. Just ignore.
    ...
    '\x1b[H': Keys.Home,
    '\x1b[F': Keys.End,
    # Tmux sends following keystrokes when control+arrow is pressed, but for Emacs ansi-term sends the same sequences
    # for normal arrow keys. Consider it a normal arrow press, because that's more important.
    ...
    #     (Iterm sends ESC followed by the normal arrow_up/down/left/right sequences, and the OSX Terminal sends ESCb
    #     and ESCf for "alt arrow_left" and "alt arrow_right." We don't handle these explicitly, in here, because would
    #     could not distinguish between pressing ESC (to go to Vi navigation mode), followed by just the 'b' or 'f' key.
    #     These combinations are handled in the input processor.)
