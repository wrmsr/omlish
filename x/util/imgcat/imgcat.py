...
if ta.TYPE_CHECKING:
    ...
    PIL = lang.proxy_import('PIL', extras=['Image'])


def get_image_shape(buf: bytes) -> tuple[int | None, int | None]:
    """
    ...
    def _unpack(fmt, buffer, mode='Image'):
        try:
            ...
            raise ValueError(f'Invalid {mode} file')
    ...
    if L >= 24 and buf.startswith(b'\211PNG\r\n\032\n') and buf[12:16] == b'IHDR':
        return _unpack('>LL', buf[16:24], mode='PNG')

    if L >= 16 and buf.startswith(b'\211PNG\r\n\032\n'):
...
def to_content_buf(data: ta.Any) -> bytes:
...
def imgcat(
    ...
    if height is None:
        ...
        if im_height:
            ...
            # automatically limit height to the current tty, otherwise the image will be just erased
            try:
...
def main():
    ...
    parser.add_argument('input', nargs='*', type=str, help='Path to the images.')
    parser.add_argument(
        '--height',
        default=None,
        type=int,
    ...
    parser.add_argument(
        '-v', '--version', action='version', version='python-imgcat %s' % __version__,
    )
    ...
    for fname in args.input:
        ...
        try:
            if fname.startswith('http://') or fname.startswith('https://'):
                with contextlib.closing(urllib.request.urlopen(fname)) as fp:
