...
libX11.XCreateSimpleWindow.argtypes = [Display, Window, ctypes.c_int, ctypes.c_int,
...
libX11.XGetWindowProperty.argtypes = [Display, Window, Atom, ctypes.c_long, ctypes.c_long,
...
def get_clipboard_text(display, window):
    ...
    libX11.XConvertSelection(display, clipboard, utf8_string, target_property, window, 0)
    ...
    status = libX11.XGetWindowProperty(
        display, window, target_property, 0, (~0), False,
        0, ctypes.byref(actual_type), ctypes.byref(actual_format),
...
def get_clipboard_image(display, window):
    ...
    libX11.XConvertSelection(display, clipboard, targets_atom, target_property, window, 0)
    libX11.XFlush(display)
    ...
    status = libX11.XGetWindowProperty(
        display, window, target_property, 0, (~0), False,
        4, ctypes.byref(actual_type), ctypes.byref(actual_format),
        ctypes.byref(nitems), ctypes.byref(bytes_after), ctypes.byref(data)
    )
...
def main():
    ...
    window = libX11.XCreateSimpleWindow(display, libX11.XRootWindow(display, screen), 0, 0, 1, 1, 0, 0, 0)
