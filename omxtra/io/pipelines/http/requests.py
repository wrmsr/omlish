...
class HttpRequestHeadDecoder(ChannelPipelineHandler):
    """
...
class HttpRequestBodyAggregator(ChannelPipelineHandler):
    """
    Aggregates an HTTP/1 request body using Content-Length.

    Input:
      - HttpRequestHead (from HttpRequestDecoder)
    ...
    def __init__(
        ...
        self._want = 0
        self._buf = SegmentedByteStreamBuffer(max_bytes=self._max_body)
...
@dc.dataclass(frozen=True)
class HttpRequestAborted:
    """Signals the peer disconnected before the request body completed."""

    reason: str = 'peer disconnected'


class HttpBodyStreamDecoder(ChannelPipelineHandler):
    """
    Turns (HttpRequestHead + subsequent bytes) into streaming HttpContentChunk events + HttpRequestEnd.
    ...
    This keeps the "simple non-streaming body" path possible via a separate aggregator handler:
      HttpRequestHead -> HttpContentChunk* -> HttpRequestEnd  (stream)
      HttpRequestHead -> HttpRequest(head, body)              (aggregated)
    """
    ...
    def __init__(
        ...
        self._head: HttpRequestHead | None = None
        self._mode: str | None = None  # 'none', 'cl', 'chunked', 'eof'
        self._remain = 0

        # chunked parsing state
        self._chunk_remain: int | None = None  # None -> need size line; 0 -> need trailers/end
    ...
    def inbound(self, ctx: ChannelPipelineHandlerContext, msg: ta.Any) -> None:
        if isinstance(msg, ChannelPipelineEvents.Eof):
            ...
            if self._mode == 'eof':
                ctx.feed_in(HttpRequestEnd())
            else:
                # Abort if we were expecting more bytes.
                if self._mode == 'cl' and self._remain != 0:
                    ctx.feed_in(HttpRequestAborted('EOF before Content-Length satisfied'))
                elif self._mode == 'chunked' and self._chunk_remain != 0:
                    ctx.feed_in(HttpRequestAborted('EOF before chunked body complete'))
                else:
    ...
    def _drain_content_length(self, ctx: ChannelPipelineHandlerContext) -> int:
        ...
        while self._remain and len(self._buf):
            ...
            ctx.feed_in(HttpContentChunk(data))
            self._remain -= take
    ...
    def _drain_until_eof(self, ctx: ChannelPipelineHandlerContext) -> int:
        ...
        while len(self._buf):
            ...
            if take > self._max_chunk:
                take = self._max_chunk
            v = self._buf.split_to(take)
            ctx.feed_in(HttpContentChunk(ByteStreamBuffers.to_bytes(v)))
        return 0
