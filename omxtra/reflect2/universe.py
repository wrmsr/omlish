...
DynamicTypeNameSuffix: ta.TypeAlias = str


##


DYNAMIC_TYPE_NAME_ID: ta.Final = 'id'
DYNAMIC_TYPE_NAME_COUNTER: ta.Final = 'counter'
...
class RuntimeTypeUniverse:
    def __init__(
        ...
        if dynamic_type_name_suffix not in (DYNAMIC_TYPE_NAME_ID, DYNAMIC_TYPE_NAME_COUNTER):
            raise ReflectionValueError(f'Unsupported dynamic type name suffix mode: {dynamic_type_name_suffix!r}')

        self.dynamic_type_name_suffix = dynamic_type_name_suffix
    ...
    def _make_dynamic_type_fullname(self, obj: type) -> str:
        ...
        if self.dynamic_type_name_suffix == DYNAMIC_TYPE_NAME_COUNTER:
