...
from ..encoder import CborEncoder
from ..encoder import cbor_dump
from ..encoder import cbor_dumps
from ..encoder import cbor_shareable_encoder
from ..types import CBOR_UNDEFINED
from ..types import CborEncodeError
...
def test_dump():
    with pytest.raises(TypeError):
        cbor_dump()  # type: ignore
    with pytest.raises(TypeError):
        cbor_dumps()  # type: ignore
    assert cbor_dumps(obj=1) == b'\x01'
    with io.BytesIO() as stream:
        cbor_dump(fp=stream, obj=1)
...
def test_integer(value, expected):
    ...
    assert cbor_dumps(value) == expected
...
def test_bytearray():
    expected = binascii.unhexlify('4401020304')
    assert cbor_dumps(bytearray(b'\x01\x02\x03\x04')) == expected
...
def test_simple_val_as_key():
    ...
    result = cbor_dumps(payload)
    assert result == binascii.unhexlify('A1F86301')
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (datetime.datetime(2013, 3, 21, 20, 4, 0, tzinfo=datetime.timezone.utc), True, 'c11a514b67b0'),
        (
            datetime.datetime(2013, 3, 21, 20, 4, 0, 123456, tzinfo=datetime.timezone.utc),
            True,
...
def test_datetime(value, as_timestamp, expected):
    expected = binascii.unhexlify(expected)
    assert cbor_dumps(value, datetime_as_timestamp=as_timestamp, timezone=datetime.timezone.utc) == expected
...
def test_date(value, as_timestamp, expected):
    expected = binascii.unhexlify(expected)
    assert cbor_dumps(value, datetime_as_timestamp=as_timestamp) == expected


def test_date_as_datetime():
    expected = binascii.unhexlify('c074323031332d30332d32315430303a30303a30305a')
    assert cbor_dumps(datetime.date(2013, 3, 21), timezone=datetime.timezone.utc, date_as_datetime=True) == expected
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (decimal.Decimal('NaN'), 'f97e00'),
        (decimal.Decimal('Infinity'), 'f97c00'),
        (decimal.Decimal('-Infinity'), 'f9fc00'),
    ],
...
def test_rational():
    expected = binascii.unhexlify('d81e820205')
    assert cbor_dumps(fractions.Fraction(2, 5)) == expected


def test_regex():
    expected = binascii.unhexlify('d8236d68656c6c6f2028776f726c6429')
    assert cbor_dumps(re.compile('hello (world)')) == expected
...
def test_mime():
    ...
    message = email.mime.text.MIMEText('Hello \u20acuro', 'plain', 'iso-8859-15')
    assert cbor_dumps(message) == expected


def test_uuid():
    expected = binascii.unhexlify('d825505eaffac8b51e480581277fdcc7842faf')
    assert cbor_dumps(uuid.UUID(hex='5eaffac8b51e480581277fdcc7842faf')) == expected
...
def test_custom_tag():
    expected = binascii.unhexlify('d917706548656c6c6f')
    assert cbor_dumps(CborTag(6000, 'Hello')) == expected


def test_cyclic_array():
    """Test that an array that contains itself can be serialized with value sharing enabled."""

    expected = binascii.unhexlify('d81c81d81c81d81d00')
    a: ta.Any = [[]]
    a[0].append(a)
    assert cbor_dumps(a, value_sharing=True) == expected
...
def test_cyclic_map():
    ...
    expected = binascii.unhexlify('d81ca100d81d00')
    a: ta.Any = {}
    a[0] = a
    assert cbor_dumps(a, value_sharing=True) == expected
...
def test_not_cyclic_same_object(value_sharing, expected):
    ...
    a: ta.Any = []
    b: ta.Any = [a, a]
    assert cbor_dumps(b, value_sharing=value_sharing) == expected
...
def test_default():
    ...
    expected = binascii.unhexlify('820305')
    obj = DummyType([3, 5])
    serialized = cbor_dumps(obj, default=default_encoder)
...
def test_default_cyclic():
    ...
    @cbor_shareable_encoder
    def default_encoder(encoder, value):
        state = encoder.encode_to_bytes(value.value)
        encoder.encode(CborTag(3000, state))

    expected = binascii.unhexlify('D81CD90BB849D81CD90BB843D81D00')
    obj = DummyType()
    obj2 = DummyType(obj)
    obj.value = obj2
    serialized = cbor_dumps(obj, value_sharing=True, default=default_encoder)
    assert serialized == expected
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (collections.OrderedDict([(b'b', b''), (b'a', b'')]), 'A2416140416240'),
        (collections.OrderedDict([('a', ''), ('b', '')]), 'a2616160616260'),
        (collections.OrderedDict([('b', ''), ('a', '')]), 'a2616160616260'),
        (collections.OrderedDict([(b'00001', ''), (b'002', '')]), 'A2433030326045303030303160'),
        (collections.OrderedDict([(255, 0), (2, 0)]), 'a2020018ff00'),
        (CborFrozenDict([(b'a', b''), (b'b', b'')]), 'A2416140416240'),
    ],
    ids=[
        'empty',
        'bytes in order',
...
def test_ordered_map(value, expected):
    expected = binascii.unhexlify(expected)
    assert cbor_dumps(value, canonical=True) == expected
...
def test_tuple_key():
    assert cbor_dumps({(2, 1): ''}) == binascii.unhexlify('a182020160')


def test_dict_key():
    assert cbor_dumps({CborFrozenDict({2: 1}): ''}) == binascii.unhexlify('a1a1020160')
...
def test_set(frozen):
    ...
    serialized = cbor_dumps(value)
    assert len(serialized) == 10
    assert serialized.startswith(binascii.unhexlify('d9010283'))


@pytest.mark.parametrize('frozen', [False, True], ids=['set', 'frozenset'])
def test_canonical_set(frozen):
    value: ta.Any = {'y', 'x', 'aa', 'a'}
    if frozen:
        value = frozenset(value)

    serialized = cbor_dumps(value, canonical=True)
    assert serialized == binascii.unhexlify('d9010284616161786179626161')
...
def test_encode_stringrefs_array():
    ...
    assert cbor_dumps(value, string_referencing=True) == b'\xd9\x01\x00' + cbor_dumps(equivalent)
...
def test_encode_stringrefs_dict():
    ...
    assert cbor_dumps(value, string_referencing=True, canonical=True) == expected


def test_encode_stringrefs_datetime():
    value = [datetime.datetime(2026, 1, 19, tzinfo=datetime.timezone.utc), 'abc', 'abc']
    expected = binascii.unhexlify('D9010083C074323032362D30312D31395430303A30303A30305A63616263D81901')
    assert cbor_dumps(value, string_referencing=True) == expected
...
def test_invalid_tag(tag):
    with pytest.raises(TypeError):
        cbor_dumps(CborTag(tag, 'value'))


def test_largest_tag():
    expected = binascii.unhexlify('dbffffffffffffffff6176')
    assert cbor_dumps(CborTag(2 ** 64 - 1, 'v')) == expected


def test_indefinite_containers():
    expected = b'\x82\x00\x01'
    assert cbor_dumps([0, 1]) == expected

    expected = b'\x9f\x00\x01\xff'
    assert cbor_dumps([0, 1], indefinite_containers=True) == expected
    assert cbor_dumps([0, 1], indefinite_containers=True, canonical=True) == expected

    expected = b'\xa0'
    assert cbor_dumps({}) == expected

    expected = b'\xbf\xff'
    assert cbor_dumps({}, indefinite_containers=True) == expected
    assert cbor_dumps({}, indefinite_containers=True, canonical=True) == expected


class TestEncoderReuse:
    """Tests for correct behavior when reusing CBOREncoder instances."""

    def test_encoder_reuse_resets_shared_containers(self):
        """
        ...
        result = cbor_loads(second_output)
        assert result == ['hello']

    def test_encode_to_bytes_resets_shared_containers(self):
        ...
        result = cbor_loads(result_bytes)
        assert result == ['hello']
    ...
    def test_encoder_hook_does_not_reset_state(self):
        ...
        data = cbor_dumps(Custom(['a', 'b']), default=custom_encoder)
        ...
        data2 = cbor_dumps(Custom(Custom(['x'])), default=custom_encoder)
        result2 = cbor_loads(data2)
        assert result2 == ['x']
