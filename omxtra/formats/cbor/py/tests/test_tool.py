...
def test_key_to_str(value, expected):
    assert tool.key_to_str(value) == expected


def test_default():
    with pytest.raises(TypeError):
        json.dumps(io.BytesIO(b''), cls=tool.DefaultEncoder)
...
def test_dtypes_from_file(monkeypatch, tmpdir):
    infile = pathlib.Path(__file__).with_name('examples.cbor.b64')
    expected = pathlib.Path(__file__).with_name('examples.json').read_text()
