...
Test suite for http_trailer_parser:
  ...
  - Empty trailers (just the terminating CRLF CRLF)
  - Forbidden trailer fields (all 17 from RFC 7230 §4.1.2)
  - Character validation (inherited from header parser)
  - Config knob pass-through (obs-fold, bare LF/CR, etc.)
  - Error cases (missing terminator, trailing data, NUL bytes)
  - Case-insensitive normalization and comma-combining
...
class TestBasicRequest(unittest.TestCase):
    ...
    def test_post_with_body_headers(self) -> None:
        ...
        msg = check.not_none(hp.parse_http_message(data))
...
class TestErrorModel(unittest.TestCase):
    ...
    def test_exception_is_catchable_as_exception(self) -> None:
        with self.assertRaises(Exception):  # noqa
            hp.parse_http_message(b'JUNK')

    def test_type_error_on_non_bytes(self) -> None:
        with self.assertRaises(TypeError):
            hp.parse_http_message('GET / HTTP/1.1\r\n\r\n')  # type: ignore
...
class TestDataModels(unittest.TestCase):
    ...
    def test_message_kind_enum_values(self) -> None:
        self.assertEqual(hp.ParsedHttpMessage.Kind.REQUEST.value, 'request')
...
def _trailers(fields: ta.List[ta.Tuple[str, str]]) -> bytes:
    """Build a well-formed trailer section from (name, value) pairs."""

    lines = [f'{n}: {v}\r\n'.encode('latin-1') for n, v in fields]
    return b''.join(lines) + b'\r\n'


class TestHappyPath(unittest.TestCase):
    def test_single_trailer(self) -> None:
        data = _trailers([('Expires', 'Thu, 01 Dec 1994 16:00:00 GMT')])
        result = hp.parse_http_trailers(data)
        self.assertEqual(result.headers['expires'], 'Thu, 01 Dec 1994 16:00:00 GMT')
        self.assertEqual(len(result.raw_headers), 1)
    ...
    def test_multiple_trailers(self) -> None:
        ...
        self.assertEqual(result.headers['x-checksum'], 'abc123')
        self.assertEqual(result.headers['x-signature'], 'def456')
        self.assertEqual(result.headers['x-digest'], 'sha256=xyz')

    def test_empty_trailers(self) -> None:
        """Just the terminating empty line - zero trailer fields."""

        data = b'\r\n'
        result = hp.parse_http_trailers(data)
        self.assertEqual(len(result.raw_headers), 0)
        self.assertEqual(len(result.headers), 0)

    def test_case_insensitive_lookup(self) -> None:
        data = _trailers([('X-Checksum', 'abc')])
        result = hp.parse_http_trailers(data)
        self.assertEqual(result.headers['x-checksum'], 'abc')
        self.assertEqual(result.headers['X-Checksum'], 'abc')
        self.assertEqual(result.headers['X-CHECKSUM'], 'abc')

    def test_comma_combining(self) -> None:
        """Duplicate trailer fields should be comma-combined."""

        data = _trailers([('X-Tag', 'a'), ('X-Tag', 'b')])
        result = hp.parse_http_trailers(data)
        self.assertEqual(result.headers['x-tag'], 'a, b')

    def test_ows_stripping(self) -> None:
        data = b'X-Value:   padded   \r\n\r\n'
        result = hp.parse_http_trailers(data)
        self.assertEqual(result.headers['x-value'], 'padded')

    def test_bytearray_accepted(self) -> None:
        data = bytearray(_trailers([('X-Check', 'ok')]))
        result = hp.parse_http_trailers(data)
        self.assertEqual(result.headers['x-check'], 'ok')

    def test_latin1_value(self) -> None:
        """obs-text bytes (0x80-0xFF) in values are allowed by default."""

        data = b'X-Val: caf\xe9\r\n\r\n'
        result = hp.parse_http_trailers(data)
        self.assertEqual(result.headers['x-val'], 'caf\xe9')

    def test_parsed_trailers_is_frozen(self) -> None:
        data = _trailers([('X-Check', 'ok')])
        result = hp.parse_http_trailers(data)
        with self.assertRaises(AttributeError):
            result.headers = None  # type: ignore  # noqa


class TestForbiddenFields(unittest.TestCase):
    """Every field in _FORBIDDEN_TRAILER_FIELDS must be rejected."""

    FORBIDDEN: ta.ClassVar[ta.Sequence[str]] = [
        'Transfer-Encoding',
        'Content-Length',
        'Host',
        'Cache-Control',
    ...
    def test_all_forbidden_fields_rejected(self) -> None:
    ...
    def test_allowed_field_alongside_check(self) -> None:
        """Non-forbidden fields should parse fine."""

        data = _trailers([('X-Custom', 'ok'), ('Expires', 'never')])
        result = hp.parse_http_trailers(data)
        self.assertEqual(result.headers['x-custom'], 'ok')
        self.assertEqual(result.headers['expires'], 'never')


class TestCharacterValidation(unittest.TestCase):
    """Inherited character validation from the header parser."""
...
class TestConfigPassThrough(unittest.TestCase):
    """Parser config knobs should apply to trailer parsing."""
    ...
    def test_empty_value_allowed_by_default(self) -> None:
        data = b'X-Empty:\r\n\r\n'
        result = hp.parse_http_trailers(data)
        self.assertEqual(result.headers['x-empty'], '')
...
class TestErrorCases(unittest.TestCase):
    ...
    def test_type_error_on_string(self) -> None:
        with self.assertRaises(TypeError):
            hp.parse_http_trailers('X-Name: val\r\n\r\n')  # type: ignore

    def test_type_error_on_int(self) -> None:
        with self.assertRaises(TypeError):
            hp.parse_http_trailers(42)  # type: ignore

    def test_all_errors_are_http_parse_error(self) -> None:
        """All raised errors should be subclasses of HttpParseError."""
    ...
    def test_forbidden_field_after_valid_field(self) -> None:
...
class TestSetCookieHandling(unittest.TestCase):
    def test_set_cookie_not_combined(self) -> None:
        """Set-Cookie is not forbidden in trailers and should not be comma-combined."""

        data = _trailers([('Set-Cookie', 'a=1'), ('Set-Cookie', 'b=2')])
        result = hp.parse_http_trailers(data)
        values = result.headers.get_all('set-cookie')
        self.assertEqual(values, ['a=1', 'b=2'])


class TestRawHeaders(unittest.TestCase):
    def test_raw_headers_preserved(self) -> None:
        data = _trailers([('X-Mixed-Case', 'Value')])
        result = hp.parse_http_trailers(data)
        self.assertEqual(result.raw_headers[0].name, b'X-Mixed-Case')
        self.assertEqual(result.raw_headers[0].value, b'Value')

    def test_raw_header_count_matches(self) -> None:
