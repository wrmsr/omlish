...
#         MirrorImpl(parent=unfrozen)
#
#
# def test_freeze_is_idempotent() -> None:
#     mirror = MirrorImpl()
#     assert not mirror.is_frozen
#
#     mirror.freeze()
...
#     root = make_root_mirror()
#     left = MirrorImpl(parent=root)
#     right = MirrorImpl(parent=root)
#
...
#     child = MirrorImpl(parent=root)
#
#     parent_infos = dict(root._internal._infos_by_fullname)
#     parent_cache = dict(root._internal._type_cache)
#
#     class Local:
#         pass
#
...
#     assert isinstance(typ, types.Instance)
#
#     assert root._internal._infos_by_fullname == parent_infos
#     assert root._internal._type_cache == parent_cache
#     assert typ.type is child._internal.get_info(typ.type.fullname)
#
#
...
#     mirror.freeze()
#
#     assert [type_var.name for type_var in info.type_vars] == ['T']
#     assert mirror._internal.is_info_prepared(Box)
#
#     # A child can subscript it without preparing (mutating) the frozen parent's info.
#     child = MirrorImpl(parent=mirror)
