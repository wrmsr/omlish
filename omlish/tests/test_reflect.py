...
def test_reflect_type():
    ...
    assert rfl.reflect(ta.Union[int, float]) == rfl.Union(frozenset([int, float]))
    assert rfl.reflect(ta.Optional[int]) == rfl.Union(frozenset([int, type(None)]))
    ...
    assert rfl.reflect(ta.Mapping[int, ta.Optional[str]]) == rfl.Generic(collections.abc.Mapping, (int, rfl.Union(frozenset([str, type(None)]))))  # noqa
    assert rfl.reflect(ta.Mapping[int, ta.Sequence[ta.Optional[str]]]) == rfl.Generic(collections.abc.Mapping, (int, rfl.Generic(collections.abc.Sequence, (rfl.Union(frozenset([str, type(None)])),))))  # noqa
