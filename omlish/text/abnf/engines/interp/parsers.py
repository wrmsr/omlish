...
class _Parser(lang.Abstract):
    ...
    @property
    def furthest(self) -> _ParserImpl._Furthest | None:
        return None
...
def _furthest_rule_trace(f: _ParserImpl._Furthest) -> tuple[str, ...]:
    """Rule names enclosing the furthest match, outermost first."""

    names: list[str] = []
    ctx: _ParserContext | None = f.ctx
    ...
    def _iter_parse_repeat(self, ctx: _ParserContext) -> ta.Iterator[Match]:
        ...
        while True:
            ...
            if self._max_steps is not None and self._cur_step > self._max_steps:
                raise AbnfMaxStepsExceededError(self._cur_step)
            self._cur_step += 1
