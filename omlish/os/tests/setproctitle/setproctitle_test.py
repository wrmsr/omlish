...
The tests are executed in external processes: setproctitle should never be imported directly from here.
...
from .conftest import skip_if_no_proc_env
...
def test_unicode():
    ...
    try:
        ...
        pytest.skip(f"default encoding '{sys.getdefaultencoding()}' can't deal with snowmen")

    try:
        snowman.encode(sys.getfilesystemencoding())
    except UnicodeEncodeError:
        pytest.skip(f"file system encoding '{sys.getfilesystemencoding()}' can't deal with snowmen")
...
""",
    ...
    for snowman in snowmen:
        ...
        pytest.fail(f'unexpected ps output: {title!r}')
...
def test_weird_path(tmp_path):
    """No problem with encoded argv[0] path."""

    _check_4388()
    euro = '\u20ac'
    snowman = '\u2603'
    test_dir = tmp_path / euro / snowman
    try:
        os.makedirs(test_dir)
    except UnicodeEncodeError:
        pytest.skip("file system doesn't support unicode")

    exc = test_dir / os.path.basename(sys.executable)
