...
class Runtime(lang.Abstract, ta.Generic[T]):
    ...
    # @abc.abstractmethod
    # def compare(self, op: ComparatorName, left: T, right: T) -> T:
    #     raise NotImplementedError
...
class RuntimeImpl(Runtime[ta.Any]):
    ...
    @ta.override
    def create_object(self, fields: ta.Mapping[str, ta.Any]) -> dict[str, ta.Any]:
        return dict(fields)
    ...
    # def compare(self, op: ComparatorName, left: ta.Any, right: ta.Any) -> int | None:
    #     lty = self.get_type(left)
    #     rty = self.get_type(right)
    #     if lty != rty:
    #         return -1
    #     elif lty == ValueType.NULL:
    #         return 0
