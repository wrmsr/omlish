...
class LexerUtils(unittest.TestCase):
    ...
    def assert_tokens(self, actual, expected):
        ...
        # use assertEqual for the tests that check those line numbers.
        stripped = []
        for item in actual:
            stripped.append({'type': item['type'], 'value': item['value']})
        # Every tokenization should end in eof, so we automatically check that value, strip it off the end, and then
        # verify the remaining tokens against the expected. That way the tests don't need to add eof to every
        # assert_tokens call.
...
class TestRegexLexer(LexerUtils):
    ...
    def test_field(self):
        ...
        self.assert_tokens(tokens, [{'type': 'unquoted_identifier', 'value': 'foo'}])
    ...
    def test_quoted_identifier(self):
        ...
        self.assert_tokens(tokens, [{'type': 'quoted_identifier', 'value': 'foobar'}])

    def test_json_escaped_value(self):
        tokens = list(self.lexer.tokenize('"\u2713"'))
        self.assert_tokens(tokens, [{'type': 'quoted_identifier', 'value': '\u2713'}])
