...
class Container(ta.Protocol[T_contra]):
    # Per mypy: "This is generic more on vibes than anything else"
    def __contains__(self, x: T_contra) -> bool: ...


class Collection(Sized, Iterable[T_co], Container[ta.Any], ta.Protocol[T_co]):
    # Per mypy: "Note: need to use Container[Any] instead of Container[_T_co] to ensure covariance."
...
class Sequence(Reversible[T_co], Collection[T_co], ta.Protocol[T_co]):
    @ta.overload
