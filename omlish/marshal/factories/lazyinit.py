"""
TODO:
 - (re-)migrate too InternalState?
  - this is the last thing that uses a nasty hidden config entry, but lazyinit still kinda keys on that - its callbacks
    take the ConfigRegistry and update it directly.
"""
import typing as ta
...
class _LazyInitRunningFactory(_PreReflectFactory, ta.Generic[FactoryT]):
    ...
    def _run_if_necessary(self, ctx: BaseContext) -> None:
        cfgs = check.isinstance(ctx.configs, ConfigRegistry)
