...
class ShiftReduceField(data.Field):
    def __init__(self):
        super().__init__(
            preprocessing=lambda parse: ['reduce' if t == ')' else 'shift' for t in parse if t != '('],
        )
...
class ParsedTextField(data.Field):
    """
    Field for parsed sentences data in NLI datasets. Expensive tokenization could be omitted from the pipeline as the
    parse tree annotations are already in tokenized form.
    """
...
class NLIDataset(data.TabularDataset):
    ...
    def iters(
        ...
        Arguments:
            ...
            vectors: one of the available pretrained vectors or a list with each element one of the available pretrained
                vectors (see Vocab.load_vectors)
