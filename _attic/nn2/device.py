...
class _Device:
    ...
    def __getitem__(self, x: str) -> ta.Union[Interpreted, Compiled]:
        ...
        return [
            cls
            for cname, cls in inspect.getmembers(importlib.import_module(f'tinygrad.runtime.ops_{x.lower()}'))
            if (cname.lower() == x.lower() + "buffer")
            and x in self._buffers
        ][0]

    @functools.cached_property
    def DEFAULT(self) -> str:
        device_from_env: ta.Optional[str] = functools.reduce(  # type: ignore
            lambda val, ele: ele if getenv(ele) == 1 else val,
            self._buffers,
            None,
        )
        if device_from_env:
...
class JITRunner:
    ...
    def exec(self, rawbufs: list[RawBuffer], var_vals: ta.Optional[dict[Variable, int]] = None) -> ta.Optional[float]:
    ...
    def __call__(self, rawbufs: list[RawBuffer], var_vals: dict[Variable, int], wait=False, jit=False) -> ta.Optional[float]:
...
class InterpretedAstRunner(JITRunner):
    def __init__(self, ast: ops.LazyOp, fxn: ta.Callable):
        super().__init__()
        self.fxn = fxn
...
class Interpreted:
    def __init__(self, buffer: type[RawBuffer], fxn_for_op: dict[type[ops.LazyOp], ta.Callable]):
        self.buffer = buffer
    ...
    def get_runner(self, ast: ops.LazyOp) -> InterpretedAstRunner:
        return _get_interpreted_fxn(self.fxn_for_op, ast)


def _get_interpreted_fxn(fxn_for_op: dict[type[ops.LazyOp], ta.Callable], ast: ops.LazyOp) -> InterpretedAstRunner:
    if DEBUG >= 3:
    ...
    def _interpret_ast(ast: ops.LazyOp) -> str:
        if ops.MulAcc in fxn_for_op and ast.op == ops.Sum and isinstance(ast.src[0], ops.LazyOp) and isinstance(ast.src[0], ops.Mul):
            ast = ops.LazyOp(ops.MulAcc, ast.src[0].src, ast.arg)

        if isinstance(ast, ops.BufferOps):
...
class Compiled:
    ...
    @functools.lru_cache(None)  # pylint: disable=method-cache-max-size-none
    def get_runner(self, ast: ops.LazyOp) -> CompiledAstRunner:
        return self.to_program(_get_optimized_linearizer(self.linearizer_opts, ast))


def _get_optimized_linearizer(linearizer_opts: LinearizerOptions, ast: ops.LazyOp) -> Linearizer:
    if DEBUG >= 3:
        from .lazy import print_tree
        print_tree(ast)
    ...
    if not NOOPT:
        ...
        if BEAM >= 1:
            ...
            lins.append(
                (f"beam{BEAM.value}", beam_search(kb, test_rawbuffers, BEAM.value, bool(getenv("BEAM_ESTIMATE", 1)))))
            if used_tensor_cores:
                lins.append(("hc", Linearizer(ast, linearizer_opts)))
                lins[-1][1].hand_coded_optimizations()
            timed = sorted(
                [(nm, tk, time_linearizer(tk, test_rawbuffers, allow_test_size=False, clear_l2=True)) for nm, tk in
                 lins], key=lambda x: x[2])
            if DEBUG >= 1: print("  <  ".join(
                f"{nm:6s} : {lin.colored_shape(30, dense=True)} : {tm * 1e6:8.2f} us" for nm, lin, tm in timed))
            k = timed[0][1]
    else:
