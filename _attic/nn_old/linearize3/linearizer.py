...
class Linearizer(OptimizedKernel):
    def get_buffer_name(self, i):
        if self.bufs[i].__class__ == LocalBuffer:
            return self.bufs[i].name
        assert (
            self.bufs[i].realized.__class__ is not RawConst
        )  # constants shouldn't be loaded with memops
    ...
    def const(self, b: Union[int, float], dtype=dtypes.int32) -> UOp:
        return self.uop(UOps.CONST, dtype, tuple(), b, cachable=True)
            ...
            if key not in self.load_cache:
                if acc is not None:
                    ...
                    self.load_cache[key] = self.uop(
                        UOps.DEFINE_ACC, localtype, (), this_const
                    )
    ...
    def linearize(self):
        ...
        for i, b in enumerate(self.bufs):
            if b.realized in arg_bufs:
                self.buf_uops[i] = arg_bufs[b.realized]
        # add variables from symbolic shapes
        for var in sorted(
            set(v for buf in self.ast.buffers for v in buf.var_vals),
            key=lambda k: k.key,
        ):
    ...
    def ast_parse(self, x, acc, loaded_buffers, do_reduce=False) -> List[UOp]:
        ...
        if x.op in ops:
            ret = [
                (
                    ...
                    self.uop(
                        UOps.STORE,
                        dtypes.float32,
                        (val[-1], self.uop(UOps.ALU, dtypes.float32, val, ops[x.op])),
                    ),
                )
                for idx, val in zip(
            ...
            ret = [
                (idx, self.uop(UOps.ALU, dtypes.float32, val, x.op, cachable=True))
