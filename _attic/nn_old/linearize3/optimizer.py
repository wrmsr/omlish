...
class OptimizedKernel(Kernel):
    ...
    def simplify_merge_adjacent(self):
        ...
        for i, x in enumerate(rets):
            self.sts[i].reshape(tuple([y[0] for y in x]))
    ...
    def alias_buffer(self, i, pattern):
        ...
        self.sts.append(
            ShapeTracker(tuple(shp), [View.create(tuple(shp), tuple(stride))])
        )
    ...
    def hand_coded_optimizations(self):
        ...
        if (
            ...
            and isinstance(self.reduceop.src[0].src[0].src[0], LazyBuffer)
            and isinstance(self.reduceop.src[0].src[0].src[1], LazyBuffer)
            and self.opts.has_local
            and self.reduceop.src[0].src[0].src[0].dtype == dtypes.half
            and self.reduceop.src[0].src[0].src[1].dtype == dtypes.half
        ):
        ...
        tensor_cores_allowed = getenv("TC", 1) != 0 and (
            getenv("TC", 1) == 2
            or (self.bufs[0].device == "METAL" and os.uname().machine == "arm64")
        )
        if (
            ...
            and isinstance(self.reduceop.src[0].src[0], LazyBuffer)
            and isinstance(self.reduceop.src[0].src[1], LazyBuffer)
            and self.opts.has_local
        ):
        ...
        if self.opts.has_local and all(
            isinstance(s, int) for s in self.sts[0].shape[: self.first_reduce]
        ):
