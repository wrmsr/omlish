...
def fix_schedule_for_images(
    schedule: list[tuple[LazyOp, LazyBuffer, tuple[LazyBuffer, ...]]]
):
    # this is the fundamental fix, find unwritable or unreadable images and convert them to normal float32
    # (TODO: should it be float16?)
    for op, out, buffers in schedule:
        if isinstance(out.dtype, ImageDType) and (
        ...
        for b in bops:
            if isinstance(buffers[b.arg.idx - 1].dtype, ImageDType) and (
    ...
    for op, out, buffers in schedule:
        ...
        for x in bops:
            if x.arg.dtype != buffers[x.arg.idx - 1].dtype:
                replacements[x] = ops.Mem(
                    (),
                    MemBuffer(x.arg.idx, buffers[x.arg.idx - 1].dtype, x.arg.st),
                )
...
def _realize_from(buffer: LazyBuffer, src: LazyBuffer) -> None:
    assert (
        src.realized.size == buffer.st.size()
    ), f"size mismatch on FROM {src.realized.size} != {buffer.st.size()}"
    assert src.st.contiguous and buffer.st.contiguous, "all must be contiguous for from"
    if DEBUG >= 3:
        print(
            f"*** copy {buffer.device} <- {src.device} size {src.realized.size} dtype {src.realized.dtype}"
        )

    # TODO: make this generic
    if isinstance(src.realized, RawDiskBuffer) and issubclass(
        Device[buffer.device].buffer, RawBufferMapped
    ):
        assert all_int(buffer.shape), "does not support symbolic shape"
        buffer.realized = Device[buffer.device].buffer(
        ...
        buffer.realized = ta.cast(
            ...
            src.realized, buffer.shape, buffer.dtype, **buffer._device_extra_args()
        )
    else:
        # TODO: schedule this as FROM to go to CPU, and a FROM to go to device
        buffer.realized = Device[buffer.device].buffer.fromCpu(
            src.realized.toCpu(), **buffer._device_extra_args()
        )
