# @omlish-generated
# type: ignore
# ruff: noqa
# flake8: noqa
import dataclasses
import reprlib
import types


##


REGISTRY = {}


def _register(**kwargs):
    def inner(fn):
        REGISTRY[kwargs['plan_repr']] = (kwargs, fn)
        return fn
    return inner


##


@_register(
    plan_repr=(
        "Plans(tup=(CopyPlan(fields=()), EqPlan(fields=()), FrozenPlan(fields=(), allow_dynamic_dunder_attrs=False), Ha"
        "shPlan(action='add', fields=(), cache=False), InitPlan(fields=(), self_param='self', std_params=(), kw_only_pa"
        "rams=(), frozen=True, slots=False, post_init_params=None, init_fns=(), validate_fns=()), ReprPlan(fields=(), i"
        "d=False, terse=False, default_fn=None)))"
    ),
    plan_repr_sha1='e1f7edfe11f2b721d6a656c46e698fedc95461bb',
    op_ref_idents=(),
    cls_names=(
        ('ommlds.minichain.modules.bash.configs', 'BashConfig'),
        ('ommlds.minichain.modules.code.configs', 'CodeConfig'),
        ('ommlds.minichain.modules.configs', 'ModuleConfig'),
        ('ommlds.minichain.modules.todo.configs', 'TodoConfig'),
        ('ommlds.minichain.modules.weathertest.configs', 'WeatherTestConfig'),
    ),
)
def _process_dataclass__e1f7edfe11f2b721d6a656c46e698fedc95461bb():
    def _process_dataclass(
        *,
        __class__,
        __dataclass__FieldFnValidationError,  # noqa
        __dataclass__FieldTypeValidationError,  # noqa
        __dataclass__FnValidationError,  # noqa
        __dataclass__FrozenInstanceError=dataclasses.FrozenInstanceError,  # noqa
        __dataclass__FunctionType=types.FunctionType,  # noqa
        __dataclass__HAS_DEFAULT_FACTORY=dataclasses._HAS_DEFAULT_FACTORY,  # noqa
        __dataclass__MISSING=dataclasses.MISSING,  # noqa
        __dataclass__None=None,  # noqa
        __dataclass__TypeError=TypeError,  # noqa
        __dataclass___recursive_repr=reprlib.recursive_repr,  # noqa
        __dataclass__isinstance=isinstance,  # noqa
        __dataclass__object_setattr=object.__setattr__,  # noqa
        __dataclass__property=property,  # noqa
    ):
        def __copy__(self):
            if self.__class__ is not __class__:
                raise TypeError(self)
            return __class__()  # noqa

        __copy__.__qualname__ = f"{__class__.__qualname__}.__copy__"
        if '__copy__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __copy__ in class {__class__.__name__}")
        setattr(__class__, '__copy__', __copy__)

        def __eq__(self, other):
            if self is other:
                return True
            if self.__class__ is not other.__class__:
                return NotImplemented
            return True

        __eq__.__qualname__ = f"{__class__.__qualname__}.__eq__"
        if '__eq__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __eq__ in class {__class__.__name__}")
        setattr(__class__, '__eq__', __eq__)

        def __setattr__(self, name, value):
            if (
                type(self) is __class__
            ):
                raise __dataclass__FrozenInstanceError(f"cannot assign to field {name!r}")
            super(__class__, self).__setattr__(name, value)

        __setattr__.__qualname__ = f"{__class__.__qualname__}.__setattr__"
        if '__setattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __setattr__ in class {__class__.__name__}")
        setattr(__class__, '__setattr__', __setattr__)

        def __delattr__(self, name):
            if (
                type(self) is __class__
            ):
                raise __dataclass__FrozenInstanceError(f"cannot delete field {name!r}")
            super(__class__, self).__delattr__(name)

        __delattr__.__qualname__ = f"{__class__.__qualname__}.__delattr__"
        if '__delattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __delattr__ in class {__class__.__name__}")
        setattr(__class__, '__delattr__', __delattr__)

        def __hash__(self):
            return hash(())

        __hash__.__qualname__ = f"{__class__.__qualname__}.__hash__"
        setattr(__class__, '__hash__', __hash__)

        def __init__(
            self,
        ) -> __dataclass__None:
            pass

        __init__.__qualname__ = f"{__class__.__qualname__}.__init__"
        if '__init__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __init__ in class {__class__.__name__}")
        setattr(__class__, '__init__', __init__)

        @__dataclass___recursive_repr()
        def __repr__(self):
            parts = []
            return (
                f"{self.__class__.__qualname__}("
                f"{', '.join(parts)}"
                f")"
            )

        __repr__.__qualname__ = f"{__class__.__qualname__}.__repr__"
        if '__repr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __repr__ in class {__class__.__name__}")
        setattr(__class__, '__repr__', __repr__)

    return _process_dataclass


@_register(
    plan_repr=(
        "Plans(tup=(CopyPlan(fields=('name', 'config_cls', 'binder')), EqPlan(fields=('name', 'config_cls', 'binder')),"
        " FrozenPlan(fields=('name', 'config_cls', 'binder'), allow_dynamic_dunder_attrs=False), HashPlan(action='add',"
        " fields=('name', 'config_cls', 'binder'), cache=False), InitPlan(fields=(InitPlan.Field(name='name', annotatio"
        "n=OpRef(name='init.fields.0.annotation'), default=None, default_factory=None, init=True, override=False, field"
        "_type=FieldType.INSTANCE, coerce=None, validate=None, check_type=None), InitPlan.Field(name='config_cls', anno"
        "tation=OpRef(name='init.fields.1.annotation'), default=None, default_factory=None, init=True, override=False, "
        "field_type=FieldType.INSTANCE, coerce=None, validate=None, check_type=None), InitPlan.Field(name='binder', ann"
        "otation=OpRef(name='init.fields.2.annotation'), default=None, default_factory=None, init=True, override=False,"
        " field_type=FieldType.INSTANCE, coerce=None, validate=None, check_type=None)), self_param='self', std_params=("
        "), kw_only_params=('name', 'config_cls', 'binder'), frozen=True, slots=False, post_init_params=None, init_fns="
        "(), validate_fns=()), ReprPlan(fields=(ReprPlan.Field(name='name', kw_only=True, fn=None), ReprPlan.Field(name"
        "='config_cls', kw_only=True, fn=None), ReprPlan.Field(name='binder', kw_only=True, fn=None)), id=False, terse="
        "False, default_fn=None)))"
    ),
    plan_repr_sha1='22480a2a2aaaf96b870b1b90c6449fd0932820d8',
    op_ref_idents=(
        '__dataclass__init__fields__0__annotation',
        '__dataclass__init__fields__1__annotation',
        '__dataclass__init__fields__2__annotation',
    ),
    cls_names=(
        ('ommlds.minichain.modules.descriptors', 'ModuleDescriptor'),
    ),
)
def _process_dataclass__22480a2a2aaaf96b870b1b90c6449fd0932820d8():
    def _process_dataclass(
        *,
        __class__,
        __dataclass__init__fields__0__annotation,
        __dataclass__init__fields__1__annotation,
        __dataclass__init__fields__2__annotation,
        __dataclass__FieldFnValidationError,  # noqa
        __dataclass__FieldTypeValidationError,  # noqa
        __dataclass__FnValidationError,  # noqa
        __dataclass__FrozenInstanceError=dataclasses.FrozenInstanceError,  # noqa
        __dataclass__FunctionType=types.FunctionType,  # noqa
        __dataclass__HAS_DEFAULT_FACTORY=dataclasses._HAS_DEFAULT_FACTORY,  # noqa
        __dataclass__MISSING=dataclasses.MISSING,  # noqa
        __dataclass__None=None,  # noqa
        __dataclass__TypeError=TypeError,  # noqa
        __dataclass___recursive_repr=reprlib.recursive_repr,  # noqa
        __dataclass__isinstance=isinstance,  # noqa
        __dataclass__object_setattr=object.__setattr__,  # noqa
        __dataclass__property=property,  # noqa
    ):
        def __copy__(self):
            if self.__class__ is not __class__:
                raise TypeError(self)
            return __class__(  # noqa
                name=self.name,
                config_cls=self.config_cls,
                binder=self.binder,
            )

        __copy__.__qualname__ = f"{__class__.__qualname__}.__copy__"
        if '__copy__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __copy__ in class {__class__.__name__}")
        setattr(__class__, '__copy__', __copy__)

        def __eq__(self, other):
            if self is other:
                return True
            if self.__class__ is not other.__class__:
                return NotImplemented
            return (
                self.name == other.name and
                self.config_cls == other.config_cls and
                self.binder == other.binder
            )

        __eq__.__qualname__ = f"{__class__.__qualname__}.__eq__"
        if '__eq__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __eq__ in class {__class__.__name__}")
        setattr(__class__, '__eq__', __eq__)

        __dataclass___setattr_frozen_fields = {
            'name',
            'config_cls',
            'binder',
        }

        def __setattr__(self, name, value):
            if (
                type(self) is __class__
                or name in __dataclass___setattr_frozen_fields
            ):
                raise __dataclass__FrozenInstanceError(f"cannot assign to field {name!r}")
            super(__class__, self).__setattr__(name, value)

        __setattr__.__qualname__ = f"{__class__.__qualname__}.__setattr__"
        if '__setattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __setattr__ in class {__class__.__name__}")
        setattr(__class__, '__setattr__', __setattr__)

        __dataclass___delattr_frozen_fields = {
            'name',
            'config_cls',
            'binder',
        }

        def __delattr__(self, name):
            if (
                type(self) is __class__
                or name in __dataclass___delattr_frozen_fields
            ):
                raise __dataclass__FrozenInstanceError(f"cannot delete field {name!r}")
            super(__class__, self).__delattr__(name)

        __delattr__.__qualname__ = f"{__class__.__qualname__}.__delattr__"
        if '__delattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __delattr__ in class {__class__.__name__}")
        setattr(__class__, '__delattr__', __delattr__)

        def __hash__(self):
            return hash((
                self.name,
                self.config_cls,
                self.binder,
            ))

        __hash__.__qualname__ = f"{__class__.__qualname__}.__hash__"
        setattr(__class__, '__hash__', __hash__)

        def __init__(
            self,
            *,
            name: __dataclass__init__fields__0__annotation,
            config_cls: __dataclass__init__fields__1__annotation,
            binder: __dataclass__init__fields__2__annotation,
        ) -> __dataclass__None:
            __dataclass__object_setattr(self, 'name', name)
            __dataclass__object_setattr(self, 'config_cls', config_cls)
            __dataclass__object_setattr(self, 'binder', binder)

        __init__.__qualname__ = f"{__class__.__qualname__}.__init__"
        if '__init__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __init__ in class {__class__.__name__}")
        setattr(__class__, '__init__', __init__)

        @__dataclass___recursive_repr()
        def __repr__(self):
            parts = []
            parts.append(f"name={self.name!r}")
            parts.append(f"config_cls={self.config_cls!r}")
            parts.append(f"binder={self.binder!r}")
            return (
                f"{self.__class__.__qualname__}("
                f"{', '.join(parts)}"
                f")"
            )

        __repr__.__qualname__ = f"{__class__.__qualname__}.__repr__"
        if '__repr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __repr__ in class {__class__.__name__}")
        setattr(__class__, '__repr__', __repr__)

    return _process_dataclass


@_register(
    plan_repr=(
        "Plans(tup=(CopyPlan(fields=('enable_writes',)), EqPlan(fields=('enable_writes',)), FrozenPlan(fields=('enable_"
        "writes',), allow_dynamic_dunder_attrs=False), HashPlan(action='add', fields=('enable_writes',), cache=False), "
        "InitPlan(fields=(InitPlan.Field(name='enable_writes', annotation=OpRef(name='init.fields.0.annotation'), defau"
        "lt=OpRef(name='init.fields.0.default'), default_factory=None, init=True, override=False, field_type=FieldType."
        "INSTANCE, coerce=None, validate=None, check_type=None),), self_param='self', std_params=(), kw_only_params=('e"
        "nable_writes',), frozen=True, slots=False, post_init_params=None, init_fns=(), validate_fns=()), ReprPlan(fiel"
        "ds=(ReprPlan.Field(name='enable_writes', kw_only=True, fn=None),), id=False, terse=False, default_fn=None)))"
    ),
    plan_repr_sha1='1bfce0db085f37de5c2dcfdf7b174bafdfaea6cd',
    op_ref_idents=(
        '__dataclass__init__fields__0__annotation',
        '__dataclass__init__fields__0__default',
    ),
    cls_names=(
        ('ommlds.minichain.modules.fs.configs', 'FsConfig'),
    ),
)
def _process_dataclass__1bfce0db085f37de5c2dcfdf7b174bafdfaea6cd():
    def _process_dataclass(
        *,
        __class__,
        __dataclass__init__fields__0__annotation,
        __dataclass__init__fields__0__default,
        __dataclass__FieldFnValidationError,  # noqa
        __dataclass__FieldTypeValidationError,  # noqa
        __dataclass__FnValidationError,  # noqa
        __dataclass__FrozenInstanceError=dataclasses.FrozenInstanceError,  # noqa
        __dataclass__FunctionType=types.FunctionType,  # noqa
        __dataclass__HAS_DEFAULT_FACTORY=dataclasses._HAS_DEFAULT_FACTORY,  # noqa
        __dataclass__MISSING=dataclasses.MISSING,  # noqa
        __dataclass__None=None,  # noqa
        __dataclass__TypeError=TypeError,  # noqa
        __dataclass___recursive_repr=reprlib.recursive_repr,  # noqa
        __dataclass__isinstance=isinstance,  # noqa
        __dataclass__object_setattr=object.__setattr__,  # noqa
        __dataclass__property=property,  # noqa
    ):
        def __copy__(self):
            if self.__class__ is not __class__:
                raise TypeError(self)
            return __class__(  # noqa
                enable_writes=self.enable_writes,
            )

        __copy__.__qualname__ = f"{__class__.__qualname__}.__copy__"
        if '__copy__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __copy__ in class {__class__.__name__}")
        setattr(__class__, '__copy__', __copy__)

        def __eq__(self, other):
            if self is other:
                return True
            if self.__class__ is not other.__class__:
                return NotImplemented
            return (
                self.enable_writes == other.enable_writes
            )

        __eq__.__qualname__ = f"{__class__.__qualname__}.__eq__"
        if '__eq__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __eq__ in class {__class__.__name__}")
        setattr(__class__, '__eq__', __eq__)

        __dataclass___setattr_frozen_fields = {
            'enable_writes',
        }

        def __setattr__(self, name, value):
            if (
                type(self) is __class__
                or name in __dataclass___setattr_frozen_fields
            ):
                raise __dataclass__FrozenInstanceError(f"cannot assign to field {name!r}")
            super(__class__, self).__setattr__(name, value)

        __setattr__.__qualname__ = f"{__class__.__qualname__}.__setattr__"
        if '__setattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __setattr__ in class {__class__.__name__}")
        setattr(__class__, '__setattr__', __setattr__)

        __dataclass___delattr_frozen_fields = {
            'enable_writes',
        }

        def __delattr__(self, name):
            if (
                type(self) is __class__
                or name in __dataclass___delattr_frozen_fields
            ):
                raise __dataclass__FrozenInstanceError(f"cannot delete field {name!r}")
            super(__class__, self).__delattr__(name)

        __delattr__.__qualname__ = f"{__class__.__qualname__}.__delattr__"
        if '__delattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __delattr__ in class {__class__.__name__}")
        setattr(__class__, '__delattr__', __delattr__)

        def __hash__(self):
            return hash((
                self.enable_writes,
            ))

        __hash__.__qualname__ = f"{__class__.__qualname__}.__hash__"
        setattr(__class__, '__hash__', __hash__)

        def __init__(
            self,
            *,
            enable_writes: __dataclass__init__fields__0__annotation = __dataclass__init__fields__0__default,
        ) -> __dataclass__None:
            __dataclass__object_setattr(self, 'enable_writes', enable_writes)

        __init__.__qualname__ = f"{__class__.__qualname__}.__init__"
        if '__init__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __init__ in class {__class__.__name__}")
        setattr(__class__, '__init__', __init__)

        @__dataclass___recursive_repr()
        def __repr__(self):
            parts = []
            parts.append(f"enable_writes={self.enable_writes!r}")
            return (
                f"{self.__class__.__qualname__}("
                f"{', '.join(parts)}"
                f")"
            )

        __repr__.__qualname__ = f"{__class__.__qualname__}.__repr__"
        if '__repr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __repr__ in class {__class__.__name__}")
        setattr(__class__, '__repr__', __repr__)

    return _process_dataclass


@_register(
    plan_repr=(
        "Plans(tup=(CopyPlan(fields=('item', 'd', 's', 'children')), InitPlan(fields=(InitPlan.Field(name='item', annot"
        "ation=OpRef(name='init.fields.0.annotation'), default=None, default_factory=None, init=True, override=False, f"
        "ield_type=FieldType.INSTANCE, coerce=None, validate=None, check_type=None), InitPlan.Field(name='d', annotatio"
        "n=OpRef(name='init.fields.1.annotation'), default=None, default_factory=None, init=True, override=False, field"
        "_type=FieldType.INSTANCE, coerce=None, validate=None, check_type=None), InitPlan.Field(name='s', annotation=Op"
        "Ref(name='init.fields.2.annotation'), default=OpRef(name='init.fields.2.default'), default_factory=None, init="
        "True, override=False, field_type=FieldType.INSTANCE, coerce=None, validate=None, check_type=None), InitPlan.Fi"
        "eld(name='children', annotation=OpRef(name='init.fields.3.annotation'), default=OpRef(name='init.fields.3.defa"
        "ult'), default_factory=None, init=True, override=False, field_type=FieldType.INSTANCE, coerce=None, validate=N"
        "one, check_type=None)), self_param='self', std_params=('item', 'd'), kw_only_params=('s', 'children'), frozen="
        "False, slots=False, post_init_params=None, init_fns=(), validate_fns=())))"
    ),
    plan_repr_sha1='0ffe7e6981ca897a53904accd1fc4d5bb759d34b',
    op_ref_idents=(
        '__dataclass__init__fields__0__annotation',
        '__dataclass__init__fields__1__annotation',
        '__dataclass__init__fields__2__annotation',
        '__dataclass__init__fields__2__default',
        '__dataclass__init__fields__3__annotation',
        '__dataclass__init__fields__3__default',
    ),
    cls_names=(
        ('ommlds.minichain.modules.fs.tools.recursivels.rendering', '_LsLine'),
    ),
)
def _process_dataclass__0ffe7e6981ca897a53904accd1fc4d5bb759d34b():
    def _process_dataclass(
        *,
        __class__,
        __dataclass__init__fields__0__annotation,
        __dataclass__init__fields__1__annotation,
        __dataclass__init__fields__2__annotation,
        __dataclass__init__fields__2__default,
        __dataclass__init__fields__3__annotation,
        __dataclass__init__fields__3__default,
        __dataclass__FieldFnValidationError,  # noqa
        __dataclass__FieldTypeValidationError,  # noqa
        __dataclass__FnValidationError,  # noqa
        __dataclass__FrozenInstanceError=dataclasses.FrozenInstanceError,  # noqa
        __dataclass__FunctionType=types.FunctionType,  # noqa
        __dataclass__HAS_DEFAULT_FACTORY=dataclasses._HAS_DEFAULT_FACTORY,  # noqa
        __dataclass__MISSING=dataclasses.MISSING,  # noqa
        __dataclass__None=None,  # noqa
        __dataclass__TypeError=TypeError,  # noqa
        __dataclass___recursive_repr=reprlib.recursive_repr,  # noqa
        __dataclass__isinstance=isinstance,  # noqa
        __dataclass__object_setattr=object.__setattr__,  # noqa
        __dataclass__property=property,  # noqa
    ):
        def __copy__(self):
            if self.__class__ is not __class__:
                raise TypeError(self)
            return __class__(  # noqa
                item=self.item,
                d=self.d,
                s=self.s,
                children=self.children,
            )

        __copy__.__qualname__ = f"{__class__.__qualname__}.__copy__"
        if '__copy__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __copy__ in class {__class__.__name__}")
        setattr(__class__, '__copy__', __copy__)

        def __init__(
            self,
            item: __dataclass__init__fields__0__annotation,
            d: __dataclass__init__fields__1__annotation,
            *,
            s: __dataclass__init__fields__2__annotation = __dataclass__init__fields__2__default,
            children: __dataclass__init__fields__3__annotation = __dataclass__init__fields__3__default,
        ) -> __dataclass__None:
            self.item = item
            self.d = d
            self.s = s
            self.children = children

        __init__.__qualname__ = f"{__class__.__qualname__}.__init__"
        if '__init__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __init__ in class {__class__.__name__}")
        setattr(__class__, '__init__', __init__)

    return _process_dataclass


@_register(
    plan_repr=(
        "Plans(tup=(CopyPlan(fields=('root', 'soft_max_len')), EqPlan(fields=('root', 'soft_max_len')), HashPlan(action"
        "='set_none', fields=None, cache=None), InitPlan(fields=(InitPlan.Field(name='root', annotation=OpRef(name='ini"
        "t.fields.0.annotation'), default=None, default_factory=None, init=True, override=False, field_type=FieldType.I"
        "NSTANCE, coerce=None, validate=None, check_type=None), InitPlan.Field(name='soft_max_len', annotation=OpRef(na"
        "me='init.fields.1.annotation'), default=OpRef(name='init.fields.1.default'), default_factory=None, init=True, "
        "override=False, field_type=FieldType.INSTANCE, coerce=None, validate=None, check_type=None)), self_param='self"
        "', std_params=('root',), kw_only_params=('soft_max_len',), frozen=False, slots=False, post_init_params=None, i"
        "nit_fns=(), validate_fns=()), ReprPlan(fields=(ReprPlan.Field(name='root', kw_only=False, fn=None), ReprPlan.F"
        "ield(name='soft_max_len', kw_only=True, fn=None)), id=False, terse=False, default_fn=None)))"
    ),
    plan_repr_sha1='68ec51656ec2810b10c4b88f8e5fa731750d733f',
    op_ref_idents=(
        '__dataclass__init__fields__0__annotation',
        '__dataclass__init__fields__1__annotation',
        '__dataclass__init__fields__1__default',
    ),
    cls_names=(
        ('ommlds.minichain.modules.fs.tools.recursivels.rendering', '_LsRenderRun'),
    ),
)
def _process_dataclass__68ec51656ec2810b10c4b88f8e5fa731750d733f():
    def _process_dataclass(
        *,
        __class__,
        __dataclass__init__fields__0__annotation,
        __dataclass__init__fields__1__annotation,
        __dataclass__init__fields__1__default,
        __dataclass__FieldFnValidationError,  # noqa
        __dataclass__FieldTypeValidationError,  # noqa
        __dataclass__FnValidationError,  # noqa
        __dataclass__FrozenInstanceError=dataclasses.FrozenInstanceError,  # noqa
        __dataclass__FunctionType=types.FunctionType,  # noqa
        __dataclass__HAS_DEFAULT_FACTORY=dataclasses._HAS_DEFAULT_FACTORY,  # noqa
        __dataclass__MISSING=dataclasses.MISSING,  # noqa
        __dataclass__None=None,  # noqa
        __dataclass__TypeError=TypeError,  # noqa
        __dataclass___recursive_repr=reprlib.recursive_repr,  # noqa
        __dataclass__isinstance=isinstance,  # noqa
        __dataclass__object_setattr=object.__setattr__,  # noqa
        __dataclass__property=property,  # noqa
    ):
        def __copy__(self):
            if self.__class__ is not __class__:
                raise TypeError(self)
            return __class__(  # noqa
                root=self.root,
                soft_max_len=self.soft_max_len,
            )

        __copy__.__qualname__ = f"{__class__.__qualname__}.__copy__"
        if '__copy__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __copy__ in class {__class__.__name__}")
        setattr(__class__, '__copy__', __copy__)

        def __eq__(self, other):
            if self is other:
                return True
            if self.__class__ is not other.__class__:
                return NotImplemented
            return (
                self.root == other.root and
                self.soft_max_len == other.soft_max_len
            )

        __eq__.__qualname__ = f"{__class__.__qualname__}.__eq__"
        if '__eq__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __eq__ in class {__class__.__name__}")
        setattr(__class__, '__eq__', __eq__)

        setattr(__class__, '__hash__', None)

        def __init__(
            self,
            root: __dataclass__init__fields__0__annotation,
            *,
            soft_max_len: __dataclass__init__fields__1__annotation = __dataclass__init__fields__1__default,
        ) -> __dataclass__None:
            self.root = root
            self.soft_max_len = soft_max_len

        __init__.__qualname__ = f"{__class__.__qualname__}.__init__"
        if '__init__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __init__ in class {__class__.__name__}")
        setattr(__class__, '__init__', __init__)

        @__dataclass___recursive_repr()
        def __repr__(self):
            parts = []
            parts.append(f"root={self.root!r}")
            parts.append(f"soft_max_len={self.soft_max_len!r}")
            return (
                f"{self.__class__.__qualname__}("
                f"{', '.join(parts)}"
                f")"
            )

        __repr__.__qualname__ = f"{__class__.__qualname__}.__repr__"
        if '__repr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __repr__ in class {__class__.__name__}")
        setattr(__class__, '__repr__', __repr__)

    return _process_dataclass


@_register(
    plan_repr=(
        "Plans(tup=(CopyPlan(fields=('name', 'path', 'children')), EqPlan(fields=('name', 'path', 'children')), FrozenP"
        "lan(fields=('name', 'path', 'children'), allow_dynamic_dunder_attrs=False), HashPlan(action='add', fields=('na"
        "me', 'path', 'children'), cache=False), InitPlan(fields=(InitPlan.Field(name='name', annotation=OpRef(name='in"
        "it.fields.0.annotation'), default=None, default_factory=None, init=True, override=False, field_type=FieldType."
        "INSTANCE, coerce=None, validate=None, check_type=None), InitPlan.Field(name='path', annotation=OpRef(name='ini"
        "t.fields.1.annotation'), default=None, default_factory=None, init=True, override=False, field_type=FieldType.I"
        "NSTANCE, coerce=None, validate=None, check_type=None), InitPlan.Field(name='children', annotation=OpRef(name='"
        "init.fields.2.annotation'), default=None, default_factory=None, init=True, override=False, field_type=FieldTyp"
        "e.INSTANCE, coerce=None, validate=None, check_type=None)), self_param='self', std_params=('name', 'path', 'chi"
        "ldren'), kw_only_params=(), frozen=True, slots=False, post_init_params=None, init_fns=(), validate_fns=()), Re"
        "prPlan(fields=(ReprPlan.Field(name='name', kw_only=False, fn=None), ReprPlan.Field(name='path', kw_only=False,"
        " fn=None), ReprPlan.Field(name='children', kw_only=False, fn=None)), id=False, terse=False, default_fn=None)))"
    ),
    plan_repr_sha1='8fc7babd1081656cee23d33050b77c4791d93c20',
    op_ref_idents=(
        '__dataclass__init__fields__0__annotation',
        '__dataclass__init__fields__1__annotation',
        '__dataclass__init__fields__2__annotation',
    ),
    cls_names=(
        ('ommlds.minichain.modules.fs.tools.recursivels.running', 'DirLsItem'),
    ),
)
def _process_dataclass__8fc7babd1081656cee23d33050b77c4791d93c20():
    def _process_dataclass(
        *,
        __class__,
        __dataclass__init__fields__0__annotation,
        __dataclass__init__fields__1__annotation,
        __dataclass__init__fields__2__annotation,
        __dataclass__FieldFnValidationError,  # noqa
        __dataclass__FieldTypeValidationError,  # noqa
        __dataclass__FnValidationError,  # noqa
        __dataclass__FrozenInstanceError=dataclasses.FrozenInstanceError,  # noqa
        __dataclass__FunctionType=types.FunctionType,  # noqa
        __dataclass__HAS_DEFAULT_FACTORY=dataclasses._HAS_DEFAULT_FACTORY,  # noqa
        __dataclass__MISSING=dataclasses.MISSING,  # noqa
        __dataclass__None=None,  # noqa
        __dataclass__TypeError=TypeError,  # noqa
        __dataclass___recursive_repr=reprlib.recursive_repr,  # noqa
        __dataclass__isinstance=isinstance,  # noqa
        __dataclass__object_setattr=object.__setattr__,  # noqa
        __dataclass__property=property,  # noqa
    ):
        def __copy__(self):
            if self.__class__ is not __class__:
                raise TypeError(self)
            return __class__(  # noqa
                name=self.name,
                path=self.path,
                children=self.children,
            )

        __copy__.__qualname__ = f"{__class__.__qualname__}.__copy__"
        if '__copy__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __copy__ in class {__class__.__name__}")
        setattr(__class__, '__copy__', __copy__)

        def __eq__(self, other):
            if self is other:
                return True
            if self.__class__ is not other.__class__:
                return NotImplemented
            return (
                self.name == other.name and
                self.path == other.path and
                self.children == other.children
            )

        __eq__.__qualname__ = f"{__class__.__qualname__}.__eq__"
        if '__eq__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __eq__ in class {__class__.__name__}")
        setattr(__class__, '__eq__', __eq__)

        __dataclass___setattr_frozen_fields = {
            'name',
            'path',
            'children',
        }

        def __setattr__(self, name, value):
            if (
                type(self) is __class__
                or name in __dataclass___setattr_frozen_fields
            ):
                raise __dataclass__FrozenInstanceError(f"cannot assign to field {name!r}")
            super(__class__, self).__setattr__(name, value)

        __setattr__.__qualname__ = f"{__class__.__qualname__}.__setattr__"
        if '__setattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __setattr__ in class {__class__.__name__}")
        setattr(__class__, '__setattr__', __setattr__)

        __dataclass___delattr_frozen_fields = {
            'name',
            'path',
            'children',
        }

        def __delattr__(self, name):
            if (
                type(self) is __class__
                or name in __dataclass___delattr_frozen_fields
            ):
                raise __dataclass__FrozenInstanceError(f"cannot delete field {name!r}")
            super(__class__, self).__delattr__(name)

        __delattr__.__qualname__ = f"{__class__.__qualname__}.__delattr__"
        if '__delattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __delattr__ in class {__class__.__name__}")
        setattr(__class__, '__delattr__', __delattr__)

        def __hash__(self):
            return hash((
                self.name,
                self.path,
                self.children,
            ))

        __hash__.__qualname__ = f"{__class__.__qualname__}.__hash__"
        setattr(__class__, '__hash__', __hash__)

        def __init__(
            self,
            name: __dataclass__init__fields__0__annotation,
            path: __dataclass__init__fields__1__annotation,
            children: __dataclass__init__fields__2__annotation,
        ) -> __dataclass__None:
            __dataclass__object_setattr(self, 'name', name)
            __dataclass__object_setattr(self, 'path', path)
            __dataclass__object_setattr(self, 'children', children)

        __init__.__qualname__ = f"{__class__.__qualname__}.__init__"
        if '__init__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __init__ in class {__class__.__name__}")
        setattr(__class__, '__init__', __init__)

        @__dataclass___recursive_repr()
        def __repr__(self):
            parts = []
            parts.append(f"name={self.name!r}")
            parts.append(f"path={self.path!r}")
            parts.append(f"children={self.children!r}")
            return (
                f"{self.__class__.__qualname__}("
                f"{', '.join(parts)}"
                f")"
            )

        __repr__.__qualname__ = f"{__class__.__qualname__}.__repr__"
        if '__repr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __repr__ in class {__class__.__name__}")
        setattr(__class__, '__repr__', __repr__)

    return _process_dataclass


@_register(
    plan_repr=(
        "Plans(tup=(CopyPlan(fields=('name', 'path')), EqPlan(fields=('name', 'path')), FrozenPlan(fields=('name', 'pat"
        "h'), allow_dynamic_dunder_attrs=False), HashPlan(action='add', fields=('name', 'path'), cache=False), InitPlan"
        "(fields=(InitPlan.Field(name='name', annotation=OpRef(name='init.fields.0.annotation'), default=None, default_"
        "factory=None, init=True, override=False, field_type=FieldType.INSTANCE, coerce=None, validate=None, check_type"
        "=None), InitPlan.Field(name='path', annotation=OpRef(name='init.fields.1.annotation'), default=None, default_f"
        "actory=None, init=True, override=False, field_type=FieldType.INSTANCE, coerce=None, validate=None, check_type="
        "None)), self_param='self', std_params=('name', 'path'), kw_only_params=(), frozen=True, slots=False, post_init"
        "_params=None, init_fns=(), validate_fns=()), ReprPlan(fields=(ReprPlan.Field(name='name', kw_only=False, fn=No"
        "ne), ReprPlan.Field(name='path', kw_only=False, fn=None)), id=False, terse=False, default_fn=None)))"
    ),
    plan_repr_sha1='9a639e5124ba99ab9dbe43d0f65a5795e369a423',
    op_ref_idents=(
        '__dataclass__init__fields__0__annotation',
        '__dataclass__init__fields__1__annotation',
    ),
    cls_names=(
        ('ommlds.minichain.modules.fs.tools.recursivels.running', 'FileLsItem'),
        ('ommlds.minichain.modules.fs.tools.recursivels.running', 'LsItem'),
    ),
)
def _process_dataclass__9a639e5124ba99ab9dbe43d0f65a5795e369a423():
    def _process_dataclass(
        *,
        __class__,
        __dataclass__init__fields__0__annotation,
        __dataclass__init__fields__1__annotation,
        __dataclass__FieldFnValidationError,  # noqa
        __dataclass__FieldTypeValidationError,  # noqa
        __dataclass__FnValidationError,  # noqa
        __dataclass__FrozenInstanceError=dataclasses.FrozenInstanceError,  # noqa
        __dataclass__FunctionType=types.FunctionType,  # noqa
        __dataclass__HAS_DEFAULT_FACTORY=dataclasses._HAS_DEFAULT_FACTORY,  # noqa
        __dataclass__MISSING=dataclasses.MISSING,  # noqa
        __dataclass__None=None,  # noqa
        __dataclass__TypeError=TypeError,  # noqa
        __dataclass___recursive_repr=reprlib.recursive_repr,  # noqa
        __dataclass__isinstance=isinstance,  # noqa
        __dataclass__object_setattr=object.__setattr__,  # noqa
        __dataclass__property=property,  # noqa
    ):
        def __copy__(self):
            if self.__class__ is not __class__:
                raise TypeError(self)
            return __class__(  # noqa
                name=self.name,
                path=self.path,
            )

        __copy__.__qualname__ = f"{__class__.__qualname__}.__copy__"
        if '__copy__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __copy__ in class {__class__.__name__}")
        setattr(__class__, '__copy__', __copy__)

        def __eq__(self, other):
            if self is other:
                return True
            if self.__class__ is not other.__class__:
                return NotImplemented
            return (
                self.name == other.name and
                self.path == other.path
            )

        __eq__.__qualname__ = f"{__class__.__qualname__}.__eq__"
        if '__eq__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __eq__ in class {__class__.__name__}")
        setattr(__class__, '__eq__', __eq__)

        __dataclass___setattr_frozen_fields = {
            'name',
            'path',
        }

        def __setattr__(self, name, value):
            if (
                type(self) is __class__
                or name in __dataclass___setattr_frozen_fields
            ):
                raise __dataclass__FrozenInstanceError(f"cannot assign to field {name!r}")
            super(__class__, self).__setattr__(name, value)

        __setattr__.__qualname__ = f"{__class__.__qualname__}.__setattr__"
        if '__setattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __setattr__ in class {__class__.__name__}")
        setattr(__class__, '__setattr__', __setattr__)

        __dataclass___delattr_frozen_fields = {
            'name',
            'path',
        }

        def __delattr__(self, name):
            if (
                type(self) is __class__
                or name in __dataclass___delattr_frozen_fields
            ):
                raise __dataclass__FrozenInstanceError(f"cannot delete field {name!r}")
            super(__class__, self).__delattr__(name)

        __delattr__.__qualname__ = f"{__class__.__qualname__}.__delattr__"
        if '__delattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __delattr__ in class {__class__.__name__}")
        setattr(__class__, '__delattr__', __delattr__)

        def __hash__(self):
            return hash((
                self.name,
                self.path,
            ))

        __hash__.__qualname__ = f"{__class__.__qualname__}.__hash__"
        setattr(__class__, '__hash__', __hash__)

        def __init__(
            self,
            name: __dataclass__init__fields__0__annotation,
            path: __dataclass__init__fields__1__annotation,
        ) -> __dataclass__None:
            __dataclass__object_setattr(self, 'name', name)
            __dataclass__object_setattr(self, 'path', path)

        __init__.__qualname__ = f"{__class__.__qualname__}.__init__"
        if '__init__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __init__ in class {__class__.__name__}")
        setattr(__class__, '__init__', __init__)

        @__dataclass___recursive_repr()
        def __repr__(self):
            parts = []
            parts.append(f"name={self.name!r}")
            parts.append(f"path={self.path!r}")
            return (
                f"{self.__class__.__qualname__}("
                f"{', '.join(parts)}"
                f")"
            )

        __repr__.__qualname__ = f"{__class__.__qualname__}.__repr__"
        if '__repr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __repr__ in class {__class__.__name__}")
        setattr(__class__, '__repr__', __repr__)

    return _process_dataclass


@_register(
    plan_repr=(
        "Plans(tup=(CopyPlan(fields=('skill_paths',)), EqPlan(fields=('skill_paths',)), FrozenPlan(fields=('skill_paths"
        "',), allow_dynamic_dunder_attrs=False), HashPlan(action='add', fields=('skill_paths',), cache=False), InitPlan"
        "(fields=(InitPlan.Field(name='skill_paths', annotation=OpRef(name='init.fields.0.annotation'), default=OpRef(n"
        "ame='init.fields.0.default'), default_factory=None, init=True, override=False, field_type=FieldType.INSTANCE, "
        "coerce=None, validate=None, check_type=None),), self_param='self', std_params=(), kw_only_params=('skill_paths"
        "',), frozen=True, slots=False, post_init_params=None, init_fns=(), validate_fns=(InitPlan.ValidateFnWithParams"
        "(fn=OpRef(name='init.validate_fns.0'), params=('self',)),)), ReprPlan(fields=(ReprPlan.Field(name='skill_paths"
        "', kw_only=True, fn=None),), id=False, terse=False, default_fn=None)))"
    ),
    plan_repr_sha1='c088fcf850721e4dcac4781f19820a6668f38121',
    op_ref_idents=(
        '__dataclass__init__fields__0__annotation',
        '__dataclass__init__fields__0__default',
        '__dataclass__init__validate_fns__0',
    ),
    cls_names=(
        ('ommlds.minichain.modules.skills.configs', 'SkillsConfig'),
    ),
)
def _process_dataclass__c088fcf850721e4dcac4781f19820a6668f38121():
    def _process_dataclass(
        *,
        __class__,
        __dataclass__init__fields__0__annotation,
        __dataclass__init__fields__0__default,
        __dataclass__init__validate_fns__0,
        __dataclass__FieldFnValidationError,  # noqa
        __dataclass__FieldTypeValidationError,  # noqa
        __dataclass__FnValidationError,  # noqa
        __dataclass__FrozenInstanceError=dataclasses.FrozenInstanceError,  # noqa
        __dataclass__FunctionType=types.FunctionType,  # noqa
        __dataclass__HAS_DEFAULT_FACTORY=dataclasses._HAS_DEFAULT_FACTORY,  # noqa
        __dataclass__MISSING=dataclasses.MISSING,  # noqa
        __dataclass__None=None,  # noqa
        __dataclass__TypeError=TypeError,  # noqa
        __dataclass___recursive_repr=reprlib.recursive_repr,  # noqa
        __dataclass__isinstance=isinstance,  # noqa
        __dataclass__object_setattr=object.__setattr__,  # noqa
        __dataclass__property=property,  # noqa
    ):
        def __copy__(self):
            if self.__class__ is not __class__:
                raise TypeError(self)
            return __class__(  # noqa
                skill_paths=self.skill_paths,
            )

        __copy__.__qualname__ = f"{__class__.__qualname__}.__copy__"
        if '__copy__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __copy__ in class {__class__.__name__}")
        setattr(__class__, '__copy__', __copy__)

        def __eq__(self, other):
            if self is other:
                return True
            if self.__class__ is not other.__class__:
                return NotImplemented
            return (
                self.skill_paths == other.skill_paths
            )

        __eq__.__qualname__ = f"{__class__.__qualname__}.__eq__"
        if '__eq__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __eq__ in class {__class__.__name__}")
        setattr(__class__, '__eq__', __eq__)

        __dataclass___setattr_frozen_fields = {
            'skill_paths',
        }

        def __setattr__(self, name, value):
            if (
                type(self) is __class__
                or name in __dataclass___setattr_frozen_fields
            ):
                raise __dataclass__FrozenInstanceError(f"cannot assign to field {name!r}")
            super(__class__, self).__setattr__(name, value)

        __setattr__.__qualname__ = f"{__class__.__qualname__}.__setattr__"
        if '__setattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __setattr__ in class {__class__.__name__}")
        setattr(__class__, '__setattr__', __setattr__)

        __dataclass___delattr_frozen_fields = {
            'skill_paths',
        }

        def __delattr__(self, name):
            if (
                type(self) is __class__
                or name in __dataclass___delattr_frozen_fields
            ):
                raise __dataclass__FrozenInstanceError(f"cannot delete field {name!r}")
            super(__class__, self).__delattr__(name)

        __delattr__.__qualname__ = f"{__class__.__qualname__}.__delattr__"
        if '__delattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __delattr__ in class {__class__.__name__}")
        setattr(__class__, '__delattr__', __delattr__)

        def __hash__(self):
            return hash((
                self.skill_paths,
            ))

        __hash__.__qualname__ = f"{__class__.__qualname__}.__hash__"
        setattr(__class__, '__hash__', __hash__)

        def __init__(
            self,
            *,
            skill_paths: __dataclass__init__fields__0__annotation = __dataclass__init__fields__0__default,
        ) -> __dataclass__None:
            __dataclass__object_setattr(self, 'skill_paths', skill_paths)
            if not __dataclass__init__validate_fns__0(
                self,
            ):
                raise __dataclass__FnValidationError(
                    obj=self,
                    fn=__dataclass__init__validate_fns__0,
                )

        __init__.__qualname__ = f"{__class__.__qualname__}.__init__"
        if '__init__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __init__ in class {__class__.__name__}")
        setattr(__class__, '__init__', __init__)

        @__dataclass___recursive_repr()
        def __repr__(self):
            parts = []
            parts.append(f"skill_paths={self.skill_paths!r}")
            return (
                f"{self.__class__.__qualname__}("
                f"{', '.join(parts)}"
                f")"
            )

        __repr__.__qualname__ = f"{__class__.__qualname__}.__repr__"
        if '__repr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __repr__ in class {__class__.__name__}")
        setattr(__class__, '__repr__', __repr__)

    return _process_dataclass


@_register(
    plan_repr=(
        "Plans(tup=(CopyPlan(fields=('header', 'body')), EqPlan(fields=('header', 'body')), FrozenPlan(fields=('header'"
        ", 'body'), allow_dynamic_dunder_attrs=False), HashPlan(action='add', fields=('header', 'body'), cache=False), "
        "InitPlan(fields=(InitPlan.Field(name='header', annotation=OpRef(name='init.fields.0.annotation'), default=None"
        ", default_factory=None, init=True, override=False, field_type=FieldType.INSTANCE, coerce=None, validate=None, "
        "check_type=None), InitPlan.Field(name='body', annotation=OpRef(name='init.fields.1.annotation'), default=None,"
        " default_factory=None, init=True, override=False, field_type=FieldType.INSTANCE, coerce=None, validate=None, c"
        "heck_type=None)), self_param='self', std_params=('header', 'body'), kw_only_params=(), frozen=True, slots=Fals"
        "e, post_init_params=None, init_fns=(), validate_fns=()), ReprPlan(fields=(ReprPlan.Field(name='header', kw_onl"
        "y=False, fn=None), ReprPlan.Field(name='body', kw_only=False, fn=None)), id=False, terse=False, default_fn=Non"
        "e)))"
    ),
    plan_repr_sha1='883b94b4b8e3efa5ad1ef4a7e4a87fa6444cc38d',
    op_ref_idents=(
        '__dataclass__init__fields__0__annotation',
        '__dataclass__init__fields__1__annotation',
    ),
    cls_names=(
        ('ommlds.minichain.modules.skills.types', 'Skill'),
    ),
)
def _process_dataclass__883b94b4b8e3efa5ad1ef4a7e4a87fa6444cc38d():
    def _process_dataclass(
        *,
        __class__,
        __dataclass__init__fields__0__annotation,
        __dataclass__init__fields__1__annotation,
        __dataclass__FieldFnValidationError,  # noqa
        __dataclass__FieldTypeValidationError,  # noqa
        __dataclass__FnValidationError,  # noqa
        __dataclass__FrozenInstanceError=dataclasses.FrozenInstanceError,  # noqa
        __dataclass__FunctionType=types.FunctionType,  # noqa
        __dataclass__HAS_DEFAULT_FACTORY=dataclasses._HAS_DEFAULT_FACTORY,  # noqa
        __dataclass__MISSING=dataclasses.MISSING,  # noqa
        __dataclass__None=None,  # noqa
        __dataclass__TypeError=TypeError,  # noqa
        __dataclass___recursive_repr=reprlib.recursive_repr,  # noqa
        __dataclass__isinstance=isinstance,  # noqa
        __dataclass__object_setattr=object.__setattr__,  # noqa
        __dataclass__property=property,  # noqa
    ):
        def __copy__(self):
            if self.__class__ is not __class__:
                raise TypeError(self)
            return __class__(  # noqa
                header=self.header,
                body=self.body,
            )

        __copy__.__qualname__ = f"{__class__.__qualname__}.__copy__"
        if '__copy__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __copy__ in class {__class__.__name__}")
        setattr(__class__, '__copy__', __copy__)

        def __eq__(self, other):
            if self is other:
                return True
            if self.__class__ is not other.__class__:
                return NotImplemented
            return (
                self.header == other.header and
                self.body == other.body
            )

        __eq__.__qualname__ = f"{__class__.__qualname__}.__eq__"
        if '__eq__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __eq__ in class {__class__.__name__}")
        setattr(__class__, '__eq__', __eq__)

        __dataclass___setattr_frozen_fields = {
            'header',
            'body',
        }

        def __setattr__(self, name, value):
            if (
                type(self) is __class__
                or name in __dataclass___setattr_frozen_fields
            ):
                raise __dataclass__FrozenInstanceError(f"cannot assign to field {name!r}")
            super(__class__, self).__setattr__(name, value)

        __setattr__.__qualname__ = f"{__class__.__qualname__}.__setattr__"
        if '__setattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __setattr__ in class {__class__.__name__}")
        setattr(__class__, '__setattr__', __setattr__)

        __dataclass___delattr_frozen_fields = {
            'header',
            'body',
        }

        def __delattr__(self, name):
            if (
                type(self) is __class__
                or name in __dataclass___delattr_frozen_fields
            ):
                raise __dataclass__FrozenInstanceError(f"cannot delete field {name!r}")
            super(__class__, self).__delattr__(name)

        __delattr__.__qualname__ = f"{__class__.__qualname__}.__delattr__"
        if '__delattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __delattr__ in class {__class__.__name__}")
        setattr(__class__, '__delattr__', __delattr__)

        def __hash__(self):
            return hash((
                self.header,
                self.body,
            ))

        __hash__.__qualname__ = f"{__class__.__qualname__}.__hash__"
        setattr(__class__, '__hash__', __hash__)

        def __init__(
            self,
            header: __dataclass__init__fields__0__annotation,
            body: __dataclass__init__fields__1__annotation,
        ) -> __dataclass__None:
            __dataclass__object_setattr(self, 'header', header)
            __dataclass__object_setattr(self, 'body', body)

        __init__.__qualname__ = f"{__class__.__qualname__}.__init__"
        if '__init__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __init__ in class {__class__.__name__}")
        setattr(__class__, '__init__', __init__)

        @__dataclass___recursive_repr()
        def __repr__(self):
            parts = []
            parts.append(f"header={self.header!r}")
            parts.append(f"body={self.body!r}")
            return (
                f"{self.__class__.__qualname__}("
                f"{', '.join(parts)}"
                f")"
            )

        __repr__.__qualname__ = f"{__class__.__qualname__}.__repr__"
        if '__repr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __repr__ in class {__class__.__name__}")
        setattr(__class__, '__repr__', __repr__)

    return _process_dataclass


@_register(
    plan_repr=(
        "Plans(tup=(CopyPlan(fields=('name', 'description', 'license', 'compatibility', 'metadata', 'allowed_tools')), "
        "EqPlan(fields=('name', 'description', 'license', 'compatibility', 'metadata', 'allowed_tools')), FrozenPlan(fi"
        "elds=('name', 'description', 'license', 'compatibility', 'metadata', 'allowed_tools'), allow_dynamic_dunder_at"
        "trs=False), HashPlan(action='add', fields=('name', 'description', 'license', 'compatibility', 'metadata', 'all"
        "owed_tools'), cache=False), InitPlan(fields=(InitPlan.Field(name='name', annotation=OpRef(name='init.fields.0."
        "annotation'), default=None, default_factory=None, init=True, override=False, field_type=FieldType.INSTANCE, co"
        "erce=None, validate=None, check_type=None), InitPlan.Field(name='description', annotation=OpRef(name='init.fie"
        "lds.1.annotation'), default=None, default_factory=None, init=True, override=False, field_type=FieldType.INSTAN"
        "CE, coerce=None, validate=None, check_type=None), InitPlan.Field(name='license', annotation=OpRef(name='init.f"
        "ields.2.annotation'), default=OpRef(name='init.fields.2.default'), default_factory=None, init=True, override=F"
        "alse, field_type=FieldType.INSTANCE, coerce=None, validate=None, check_type=None), InitPlan.Field(name='compat"
        "ibility', annotation=OpRef(name='init.fields.3.annotation'), default=OpRef(name='init.fields.3.default'), defa"
        "ult_factory=None, init=True, override=False, field_type=FieldType.INSTANCE, coerce=None, validate=None, check_"
        "type=None), InitPlan.Field(name='metadata', annotation=OpRef(name='init.fields.4.annotation'), default=OpRef(n"
        "ame='init.fields.4.default'), default_factory=None, init=True, override=False, field_type=FieldType.INSTANCE, "
        "coerce=None, validate=None, check_type=None), InitPlan.Field(name='allowed_tools', annotation=OpRef(name='init"
        ".fields.5.annotation'), default=OpRef(name='init.fields.5.default'), default_factory=None, init=True, override"
        "=False, field_type=FieldType.INSTANCE, coerce=None, validate=None, check_type=None)), self_param='self', std_p"
        "arams=(), kw_only_params=('name', 'description', 'license', 'compatibility', 'metadata', 'allowed_tools'), fro"
        "zen=True, slots=False, post_init_params=None, init_fns=(), validate_fns=()), ReprPlan(fields=(ReprPlan.Field(n"
        "ame='name', kw_only=True, fn=None), ReprPlan.Field(name='description', kw_only=True, fn=None), ReprPlan.Field("
        "name='license', kw_only=True, fn=None), ReprPlan.Field(name='compatibility', kw_only=True, fn=None), ReprPlan."
        "Field(name='metadata', kw_only=True, fn=None), ReprPlan.Field(name='allowed_tools', kw_only=True, fn=None)), i"
        "d=False, terse=False, default_fn=OpRef(name='repr.default_fn'))))"
    ),
    plan_repr_sha1='62dee6166588a1c4268bef1e4fdb39a238534707',
    op_ref_idents=(
        '__dataclass__init__fields__0__annotation',
        '__dataclass__init__fields__1__annotation',
        '__dataclass__init__fields__2__annotation',
        '__dataclass__init__fields__2__default',
        '__dataclass__init__fields__3__annotation',
        '__dataclass__init__fields__3__default',
        '__dataclass__init__fields__4__annotation',
        '__dataclass__init__fields__4__default',
        '__dataclass__init__fields__5__annotation',
        '__dataclass__init__fields__5__default',
        '__dataclass__repr__default_fn',
    ),
    cls_names=(
        ('ommlds.minichain.modules.skills.types', 'SkillHeader'),
    ),
)
def _process_dataclass__62dee6166588a1c4268bef1e4fdb39a238534707():
    def _process_dataclass(
        *,
        __class__,
        __dataclass__init__fields__0__annotation,
        __dataclass__init__fields__1__annotation,
        __dataclass__init__fields__2__annotation,
        __dataclass__init__fields__2__default,
        __dataclass__init__fields__3__annotation,
        __dataclass__init__fields__3__default,
        __dataclass__init__fields__4__annotation,
        __dataclass__init__fields__4__default,
        __dataclass__init__fields__5__annotation,
        __dataclass__init__fields__5__default,
        __dataclass__repr__default_fn,
        __dataclass__FieldFnValidationError,  # noqa
        __dataclass__FieldTypeValidationError,  # noqa
        __dataclass__FnValidationError,  # noqa
        __dataclass__FrozenInstanceError=dataclasses.FrozenInstanceError,  # noqa
        __dataclass__FunctionType=types.FunctionType,  # noqa
        __dataclass__HAS_DEFAULT_FACTORY=dataclasses._HAS_DEFAULT_FACTORY,  # noqa
        __dataclass__MISSING=dataclasses.MISSING,  # noqa
        __dataclass__None=None,  # noqa
        __dataclass__TypeError=TypeError,  # noqa
        __dataclass___recursive_repr=reprlib.recursive_repr,  # noqa
        __dataclass__isinstance=isinstance,  # noqa
        __dataclass__object_setattr=object.__setattr__,  # noqa
        __dataclass__property=property,  # noqa
    ):
        def __copy__(self):
            if self.__class__ is not __class__:
                raise TypeError(self)
            return __class__(  # noqa
                name=self.name,
                description=self.description,
                license=self.license,
                compatibility=self.compatibility,
                metadata=self.metadata,
                allowed_tools=self.allowed_tools,
            )

        __copy__.__qualname__ = f"{__class__.__qualname__}.__copy__"
        if '__copy__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __copy__ in class {__class__.__name__}")
        setattr(__class__, '__copy__', __copy__)

        def __eq__(self, other):
            if self is other:
                return True
            if self.__class__ is not other.__class__:
                return NotImplemented
            return (
                self.name == other.name and
                self.description == other.description and
                self.license == other.license and
                self.compatibility == other.compatibility and
                self.metadata == other.metadata and
                self.allowed_tools == other.allowed_tools
            )

        __eq__.__qualname__ = f"{__class__.__qualname__}.__eq__"
        if '__eq__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __eq__ in class {__class__.__name__}")
        setattr(__class__, '__eq__', __eq__)

        __dataclass___setattr_frozen_fields = {
            'name',
            'description',
            'license',
            'compatibility',
            'metadata',
            'allowed_tools',
        }

        def __setattr__(self, name, value):
            if (
                type(self) is __class__
                or name in __dataclass___setattr_frozen_fields
            ):
                raise __dataclass__FrozenInstanceError(f"cannot assign to field {name!r}")
            super(__class__, self).__setattr__(name, value)

        __setattr__.__qualname__ = f"{__class__.__qualname__}.__setattr__"
        if '__setattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __setattr__ in class {__class__.__name__}")
        setattr(__class__, '__setattr__', __setattr__)

        __dataclass___delattr_frozen_fields = {
            'name',
            'description',
            'license',
            'compatibility',
            'metadata',
            'allowed_tools',
        }

        def __delattr__(self, name):
            if (
                type(self) is __class__
                or name in __dataclass___delattr_frozen_fields
            ):
                raise __dataclass__FrozenInstanceError(f"cannot delete field {name!r}")
            super(__class__, self).__delattr__(name)

        __delattr__.__qualname__ = f"{__class__.__qualname__}.__delattr__"
        if '__delattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __delattr__ in class {__class__.__name__}")
        setattr(__class__, '__delattr__', __delattr__)

        def __hash__(self):
            return hash((
                self.name,
                self.description,
                self.license,
                self.compatibility,
                self.metadata,
                self.allowed_tools,
            ))

        __hash__.__qualname__ = f"{__class__.__qualname__}.__hash__"
        setattr(__class__, '__hash__', __hash__)

        def __init__(
            self,
            *,
            name: __dataclass__init__fields__0__annotation,
            description: __dataclass__init__fields__1__annotation,
            license: __dataclass__init__fields__2__annotation = __dataclass__init__fields__2__default,
            compatibility: __dataclass__init__fields__3__annotation = __dataclass__init__fields__3__default,
            metadata: __dataclass__init__fields__4__annotation = __dataclass__init__fields__4__default,
            allowed_tools: __dataclass__init__fields__5__annotation = __dataclass__init__fields__5__default,
        ) -> __dataclass__None:
            __dataclass__object_setattr(self, 'name', name)
            __dataclass__object_setattr(self, 'description', description)
            __dataclass__object_setattr(self, 'license', license)
            __dataclass__object_setattr(self, 'compatibility', compatibility)
            __dataclass__object_setattr(self, 'metadata', metadata)
            __dataclass__object_setattr(self, 'allowed_tools', allowed_tools)

        __init__.__qualname__ = f"{__class__.__qualname__}.__init__"
        if '__init__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __init__ in class {__class__.__name__}")
        setattr(__class__, '__init__', __init__)

        @__dataclass___recursive_repr()
        def __repr__(self):
            parts = []
            if (s := __dataclass__repr__default_fn(self.name)) is not None:
                parts.append(f"name={s}")
            if (s := __dataclass__repr__default_fn(self.description)) is not None:
                parts.append(f"description={s}")
            if (s := __dataclass__repr__default_fn(self.license)) is not None:
                parts.append(f"license={s}")
            if (s := __dataclass__repr__default_fn(self.compatibility)) is not None:
                parts.append(f"compatibility={s}")
            if (s := __dataclass__repr__default_fn(self.metadata)) is not None:
                parts.append(f"metadata={s}")
            if (s := __dataclass__repr__default_fn(self.allowed_tools)) is not None:
                parts.append(f"allowed_tools={s}")
            return (
                f"{self.__class__.__qualname__}("
                f"{', '.join(parts)}"
                f")"
            )

        __repr__.__qualname__ = f"{__class__.__qualname__}.__repr__"
        if '__repr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __repr__ in class {__class__.__name__}")
        setattr(__class__, '__repr__', __repr__)

    return _process_dataclass


@_register(
    plan_repr=(
        "Plans(tup=(CopyPlan(fields=('id', 'content', 'priority', 'status')), EqPlan(fields=('id', 'content', 'priority"
        "', 'status')), FrozenPlan(fields=('id', 'content', 'priority', 'status'), allow_dynamic_dunder_attrs=False), H"
        "ashPlan(action='add', fields=('id', 'content', 'priority', 'status'), cache=False), InitPlan(fields=(InitPlan."
        "Field(name='id', annotation=OpRef(name='init.fields.0.annotation'), default=OpRef(name='init.fields.0.default'"
        "), default_factory=None, init=True, override=False, field_type=FieldType.INSTANCE, coerce=None, validate=None,"
        " check_type=None), InitPlan.Field(name='content', annotation=OpRef(name='init.fields.1.annotation'), default=N"
        "one, default_factory=None, init=True, override=False, field_type=FieldType.INSTANCE, coerce=None, validate=Non"
        "e, check_type=None), InitPlan.Field(name='priority', annotation=OpRef(name='init.fields.2.annotation'), defaul"
        "t=None, default_factory=None, init=True, override=False, field_type=FieldType.INSTANCE, coerce=None, validate="
        "None, check_type=None), InitPlan.Field(name='status', annotation=OpRef(name='init.fields.3.annotation'), defau"
        "lt=None, default_factory=None, init=True, override=False, field_type=FieldType.INSTANCE, coerce=None, validate"
        "=None, check_type=None)), self_param='self', std_params=(), kw_only_params=('id', 'content', 'priority', 'stat"
        "us'), frozen=True, slots=False, post_init_params=None, init_fns=(), validate_fns=()), ReprPlan(fields=(ReprPla"
        "n.Field(name='id', kw_only=True, fn=None), ReprPlan.Field(name='content', kw_only=True, fn=None), ReprPlan.Fie"
        "ld(name='priority', kw_only=True, fn=None), ReprPlan.Field(name='status', kw_only=True, fn=None)), id=False, t"
        "erse=False, default_fn=None)))"
    ),
    plan_repr_sha1='90327d7499fc348378f37c29946fda05f802116e',
    op_ref_idents=(
        '__dataclass__init__fields__0__annotation',
        '__dataclass__init__fields__0__default',
        '__dataclass__init__fields__1__annotation',
        '__dataclass__init__fields__2__annotation',
        '__dataclass__init__fields__3__annotation',
    ),
    cls_names=(
        ('ommlds.minichain.modules.todo.types', 'TodoItem'),
    ),
)
def _process_dataclass__90327d7499fc348378f37c29946fda05f802116e():
    def _process_dataclass(
        *,
        __class__,
        __dataclass__init__fields__0__annotation,
        __dataclass__init__fields__0__default,
        __dataclass__init__fields__1__annotation,
        __dataclass__init__fields__2__annotation,
        __dataclass__init__fields__3__annotation,
        __dataclass__FieldFnValidationError,  # noqa
        __dataclass__FieldTypeValidationError,  # noqa
        __dataclass__FnValidationError,  # noqa
        __dataclass__FrozenInstanceError=dataclasses.FrozenInstanceError,  # noqa
        __dataclass__FunctionType=types.FunctionType,  # noqa
        __dataclass__HAS_DEFAULT_FACTORY=dataclasses._HAS_DEFAULT_FACTORY,  # noqa
        __dataclass__MISSING=dataclasses.MISSING,  # noqa
        __dataclass__None=None,  # noqa
        __dataclass__TypeError=TypeError,  # noqa
        __dataclass___recursive_repr=reprlib.recursive_repr,  # noqa
        __dataclass__isinstance=isinstance,  # noqa
        __dataclass__object_setattr=object.__setattr__,  # noqa
        __dataclass__property=property,  # noqa
    ):
        def __copy__(self):
            if self.__class__ is not __class__:
                raise TypeError(self)
            return __class__(  # noqa
                id=self.id,
                content=self.content,
                priority=self.priority,
                status=self.status,
            )

        __copy__.__qualname__ = f"{__class__.__qualname__}.__copy__"
        if '__copy__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __copy__ in class {__class__.__name__}")
        setattr(__class__, '__copy__', __copy__)

        def __eq__(self, other):
            if self is other:
                return True
            if self.__class__ is not other.__class__:
                return NotImplemented
            return (
                self.id == other.id and
                self.content == other.content and
                self.priority == other.priority and
                self.status == other.status
            )

        __eq__.__qualname__ = f"{__class__.__qualname__}.__eq__"
        if '__eq__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __eq__ in class {__class__.__name__}")
        setattr(__class__, '__eq__', __eq__)

        __dataclass___setattr_frozen_fields = {
            'id',
            'content',
            'priority',
            'status',
        }

        def __setattr__(self, name, value):
            if (
                type(self) is __class__
                or name in __dataclass___setattr_frozen_fields
            ):
                raise __dataclass__FrozenInstanceError(f"cannot assign to field {name!r}")
            super(__class__, self).__setattr__(name, value)

        __setattr__.__qualname__ = f"{__class__.__qualname__}.__setattr__"
        if '__setattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __setattr__ in class {__class__.__name__}")
        setattr(__class__, '__setattr__', __setattr__)

        __dataclass___delattr_frozen_fields = {
            'id',
            'content',
            'priority',
            'status',
        }

        def __delattr__(self, name):
            if (
                type(self) is __class__
                or name in __dataclass___delattr_frozen_fields
            ):
                raise __dataclass__FrozenInstanceError(f"cannot delete field {name!r}")
            super(__class__, self).__delattr__(name)

        __delattr__.__qualname__ = f"{__class__.__qualname__}.__delattr__"
        if '__delattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __delattr__ in class {__class__.__name__}")
        setattr(__class__, '__delattr__', __delattr__)

        def __hash__(self):
            return hash((
                self.id,
                self.content,
                self.priority,
                self.status,
            ))

        __hash__.__qualname__ = f"{__class__.__qualname__}.__hash__"
        setattr(__class__, '__hash__', __hash__)

        def __init__(
            self,
            *,
            id: __dataclass__init__fields__0__annotation = __dataclass__init__fields__0__default,
            content: __dataclass__init__fields__1__annotation,
            priority: __dataclass__init__fields__2__annotation,
            status: __dataclass__init__fields__3__annotation,
        ) -> __dataclass__None:
            __dataclass__object_setattr(self, 'id', id)
            __dataclass__object_setattr(self, 'content', content)
            __dataclass__object_setattr(self, 'priority', priority)
            __dataclass__object_setattr(self, 'status', status)

        __init__.__qualname__ = f"{__class__.__qualname__}.__init__"
        if '__init__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __init__ in class {__class__.__name__}")
        setattr(__class__, '__init__', __init__)

        @__dataclass___recursive_repr()
        def __repr__(self):
            parts = []
            parts.append(f"id={self.id!r}")
            parts.append(f"content={self.content!r}")
            parts.append(f"priority={self.priority!r}")
            parts.append(f"status={self.status!r}")
            return (
                f"{self.__class__.__qualname__}("
                f"{', '.join(parts)}"
                f")"
            )

        __repr__.__qualname__ = f"{__class__.__qualname__}.__repr__"
        if '__repr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __repr__ in class {__class__.__name__}")
        setattr(__class__, '__repr__', __repr__)

    return _process_dataclass


@_register(
    plan_repr=(
        "Plans(tup=(CopyPlan(fields=('max_fetch_chars', 'enable_search', 'max_search_results')), EqPlan(fields=('max_fe"
        "tch_chars', 'enable_search', 'max_search_results')), FrozenPlan(fields=('max_fetch_chars', 'enable_search', 'm"
        "ax_search_results'), allow_dynamic_dunder_attrs=False), HashPlan(action='add', fields=('max_fetch_chars', 'ena"
        "ble_search', 'max_search_results'), cache=False), InitPlan(fields=(InitPlan.Field(name='max_fetch_chars', anno"
        "tation=OpRef(name='init.fields.0.annotation'), default=OpRef(name='init.fields.0.default'), default_factory=No"
        "ne, init=True, override=False, field_type=FieldType.INSTANCE, coerce=None, validate=None, check_type=None), In"
        "itPlan.Field(name='enable_search', annotation=OpRef(name='init.fields.1.annotation'), default=OpRef(name='init"
        ".fields.1.default'), default_factory=None, init=True, override=False, field_type=FieldType.INSTANCE, coerce=No"
        "ne, validate=None, check_type=None), InitPlan.Field(name='max_search_results', annotation=OpRef(name='init.fie"
        "lds.2.annotation'), default=OpRef(name='init.fields.2.default'), default_factory=None, init=True, override=Fal"
        "se, field_type=FieldType.INSTANCE, coerce=None, validate=None, check_type=None)), self_param='self', std_param"
        "s=(), kw_only_params=('max_fetch_chars', 'enable_search', 'max_search_results'), frozen=True, slots=False, pos"
        "t_init_params=None, init_fns=(), validate_fns=()), ReprPlan(fields=(ReprPlan.Field(name='max_fetch_chars', kw_"
        "only=True, fn=None), ReprPlan.Field(name='enable_search', kw_only=True, fn=None), ReprPlan.Field(name='max_sea"
        "rch_results', kw_only=True, fn=None)), id=False, terse=False, default_fn=None)))"
    ),
    plan_repr_sha1='869bfeeb53e443cc395a786c230df362a459ce09',
    op_ref_idents=(
        '__dataclass__init__fields__0__annotation',
        '__dataclass__init__fields__0__default',
        '__dataclass__init__fields__1__annotation',
        '__dataclass__init__fields__1__default',
        '__dataclass__init__fields__2__annotation',
        '__dataclass__init__fields__2__default',
    ),
    cls_names=(
        ('ommlds.minichain.modules.web.configs', 'WebConfig'),
    ),
)
def _process_dataclass__869bfeeb53e443cc395a786c230df362a459ce09():
    def _process_dataclass(
        *,
        __class__,
        __dataclass__init__fields__0__annotation,
        __dataclass__init__fields__0__default,
        __dataclass__init__fields__1__annotation,
        __dataclass__init__fields__1__default,
        __dataclass__init__fields__2__annotation,
        __dataclass__init__fields__2__default,
        __dataclass__FieldFnValidationError,  # noqa
        __dataclass__FieldTypeValidationError,  # noqa
        __dataclass__FnValidationError,  # noqa
        __dataclass__FrozenInstanceError=dataclasses.FrozenInstanceError,  # noqa
        __dataclass__FunctionType=types.FunctionType,  # noqa
        __dataclass__HAS_DEFAULT_FACTORY=dataclasses._HAS_DEFAULT_FACTORY,  # noqa
        __dataclass__MISSING=dataclasses.MISSING,  # noqa
        __dataclass__None=None,  # noqa
        __dataclass__TypeError=TypeError,  # noqa
        __dataclass___recursive_repr=reprlib.recursive_repr,  # noqa
        __dataclass__isinstance=isinstance,  # noqa
        __dataclass__object_setattr=object.__setattr__,  # noqa
        __dataclass__property=property,  # noqa
    ):
        def __copy__(self):
            if self.__class__ is not __class__:
                raise TypeError(self)
            return __class__(  # noqa
                max_fetch_chars=self.max_fetch_chars,
                enable_search=self.enable_search,
                max_search_results=self.max_search_results,
            )

        __copy__.__qualname__ = f"{__class__.__qualname__}.__copy__"
        if '__copy__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __copy__ in class {__class__.__name__}")
        setattr(__class__, '__copy__', __copy__)

        def __eq__(self, other):
            if self is other:
                return True
            if self.__class__ is not other.__class__:
                return NotImplemented
            return (
                self.max_fetch_chars == other.max_fetch_chars and
                self.enable_search == other.enable_search and
                self.max_search_results == other.max_search_results
            )

        __eq__.__qualname__ = f"{__class__.__qualname__}.__eq__"
        if '__eq__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __eq__ in class {__class__.__name__}")
        setattr(__class__, '__eq__', __eq__)

        __dataclass___setattr_frozen_fields = {
            'max_fetch_chars',
            'enable_search',
            'max_search_results',
        }

        def __setattr__(self, name, value):
            if (
                type(self) is __class__
                or name in __dataclass___setattr_frozen_fields
            ):
                raise __dataclass__FrozenInstanceError(f"cannot assign to field {name!r}")
            super(__class__, self).__setattr__(name, value)

        __setattr__.__qualname__ = f"{__class__.__qualname__}.__setattr__"
        if '__setattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __setattr__ in class {__class__.__name__}")
        setattr(__class__, '__setattr__', __setattr__)

        __dataclass___delattr_frozen_fields = {
            'max_fetch_chars',
            'enable_search',
            'max_search_results',
        }

        def __delattr__(self, name):
            if (
                type(self) is __class__
                or name in __dataclass___delattr_frozen_fields
            ):
                raise __dataclass__FrozenInstanceError(f"cannot delete field {name!r}")
            super(__class__, self).__delattr__(name)

        __delattr__.__qualname__ = f"{__class__.__qualname__}.__delattr__"
        if '__delattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __delattr__ in class {__class__.__name__}")
        setattr(__class__, '__delattr__', __delattr__)

        def __hash__(self):
            return hash((
                self.max_fetch_chars,
                self.enable_search,
                self.max_search_results,
            ))

        __hash__.__qualname__ = f"{__class__.__qualname__}.__hash__"
        setattr(__class__, '__hash__', __hash__)

        def __init__(
            self,
            *,
            max_fetch_chars: __dataclass__init__fields__0__annotation = __dataclass__init__fields__0__default,
            enable_search: __dataclass__init__fields__1__annotation = __dataclass__init__fields__1__default,
            max_search_results: __dataclass__init__fields__2__annotation = __dataclass__init__fields__2__default,
        ) -> __dataclass__None:
            __dataclass__object_setattr(self, 'max_fetch_chars', max_fetch_chars)
            __dataclass__object_setattr(self, 'enable_search', enable_search)
            __dataclass__object_setattr(self, 'max_search_results', max_search_results)

        __init__.__qualname__ = f"{__class__.__qualname__}.__init__"
        if '__init__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __init__ in class {__class__.__name__}")
        setattr(__class__, '__init__', __init__)

        @__dataclass___recursive_repr()
        def __repr__(self):
            parts = []
            parts.append(f"max_fetch_chars={self.max_fetch_chars!r}")
            parts.append(f"enable_search={self.enable_search!r}")
            parts.append(f"max_search_results={self.max_search_results!r}")
            return (
                f"{self.__class__.__qualname__}("
                f"{', '.join(parts)}"
                f")"
            )

        __repr__.__qualname__ = f"{__class__.__qualname__}.__repr__"
        if '__repr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __repr__ in class {__class__.__name__}")
        setattr(__class__, '__repr__', __repr__)

    return _process_dataclass


@_register(
    plan_repr=(
        "Plans(tup=(CopyPlan(fields=('url', 'status', 'body')), EqPlan(fields=('url', 'status', 'body')), FrozenPlan(fi"
        "elds=('url', 'status', 'body'), allow_dynamic_dunder_attrs=False), HashPlan(action='add', fields=('url', 'stat"
        "us', 'body'), cache=False), InitPlan(fields=(InitPlan.Field(name='url', annotation=OpRef(name='init.fields.0.a"
        "nnotation'), default=None, default_factory=None, init=True, override=False, field_type=FieldType.INSTANCE, coe"
        "rce=None, validate=None, check_type=None), InitPlan.Field(name='status', annotation=OpRef(name='init.fields.1."
        "annotation'), default=None, default_factory=None, init=True, override=False, field_type=FieldType.INSTANCE, co"
        "erce=None, validate=None, check_type=None), InitPlan.Field(name='body', annotation=OpRef(name='init.fields.2.a"
        "nnotation'), default=None, default_factory=None, init=True, override=False, field_type=FieldType.INSTANCE, coe"
        "rce=None, validate=None, check_type=None)), self_param='self', std_params=('url', 'status', 'body'), kw_only_p"
        "arams=(), frozen=True, slots=False, post_init_params=None, init_fns=(), validate_fns=()), ReprPlan(fields=(Rep"
        "rPlan.Field(name='url', kw_only=False, fn=None), ReprPlan.Field(name='status', kw_only=False, fn=None), ReprPl"
        "an.Field(name='body', kw_only=False, fn=None)), id=False, terse=False, default_fn=None)))"
    ),
    plan_repr_sha1='e0a89cc4dcbb47d112ac3a3f9474c89c1e0fceeb',
    op_ref_idents=(
        '__dataclass__init__fields__0__annotation',
        '__dataclass__init__fields__1__annotation',
        '__dataclass__init__fields__2__annotation',
    ),
    cls_names=(
        ('ommlds.minichain.modules.web.fetching', 'FetchedPage'),
    ),
)
def _process_dataclass__e0a89cc4dcbb47d112ac3a3f9474c89c1e0fceeb():
    def _process_dataclass(
        *,
        __class__,
        __dataclass__init__fields__0__annotation,
        __dataclass__init__fields__1__annotation,
        __dataclass__init__fields__2__annotation,
        __dataclass__FieldFnValidationError,  # noqa
        __dataclass__FieldTypeValidationError,  # noqa
        __dataclass__FnValidationError,  # noqa
        __dataclass__FrozenInstanceError=dataclasses.FrozenInstanceError,  # noqa
        __dataclass__FunctionType=types.FunctionType,  # noqa
        __dataclass__HAS_DEFAULT_FACTORY=dataclasses._HAS_DEFAULT_FACTORY,  # noqa
        __dataclass__MISSING=dataclasses.MISSING,  # noqa
        __dataclass__None=None,  # noqa
        __dataclass__TypeError=TypeError,  # noqa
        __dataclass___recursive_repr=reprlib.recursive_repr,  # noqa
        __dataclass__isinstance=isinstance,  # noqa
        __dataclass__object_setattr=object.__setattr__,  # noqa
        __dataclass__property=property,  # noqa
    ):
        def __copy__(self):
            if self.__class__ is not __class__:
                raise TypeError(self)
            return __class__(  # noqa
                url=self.url,
                status=self.status,
                body=self.body,
            )

        __copy__.__qualname__ = f"{__class__.__qualname__}.__copy__"
        if '__copy__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __copy__ in class {__class__.__name__}")
        setattr(__class__, '__copy__', __copy__)

        def __eq__(self, other):
            if self is other:
                return True
            if self.__class__ is not other.__class__:
                return NotImplemented
            return (
                self.url == other.url and
                self.status == other.status and
                self.body == other.body
            )

        __eq__.__qualname__ = f"{__class__.__qualname__}.__eq__"
        if '__eq__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __eq__ in class {__class__.__name__}")
        setattr(__class__, '__eq__', __eq__)

        __dataclass___setattr_frozen_fields = {
            'url',
            'status',
            'body',
        }

        def __setattr__(self, name, value):
            if (
                type(self) is __class__
                or name in __dataclass___setattr_frozen_fields
            ):
                raise __dataclass__FrozenInstanceError(f"cannot assign to field {name!r}")
            super(__class__, self).__setattr__(name, value)

        __setattr__.__qualname__ = f"{__class__.__qualname__}.__setattr__"
        if '__setattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __setattr__ in class {__class__.__name__}")
        setattr(__class__, '__setattr__', __setattr__)

        __dataclass___delattr_frozen_fields = {
            'url',
            'status',
            'body',
        }

        def __delattr__(self, name):
            if (
                type(self) is __class__
                or name in __dataclass___delattr_frozen_fields
            ):
                raise __dataclass__FrozenInstanceError(f"cannot delete field {name!r}")
            super(__class__, self).__delattr__(name)

        __delattr__.__qualname__ = f"{__class__.__qualname__}.__delattr__"
        if '__delattr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __delattr__ in class {__class__.__name__}")
        setattr(__class__, '__delattr__', __delattr__)

        def __hash__(self):
            return hash((
                self.url,
                self.status,
                self.body,
            ))

        __hash__.__qualname__ = f"{__class__.__qualname__}.__hash__"
        setattr(__class__, '__hash__', __hash__)

        def __init__(
            self,
            url: __dataclass__init__fields__0__annotation,
            status: __dataclass__init__fields__1__annotation,
            body: __dataclass__init__fields__2__annotation,
        ) -> __dataclass__None:
            __dataclass__object_setattr(self, 'url', url)
            __dataclass__object_setattr(self, 'status', status)
            __dataclass__object_setattr(self, 'body', body)

        __init__.__qualname__ = f"{__class__.__qualname__}.__init__"
        if '__init__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __init__ in class {__class__.__name__}")
        setattr(__class__, '__init__', __init__)

        @__dataclass___recursive_repr()
        def __repr__(self):
            parts = []
            parts.append(f"url={self.url!r}")
            parts.append(f"status={self.status!r}")
            parts.append(f"body={self.body!r}")
            return (
                f"{self.__class__.__qualname__}("
                f"{', '.join(parts)}"
                f")"
            )

        __repr__.__qualname__ = f"{__class__.__qualname__}.__repr__"
        if '__repr__' in __class__.__dict__:
            raise __dataclass__TypeError(f"Cannot overwrite attribute __repr__ in class {__class__.__name__}")
        setattr(__class__, '__repr__', __repr__)

    return _process_dataclass
