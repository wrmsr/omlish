...
if DEBUG >= 2:
    ...
    def print_globalcounters():
        if GlobalCounters.time_sum_s == 0:
            return
        print(
            f"avg: {GlobalCounters.global_ops*1e-9/GlobalCounters.time_sum_s:8.2f} GFLOPS {GlobalCounters.global_mem*1e-9/GlobalCounters.time_sum_s:8.2f} GB/s",  # noqa: E501
...
def log_schedule_item(si: ScheduleItem):
    ...
    if GRAPH:
        ...
        G.nodes[nm(si.out)]["label"] = (
            ...
            + (f"\n{si.ast.op}" if si.ast.op in LoadOps or optype is BufferOps else "")
