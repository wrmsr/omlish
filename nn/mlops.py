...
class Log(Function):
    def forward(self, x: LazyBuffer) -> LazyBuffer:
        self.x = x
        return (
            x.cast(ftype := least_upper_dtype(x.dtype, dtypes.float))
            .e(UnaryOps.LOG2)
            .e(BinaryOps.MUL, x.cast(ftype).const(math.log(2)))
        )
...
class Exp(Function):
    def forward(self, x: LazyBuffer) -> LazyBuffer:
        self.ret = (
            x.cast(ftype := least_upper_dtype(x.dtype, dtypes.float))
            .e(BinaryOps.MUL, x.cast(ftype).const(1 / math.log(2)))
            .e(UnaryOps.EXP2)
        )
        return self.ret
    ...
    def forward(self, x: LazyBuffer) -> LazyBuffer:
        self.ret = (
            ...
            .e(
                ...
                .e(
                    BinaryOps.ADD,
                    x.e(BinaryOps.MUL, x.cast(ftype).const(-1 / math.log(2))).e(
                        UnaryOps.EXP2
                    ),
                ),
            )
