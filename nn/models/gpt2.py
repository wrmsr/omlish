...
class Attention:
    ...
    def __call__(
            self,
            x: Tensor,
            cache_k: ta.Optional[Tensor],
            cache_v: ta.Optional[Tensor],
            start_pos: int,
...
class Transformer:
    ...
    def postprocess(self, x, temperature: ta.Optional[float]):
    ...
    def run_all_layers(self, tokens: Tensor, pos: Tensor, start_pos: int, temperature: float, **kv_cache):
        ...
        for i, hi in enumerate(self.h):
            h, kv_cache[f'cache_k{i}'], kv_cache[f'cache_v{i}'] = hi(
                h,
                kv_cache[f'cache_k{i}'],
                kv_cache[f'cache_v{i}'],
                start_pos=start_pos,
                mask=None,
            )
    ...
    def __call__(self, tokens: Tensor, start_pos: int, temperature: ta.Optional[float] = None):
        ...
        if seqlen == 1 and start_pos > 0 and getenv("JIT"):
            ...
            for i, hi in enumerate(self.h):
                h, self.kv_caches[f'cache_k{i}'], self.kv_caches[f'cache_v{i}'] = hi(
                    h,
                    self.kv_caches[f'cache_k{i}'],
                    self.kv_caches[f'cache_v{i}'],
                    start_pos=start_pos,
                    mask=mask,
                )
...
class GPT2:
    ...
    def build(model_size="gpt2"):
        import tiktoken
        from ..nn.state import torch_load, load_state_dict, get_state_dict
