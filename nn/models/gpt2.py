...
from ..helpers import fetch_as_file
...
class Attention:
    ...
    def __call__(self, x: Tensor, start_pos: Variable, mask: ta.Optional[Tensor]) -> Tensor:
        if mask is not None:
        ...
        xq, xk, xv = [
            xqkv.\
                shrink((None, None, (i * self.dim, (i + 1) * self.dim))).\
                reshape(xqkv.shape[0], xqkv.shape[1], self.n_heads, self.head_dim)
            for i in range(3)]
        bsz, seqlen, n_heads, head_dim = xq.shape

        # create kv cache
        if not hasattr(self, "cache_k"):
            self.cache_k = Tensor.zeros(bsz, MAX_CONTEXT, self.n_heads, self.head_dim)
            self.cache_v = Tensor.zeros(bsz, MAX_CONTEXT, self.n_heads, self.head_dim)
...
class TransformerBlock:
    def __init__(self, dim, n_heads, norm_eps) -> None:
        super().__init__()
    ...
    def __call__(self, x: Tensor, start_pos: Variable, mask: ta.Optional[Tensor]):
...
class Transformer:
    ...
    def forward(self, tokens: Tensor, start_pos: Variable, temperature: float = 0.0):
        ...
        mask = Tensor.full((1, 1, seqlen, start_pos.val + seqlen), float("-inf")).triu(
            start_pos.val + 1).realize() if seqlen > 1 else None
...
class GPT2:
    ...
    def greedy_until(self, prompt: str, max_length: int, temperature: float, timing: bool = False):
        ...
        for _ in tqdm.trange(max_length, disable=(timing == True)):
