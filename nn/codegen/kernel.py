...
class TensorCore:
    ...
    thread_local_aliases: List[
        List[List[int]]
    ]  # a list of [threads_1, ..., threads_n, upcast_1(unrolled), upcast_2(upcast)] defining the alias (-1 is upcast, 1-n is warp threads) for each TC dim # noqa: E501
...
class Kernel:
    ...
    def limit_dims_to_max(self, global_max: List[int], local_max: List[int]):
        ...
        if global_dims > 0:
            if global_max:
                ...
                ), f"device max allocation {max(self.full_shape[:global_dims])} exceeds global dim maximum {max(global_max)}"  # noqa: E501
    ...
    def apply_opt(self, opt: Opt):
        ...
        if opt.op == OptOps.LOCAL:  # cyan
            ...
            ), "invalid upcast mid reduce"  # noqa: E501
    ...
    def required_optimizations(self):
        if self.bufs[0].dtype.__class__ is ImageDType:
            unit_stride_axes_mul_4 = [
                i
                for i in self.sts[0].unit_stride_axes(ignore_valid=True)
                if self.sts[0].shape[i] % 4 == 0
            ]
            assert (
                len(unit_stride_axes_mul_4) >= 1
            ), f"needs a unit stride axis in {self.bufs[0]}"
            if (
                len(unit_stride_axes_mul_4)
                and all(
                    x < (self.shape_len - self.upcasted) for x in unit_stride_axes_mul_4
                )
