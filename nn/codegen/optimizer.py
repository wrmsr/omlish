...
from ..codegen.kernel import tensor_cores
...
class OptimizedKernel(Kernel):
    ...
    #         and ta.cast(LazyOp, self.reduceop.src[0].src[0].src[0]).arg.dtype == dtypes.half
    #         and ta.cast(LazyOp, self.reduceop.src[0].src[0].src[1]).arg.dtype == dtypes.half
    #     ):
    ...
    #             if s == 0 and self.full_shape[i] % 8 == 0 and i < self.first_reduce
    #         ]
    #         if axis_buf0 and axis_buf1 and self.full_shape[
    #             self.first_reduce] % 8 == 0 and self.shape_len - self.first_reduce == 1:
    #             if DEBUG >= 3:
    ...
    #             # alias buffer
    #             alias_pattern = (
    #                     [0] * (self.global_dims + self.exclude_local_upcast) +
    #                     [2] * (self.local_dims - self.exclude_local_upcast) +
    #                     [0] * (self.shape_len - self.upcasted - self.first_reduce) +
    #                     [1, 1] +
    #                     [3] * (self.upcasted - 2)
    #             )
    ...
    def apply_tensor_cores(self, use_tensor_cores=1, extra_opts: ta.Optional[list[Opt]] = None):
        if (
            ...
            for tc in tensor_cores[self.opts.device]:
                ...
                if not (isinstance(mul_op, LazyOp) and isinstance(mul_op, ops.Mul)):
                    continue
                if not (isinstance(mul_op.src[0], LazyOp) and isinstance(mul_op.src[0], ops.Mem) and mul_op.src[0].arg.dtype == tc.dtype_in):
                    continue
                if not (isinstance(mul_op.src[1], LazyOp) and isinstance(mul_op.src[1], ops.Mem) and mul_op.src[1].arg.dtype == tc.dtype_in):
                    continue
                buf0, buf1 = self.bufs.index(ta.cast(LazyOp, mul_op.src[0].arg)), self.bufs.index(ta.cast(LazyOp, mul_op.src[1].arg))
    ...
    def apply_opt(self, opt: Opt):
        ...
        axis = opt.axis + (
            self.first_reduce
            if opt.op == OptOps.UNROLL else (
                self.first_reduce + len(self.group_for_reduce)
                if opt.op == OptOps.GROUP or opt.op == OptOps.GROUPTOP else
                0
            )
        )
        ...
        if opt.op == OptOps.LOCAL:  # cyan
            ...
            assert (
                axis >= self.first_reduce + len(self.group_for_reduce) and axis < self.shape_len - self.upcasted
            ...
            assert self.bufs[0].dtype.name.startswith('image') and not self.float4_axis(
                0) and self.group_for_reduce and self.first_reduce <= 2 and prod(
                self.sts[0].shape) > 1, "invalid upcast mid reduce"
            ...
            if (
                ...
                for global_idx in range(self.global_dims):
                    if (
                            ...
                            and self.full_shape[global_idx] % (MV_BLOCKSIZE * MV_ROWS_PER_THREAD) == 0
                    ):
                        if DEBUG >= 3:
                            print(
                                f"MATVEC: full_shape={self.full_shape} "
