...
class CStyleLanguage(NamedTuple):
    ...
    def render_const(self, x: Union[float, int], var_dtype) -> str:
        if math.isnan(x):
            ...
            val = f"{float(x)}f" if dtypes.is_float(var_dtype) else f"{int(x)}"
        return (
    ...
    def render_store(
        ...
        if (
            self.uses_vload
            and buf_dtype == dtypes.float16
            and var_dtype != dtypes.float16
        ):
    ...
    for u in uops:
        ...
        if uop == UOps.LOOP:
            ...
            if (
                ...
                val = lang.code_for_op[args](
                    *[
                        lang.render_cast([r[x]], dtype) if x.dtype != dtype else r[x]
                        for x in vin
                    ]
                )
            else:
