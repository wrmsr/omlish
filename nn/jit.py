...
def get_input_replace(
    jit_cache: List[JitItem], input_rawbuffers: List[RawBuffer]
    ...
    assert len(set(input_replace.values())) == len(
        input_rawbuffers
...
class TinyJit(Generic[ReturnType]):
    ...
    def __call__(self, *args, **kwargs) -> ReturnType:
        ...
        input_rawbuffers: List[RawBuffer] = [
            cast(RawBuffer, v.lazydata.realized) for v in input_tensors.values()
        ]
        ...
        if self.cnt >= 2:
            ...
            for ji in self.jit_cache:
                ji.prg(
                    cast(List[RawBuffer], ji.rawbufs),
                    var_vals,
                    wait=DEBUG >= 2,
                    jit=True,
                )
                ...
                try:
                    self.jit_cache = [
                        JitItem(
                            ...
                            cast(List[Optional[RawBuffer]], input_rawbuffers),
                        )
                    ]
...
class PlaceHolder:
    def __init__(self, buf: RawBuffer):
        self.size, self.dtype, self._device, self.ref, self.buftype, self.bufid = (
            buf.size,
            buf.dtype,
...
class _CacheCollector:
    def __init__(self):
        self.cache: Optional[
            List[Tuple[JITRunner, List[Union[RawBuffer, PlaceHolder]]]]
        ] = None

    def start(self, var_vals: Optional[Dict[Variable, int]] = None):
        self.cache = []
        self.placeholders: WeakKeyDictionary[
            RawBuffer, PlaceHolder
        ] = WeakKeyDictionary()
