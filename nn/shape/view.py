...
from ..shape.symbolic import Set
...
def _merge_dims(
...
) -> Tuple[Tuple[int, int, int], ...]:  # noqa: E501
    ...
    state = (
        1
        if mask and strides[0] == 0 and shape[0] != 1 and mask[0][1] - mask[0][0] == 1
        else 0
    )
    for i, (sh, st) in enumerate(zip(shape[1:], strides[1:]), start=1):
        ...
        if state == 1 or ret[-1][1] == sh * st:  # mergeable
            ret[-1] = (
                ret[-1][0] * sh,
                st,
                (sh if state == 1 else ret[-1][2] * sh) if st else 0,
            )
        else:
            ret.append((sh, st, sh if st else 0))  # begin new
        # merging ends with either non-zero strided dim or zero strided dim with mask range > 1
        state = (
            1
            if (st == 0 and mask and mask[i][1] - mask[i][0] == 1)
            else (2 if state != 0 else 0)
        )
    return tuple(ret)
...
@functools.lru_cache(maxsize=None)
def _reshape_mask(
    view: View, new_shape: Tuple[sint, ...]
) -> Tuple[Optional[Tuple[Tuple[sint, sint], ...]], Optional[Tuple[sint, ...]], bool]:
    if view.mask is None:
        return view.mask, None, False
    if any(not isinstance(m[0], int) or not isinstance(m[1], int) for m in view.mask):
        return view.mask, None, True
    new_mask: List[Tuple[int, int]] = []

    r_masks, r_shape, r_new_shape = (
        reversed(view.mask),
        reversed(view.shape),
        reversed(new_shape),
    )
    curr_stride, off, offsets, old_dim, new_dim, mask = (
        1,
        0,
        [],
        next(r_shape, 1),
    ...
    if mask[1] - mask[0] < 1:
        return ((0, 0),) * len(new_shape), None, False  # invalid mask
    ...
    while len(new_mask) < len(new_shape):
        ...
        if old_dim >= next_stride:  # need to split mask.
            ...
            if (
                ...
                curr_stride, off, old_dim, new_dim, mask = (
                    1,
                    0,
                    next(r_shape, 1),
                    next(r_new_shape, 1),
                ...
                if (
                    l % next_stride != 0 or r % next_stride != 0
                ) and l // next_stride != (r - 1) // next_stride:
                    return view.mask, None, True
                new_mask.append(
                    (
                        l % next_stride // curr_stride,
                        (r - 1) % next_stride // curr_stride + 1,
                    )
                )
                curr_stride, new_dim = next_stride, next(
                    r_new_shape, 1
                )  # need to get mask for next dimension

        else:
            next_mask = next(r_masks, (0, 1))
            # combine if the mask can unfold continuously
            if mask != (0, old_dim) and next_mask[1] - next_mask[0] != 1:
                return view.mask, None, True
            if (
                next_mask != (0, 1)
                and mask != (0, 1)
                and (next_mask[1] - next_mask[0] == 1)
            ):
                off += next_mask[0] * old_dim
            mask, old_dim = (
                next_mask[0] * old_dim + l,
                (next_mask[1] - 1) * old_dim + r,
    ...
    for (
        ...
        r_masks
    ):  # if the old shape has leading 1s, need to make sure their mask is (0,1)
        if mask != (0, 1):
            return ((0, 0),) * len(new_shape), None, False
...
class View:
    ...
    def create(
        ...
        contiguous = (
            offset == 0 and mask is None and strides == strides_for_shape(shape)
        )
        return View(shape, strides, offset, mask, contiguous)

    def vars(self) -> Set[Variable]:
        flatten_mask = (
            tuple(x for m in self.mask for x in m) if self.mask is not None else tuple()
        )
    ...
    def unbind(self) -> View:
        unbound_vars: Dict[VariableOrNum, Node] = {
            v: v.unbind()[0] for v in self.vars() if v.val is not None
        }
        new_shape = tuple(
            [
                s if isinstance(s, int) else s.substitute(unbound_vars)
                for s in self.shape
            ]
        )
        new_strides = tuple(
        ...
        new_offset = (
            ...
            else self.offset.substitute(unbound_vars)
        )
        new_mask = (
            tuple(
                (
    ...
    def __unsafe_resize(self, arg: Tuple[Tuple[sint, sint], ...], mask=None) -> View:
        ...
        if self.mask:
            ...
            nmask = tuple(
                [
                    (max(0, min(mx - ax, ay - ax)), max(0, min(my - ax, ay - ax)))
    ...
    def shrink(self, arg: Tuple[Tuple[sint, sint], ...]) -> View:
        assert all((0 <= b <= e <= s) for s, (b, e) in zip(self.shape, arg)) and len(
            arg
        ) == len(self.shape), f"invalid shrink {arg} for {self.shape}"
    ...
    def expand(self, new_shape: Tuple[sint, ...]) -> View:
        if len(new_shape) != len(self.shape):
            raise ValueError(
                f"expand arg {new_shape=} must have same number of dimensions as shape {self.shape=}"
            )
        if 0 in self.shape:
            assert all(
                (s == x == 0) or (s > 0 and (x % s) == 0)
                for s, x in zip(self.shape, new_shape)
    ...
    def reshape(self, new_shape: Tuple[sint, ...]) -> Optional[View]:
        ...
        assert all(
            x >= 0 for x in new_shape
        ), f"shape can't contain negative numbers {new_shape}"
        if 0 in self.shape:
        ...
        if all_int(self.shape):
            assert all(
                isinstance(s, (int, Variable)) for s in new_shape
            ), f"{self.shape=} -> {new_shape=} contains non (int, Variable) dim"
            if prod(self.shape) != prod(
                [s if isinstance(s, int) else cast(Variable, s).val for s in new_shape]
            ):
                raise ValueError(
                    f"size mismatched, can't reshape {self.shape=} -> {new_shape=}"
                )

        if new_shape == () and self.mask and any(mx == my for (mx, my) in self.mask):
            return None
        ...
        for merged_dim, s, real_dim in reversed(
            _merge_dims(self.shape, self.strides, self.mask)
        ):
            acc, new_stride = 1, s
            while (
                acc <= merged_dim
                and acc != merged_dim
                and (new_dim := next(r_new_shape, None))
            ):
            ...
            strides += [
                0,
            ] * (len(new_shape) - len(strides))
            mask, off_mask, extra = _reshape_mask(self, new_shape)
            total_offset = (
                sum([off * s for off, s in zip(off_mask, strides)]) if off_mask else 0
            )
            if not extra:
                return View.create(
                    new_shape,
