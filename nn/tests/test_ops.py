...
def helper_test_op(
    ...
    def compare(s, x, y, atol, rtol):
        ...
        assert (
            x.shape == y.shape
        ), f"shape mismatch: tinygrad={x.shape} | torch={y.shape}"
        try:
...
def prepare_test_op(a, b, shps, vals, forward_only=False):
    ...
    tst = [
        Tensor(
            x.detach().numpy(), requires_grad=(not forward_only and not FORWARD_ONLY)
        )
        for x in ts
    ]
    return ts, tst
...
class TestOps(unittest.TestCase):
    def helper_test_exception(
        ...
        if getenv("CUDACPU"):
            self.skipTest("helper_test_exception fails in CUDACPU")
    ...
    def test_zeros(self):
        helper_test_op(
            [],
            lambda: torch.zeros(45, 65),
            lambda: Tensor.zeros(45, 65),
            forward_only=True,
        )
        helper_test_op(
            [],
        ...
        helper_test_op(
            [], lambda: torch.zeros([]), lambda: Tensor.zeros([]), forward_only=True
        )
    ...
    def test_empty_0(self):
        helper_test_op(
            [],
            lambda: torch.empty(45, 65) * 0 / 0,
            lambda: Tensor.empty(45, 65) * 0 / 0,
            forward_only=True,
        )
    ...
    def test_ones(self):
        helper_test_op(
            [],
            lambda: torch.ones(45, 65),
            lambda: Tensor.ones(45, 65),
            forward_only=True,
        )
        helper_test_op(
            [],
        ...
        helper_test_op(
            [], lambda: torch.ones([]), lambda: Tensor.ones([]), forward_only=True
        )
    ...
    def test_ones_like(self):
        ...
        helper_test_op(
            [],
            lambda: torch.ones_like(b),
            lambda: Tensor.ones_like(a),
            forward_only=True,
        )

    def test_eye(self):
        helper_test_op(
            [], lambda: torch.eye(10), lambda: Tensor.eye(10), forward_only=True
        )
        helper_test_op(
            [], lambda: torch.eye(1), lambda: Tensor.eye(1), forward_only=True
        )
    ...
    def test_arange(self):
        helper_test_op(
            [], lambda: torch.arange(10), lambda: Tensor.arange(10), forward_only=True
        )
        helper_test_op(
            [],
            lambda: torch.arange(5, 10, 3),
            lambda: Tensor.arange(5, 10, 3),
            forward_only=True,
        )
        helper_test_op(
            [],
            lambda: torch.arange(10, 5, -3),
            lambda: Tensor.arange(10, 5, -3),
            forward_only=True,
        )
        helper_test_op(
            [],
            lambda: torch.arange(11, 5, -3),
            lambda: Tensor.arange(11, 5, -3),
            forward_only=True,
        )
    ...
    def test_arange_big(self):
        helper_test_op(
            [], lambda: torch.arange(256), lambda: Tensor.arange(256), forward_only=True
        )
    ...
    def test_sum_collapse(self):
        helper_test_op(
            [],
            lambda: torch.ones(256, 256).sum(axis=1),
            lambda: Tensor.ones(256, 256).sum(axis=1),
            forward_only=True,
        )
    ...
    def test_sum_collapse_neg(self):
        helper_test_op(
            [],
            lambda: (-torch.ones(3, 3)).sum(axis=1),
            lambda: (-Tensor.ones(3, 3)).sum(axis=1),
            forward_only=True,
        )
    ...
    def test_sum_pad_collapse(self):
        helper_test_op(
            [],
            lambda: torch.nn.functional.pad(
                torch.ones(256, 256), pad=(0, 64, 0, 0)
            ).sum(axis=1),
            lambda: Tensor.ones(256, 256).pad(((0, 0), (0, 64))).sum(axis=1),
            forward_only=True,
        )
    ...
    def test_sum_cat_collapse(self):
        helper_test_op(
            [],
            lambda: torch.cat([torch.ones(256, 256), torch.zeros(256, 64)], dim=1).sum(
                axis=1
            ),
            lambda: Tensor.cat(Tensor.ones(256, 256), Tensor.zeros(256, 64), dim=1).sum(
                axis=1
            ),
            forward_only=True,
        )
    ...
    def test_max_dont_collapse(self):
        helper_test_op(
            [],
            lambda: torch.ones(256, 256).max(1)[0],
            lambda: Tensor.ones(256, 256).max(1),
            forward_only=True,
        )
    ...
    def test_where_permute(self):
        helper_test_op(
            ...
            lambda x: (x > 0.5).where(4, 2).permute((1, 0)),
            forward_only=True,
        )
    ...
    def _test_cmp(self, fxn, reverse=True):
        ...
        helper_test_op(
            None, fxn, fxn, forward_only=True, vals=[[0.0, 1, 2], [2.0, 1, 0]]
        )
        helper_test_op(
            None,
            lambda x, y: fxn(x, 2),
            lambda x, y: fxn(x, 2),
            forward_only=True,
            vals=[[0.0, 1, 2], [2.0, 1, 0]],
        )
        if (
            Device.DEFAULT != "WEBGPU"
        ):  # bool is not HOST_SHARABLE, so it cannot be used as a storage buffer type
            helper_test_op(
                None,
                fxn,
                fxn,
    ...
    def test_cmp_eq(self):
        self._test_cmp(lambda x, y: x == y, reverse=False)

    def test_cmp_gt(self):
        self._test_cmp(lambda x, y: x > y)
    ...
    def test_trunc(self):
        helper_test_op(
            [(45, 65)], lambda x: torch.trunc(x), lambda x: x.trunc(), forward_only=True
        )
        a, b = Tensor([1.0, 2.1, 0.0, -5.0, -2.5]), torch.tensor(
            [1.0, 2.1, 0.0, -5.0, -2.5]
        )
        helper_test_op(
            [], lambda: torch.trunc(b), lambda: Tensor.trunc(a), forward_only=True
        )

    # @unittest.skip("this is broken with contiguous")
    def test_floor(self):
        helper_test_op(
            [(45, 65)], lambda x: torch.floor(x), lambda x: x.floor(), forward_only=True
        )
        a, b = Tensor([1.0, 2.1, 0.0, -5.0, -2.5]), torch.tensor(
            [1.0, 2.1, 0.0, -5.0, -2.5]
        )
        helper_test_op(
            [], lambda: torch.floor(b), lambda: Tensor.floor(a), forward_only=True
        )

    # @unittest.skip("this is broken with contiguous")
    def test_ceil(self):
        helper_test_op(
            [(45, 65)], lambda x: torch.ceil(x), lambda x: x.ceil(), forward_only=True
        )
        a, b = Tensor([1.0, 2.1, 0.0, -5.0, -2.5]), torch.tensor(
            [1.0, 2.1, 0.0, -5.0, -2.5]
        )
        helper_test_op(
            [], lambda: torch.ceil(b), lambda: Tensor.ceil(a), forward_only=True
        )
    ...
    def test_maximum(self):
        ...
        helper_test_op(
            ...
            torch.maximum,
            Tensor.maximum,
            vals=[[1.0, 0.0, 3.0, 4.0], [1.0, 2.0, 3.0, 0.0]],
        )
        helper_test_op(
            None,
            torch.maximum,
            Tensor.maximum,
            vals=np.array([[1, 0, 3, 4], [1, 2, 3, 0]], dtype=np.int32),
            forward_only=True,
        )
    ...
    def test_div(self):
        ...
        helper_test_op(
            ...
            Tensor.div,
            forward_only=True,
            vals=np.array([[5], [1]], dtype=np.int32),
        )
    ...
    def test_div_int(self):
        helper_test_op(
            ...
            lambda x: x / 2,
            forward_only=True,
            vals=np.array([[3]], dtype=np.int32),
        )
    ...
    def test_mul_const_naninf(self):
        helper_test_op(
            [(45, 65)], lambda x: x * float("inf"), lambda x: x * float("inf")
        )
        helper_test_op(
            [(45, 65)], lambda x: x * -float("inf"), lambda x: x * -float("inf")
        )
        helper_test_op(
            [(45, 65)], lambda x: x * float("nan"), lambda x: x * float("nan")
        )
    ...
    def test_div_const_naninf(self):
        ...
        helper_test_op(
            [(45, 65)], lambda x: float("inf") / x, lambda x: float("inf") / x
        )
        helper_test_op(
            [(45, 65)], lambda x: (-float("inf")) / x, lambda x: (-float("inf")) / x
        )
        helper_test_op(
            [(45, 65)], lambda x: float("nan") / x, lambda x: float("nan") / x
        )
    ...
    def test_pow(self):
        ...
        helper_test_op(
            [(45, 65)], lambda x: x**0.2, lambda x: Tensor.pow(x, 0.2), a=-10
        )
        helper_test_op(
            [(45, 65)], lambda x: x**1.2, lambda x: Tensor.pow(x, 1.2), a=-10
        )
        helper_test_op([()], lambda x: x**0.2, lambda x: Tensor.pow(x, 0.2), a=-10)
        helper_test_op([()], lambda x: x**1.2, lambda x: Tensor.pow(x, 1.2), a=-10)
        a, b = Tensor([0.0], requires_grad=True), torch.tensor(
    ...
    def test_xor(self):
        ...
        helper_test_op(
            [], lambda: tor ^ 0x1337, lambda: ten ^ 0x1337, forward_only=True
        )
        helper_test_op(
            [], lambda: 0x1337 ^ tor, lambda: 0x1337 ^ ten, forward_only=True
        )
    ...
    def test_relu_exact(self):
        helper_test_op(None, lambda x: x.relu(), Tensor.relu, vals=[[-1.0, 0, 1]])

    def test_relu_maximum_exact(self):
        helper_test_op(
            None,
            lambda x: torch.maximum(x, torch.zeros_like(x, requires_grad=False)),
            lambda x: Tensor.maximum(x, 0),
            vals=[[-1.0, 0, 1]],
        )
    ...
    def test_leakyrelu(self):
        ...
        helper_test_op(
            [()], lambda x: torch.nn.functional.leaky_relu(x, 0.01), Tensor.leakyrelu
        )
    ...
    def test_celu(self):
        for val in range(1, 5):
            helper_test_op(
                [(45, 65)],
                lambda x: torch.nn.functional.celu(x, val),
                lambda x: x.celu(val),
            )
            helper_test_op(
                [()], lambda x: torch.nn.functional.celu(x, val), lambda x: x.celu(val)
            )
    ...
    def test_softsign(self):
        helper_test_op(
            [(45, 65)], lambda x: torch.nn.functional.softsign(x), Tensor.softsign
        )
    ...
    def test_quick_gelu(self):
        helper_test_op(
            [(45, 65)], lambda x: x * torch.sigmoid(1.702 * x), Tensor.quick_gelu
        )
        helper_test_op(
            [(45, 65)], lambda x: x * torch.sigmoid(1.702 * x), Tensor.quick_gelu, a=100
        )
        helper_test_op(
            [(45, 65)],
    ...
    def test_elu(self):
        ...
        helper_test_op(
            [(45, 65)],
            lambda x: torch.nn.functional.elu(x, alpha=0.1),
            lambda x: Tensor.elu(x, alpha=0.1),
        )
    ...
    def test_hardswish(self):
        helper_test_op(
            [(45, 65)],
            lambda x: torch.nn.functional.hardswish(x),
            Tensor.hardswish,
            atol=1e-6,
            grad_atol=1e-6,
        )
    ...
    def test_dot_1d(self):
        ...
        helper_test_op(
            [(65), (65, 45)], lambda x, y: x.matmul(y), Tensor.dot, atol=1e-4
        )
        helper_test_op(
            [(45, 65), (65)], lambda x, y: x.matmul(y), Tensor.dot, atol=1e-4
        )
        helper_test_op(
            [(8, 45, 65), (65)], lambda x, y: x.matmul(y), Tensor.dot, atol=1e-4
        )
        helper_test_op(
            [(65), (8, 65, 45)], lambda x, y: x.matmul(y), Tensor.dot, atol=1e-4
        )
    ...
    def test_dot(self):
        helper_test_op(
            [(45, 65), (65, 100)], lambda x, y: x.matmul(y), Tensor.dot, atol=1e-4
        )
        helper_test_op(
            [(8, 45, 65), (8, 65, 100)], lambda x, y: x.matmul(y), Tensor.dot, atol=1e-4
        )
        self.helper_test_exception(
            [(2, 4), (1, 3)],
            lambda x, y: x.matmul(y),
    ...
    def test_multinomial(self):
        ...
        helper_test_op(
            ...
            lambda x: Tensor.multinomial(x.clip(0, 1)),
            forward_only=True,
            atol=1000.0,
        )
    ...
    def test_cumsum(self):
        ...
        helper_test_op(
            ...
            lambda x: Tensor.cumsum(x, axis=1),
            atol=1e-6,
        )
        helper_test_op(
            [(20, 30, 40)],
            lambda x: torch.cumsum(x, dim=2),
            lambda x: Tensor.cumsum(x, axis=2),
            atol=1e-6,
        )
        helper_test_op(
            [(20, 30, 40)],
    ...
    def test_argmax(self):
        self.assertEqual(
            torch.Tensor([2, 2]).argmax().numpy(), Tensor([2, 2]).argmax().numpy()
        )  # check if returns first index for same max
        helper_test_op(
            [(10, 20)], lambda x: x.argmax(), lambda x: x.argmax(), forward_only=True
        )
        helper_test_op(
            [(10, 20)],
            lambda x: x.argmax(0, False),
            lambda x: x.argmax(0, False),
            forward_only=True,
        )
        helper_test_op(
            [(10, 20)],
            lambda x: x.argmax(1, False),
            lambda x: x.argmax(1, False),
            forward_only=True,
        )
        helper_test_op(
            [(10, 20)],
            lambda x: x.argmax(1, True),
            lambda x: x.argmax(1, True),
            forward_only=True,
        )
    ...
    def test_argmin(self):
        self.assertEqual(
            torch.Tensor([2, 2]).argmin().numpy(), Tensor([2, 2]).argmin().numpy()
        )
        helper_test_op(
            [(10, 20)], lambda x: x.argmin(), lambda x: x.argmin(), forward_only=True
        )
        helper_test_op(
            [(10, 20)],
            lambda x: x.argmin(0, False),
            lambda x: x.argmin(0, False),
            forward_only=True,
        )
        helper_test_op(
            [(10, 20)],
            lambda x: x.argmin(1, False),
            lambda x: x.argmin(1, False),
            forward_only=True,
        )
        helper_test_op(
            [(10, 20)],
            lambda x: x.argmin(1, True),
            lambda x: x.argmin(1, True),
            forward_only=True,
        )
    ...
    def test_einsum(self):
        ...
        helper_test_op(
            [(150, 150)],
            lambda a: torch.einsum("ij->ji", a),
            lambda a: Tensor.einsum("ij->ji", a),
        )
        # sum all elements
        helper_test_op(
            [(20, 30, 40)],
            lambda a: torch.einsum("ijk->", a),
            lambda a: Tensor.einsum("ijk->", a),
        )
        # column sum
        helper_test_op(
            [(50, 50)],
            lambda a: torch.einsum("ij->j", a),
            lambda a: Tensor.einsum("ij->j", a),
        )
        # row sum
        helper_test_op(
            [(15, 15)],
            lambda a: torch.einsum("ij->i", a),
            lambda a: Tensor.einsum("ij->i", a),
        )
        # matrix-vector multiplication
        helper_test_op(
            [(15, 20), (20,)],
            lambda a, b: torch.einsum("ik,k->i", a, b),
            lambda a, b: Tensor.einsum("ik,k->i", a, b),
        )
        # matrix-matrix multiplication
        helper_test_op(
            [(15, 20), (20, 30)],
            lambda a, b: torch.einsum("ik,kj->ij", a, b),
            lambda a, b: Tensor.einsum("ik,kj->ij", a, b),
        )
        # dot product
        helper_test_op(
            [(30), (30)],
            lambda a, b: torch.einsum("i,i->i", [a, b]),
            lambda a, b: Tensor.einsum("i,i->i", [a, b]),
        )
        # hadamard product
        helper_test_op(
            [(30, 40), (30, 40)],
            lambda a, b: torch.einsum("ij,ij->ij", a, b),
        ...
        helper_test_op(
            ...
            lambda a, b: Tensor.einsum("i,j->ij", a, b),
        )
        # batch matrix multiplication
        helper_test_op(
            [(10, 20, 30), (10, 30, 40)],
            lambda a, b: torch.einsum("ijk,ikl->ijl", [a, b]),
            lambda a, b: Tensor.einsum("ijk,ikl->ijl", [a, b]),
        )
        # batch matrix multiplication, result permuted
        helper_test_op(
            [(10, 20, 25), (10, 25, 32)],
            lambda a, b: torch.einsum("ijk,ikl->jil", [a, b]),
            lambda a, b: Tensor.einsum("ijk,ikl->jil", [a, b]),
        )
        # batch matrix multiplication, result & input permuted
        helper_test_op(
            [(20, 10, 25), (10, 25, 32)],
            lambda a, b: torch.einsum("jik,ikl->jil", [a, b]),
            lambda a, b: Tensor.einsum("jik,ikl->jil", [a, b]),
        )
        # tensor contraction
        helper_test_op(
            [(3, 5, 8, 10), (11, 13, 5, 16, 8)],
            lambda a, b: torch.einsum("pqrs,tuqvr->pstuv", a, b),
            lambda a, b: Tensor.einsum("pqrs,tuqvr->pstuv", a, b),
            atol=1e-5,
        )
        # tensor contraction, input permuted
        helper_test_op(
            [(3, 8, 10, 5), (11, 5, 13, 16, 8)],
            lambda a, b: torch.einsum("prsq,tquvr->pstuv", a, b),
            lambda a, b: Tensor.einsum("prsq,tquvr->pstuv", a, b),
            atol=1e-5,
        )
        # bilinear transformation
        helper_test_op(
            [(2, 3), (5, 3, 7), (2, 7)],
            lambda a, b, c: torch.einsum("ik,jkl,il->ij", [a, b, c]),
            lambda a, b, c: Tensor.einsum("ik,jkl,il->ij", [a, b, c]),
        )
    ...
    def test_matmul(self):
        helper_test_op(
            [(64), (64, 99)], lambda x, y: x.matmul(y), Tensor.dot, atol=1e-4
        )

    @unittest.skipIf(IMAGE > 0, "no batched matmul on images")
    def test_matmul_batched(self):
        helper_test_op(
            [(3), (1, 3, 3, 5)], lambda x, y: x.matmul(y), Tensor.dot, atol=1e-4
        )

    @unittest.skipIf(IMAGE > 0, "no batched matmul on images")
    def test_matmul_batched_vector(self):
        helper_test_op(
            [(4, 3), (1, 3, 3, 5)], lambda x, y: x.matmul(y), Tensor.dot, atol=1e-4
        )

    def test_small_gemm(self):
        helper_test_op(
            [(8, 8), (8, 8)], lambda x, y: x.matmul(y), lambda x, y: x @ y, atol=1e-3
        )
    ...
    def test_gemm(self):
        helper_test_op(
            [(64, 64), (64, 64)], lambda x, y: x.matmul(y), Tensor.dot, atol=1e-3
        )

    def test_big_gemm(self):
        helper_test_op(
            [(256, 256), (256, 256)], lambda x, y: x.matmul(y), Tensor.dot, atol=1e-3
        )

    def test_broadcastdot(self):
        helper_test_op(
            [(10, 45, 65), (65, 45)], lambda x, y: x @ y, Tensor.dot, atol=1e-4
        )
        with self.assertRaises(AssertionError):
            a = Tensor(3.14)
            b = Tensor.ones(3, 3)
    ...
    def test_multidot(self):
        helper_test_op(
            [(10, 45, 65), (10, 65, 45)], lambda x, y: x @ y, Tensor.dot, atol=1e-4
        )
        helper_test_op(
            [(3, 3, 45, 65), (3, 3, 65, 45)], lambda x, y: x @ y, Tensor.dot, atol=1e-4
        )

    def test_sum_simple(self):
        helper_test_op(None, lambda x: x.sum(), Tensor.sum, vals=[[1.0, 1.0]])
    ...
    def test_sum(self):
        ...
        helper_test_op(
            [(3, 4, 5, 6)], lambda x: x.sum(axis=3), lambda x: Tensor.sum(x, axis=3)
        )
        helper_test_op(
            [(3, 4, 5, 6)],
            lambda x: x.sum(axis=(1, 3)),
            lambda x: Tensor.sum(x, axis=(1, 3)),
        )
        helper_test_op(
            [(3, 4, 5, 6)],
            lambda x: x.sum(axis=(0, 2)),
            lambda x: Tensor.sum(x, axis=(0, 2)),
        )
        helper_test_op(
            [(3, 4, 5, 6)],
        ...
        helper_test_op(
            [(3, 4, 5, 6)], lambda x: x.sum(axis=1), lambda x: Tensor.sum(x, axis=1)
        )
    ...
    def test_min(self):
        ...
        helper_test_op(
            [(45, 3)], lambda x: x.min().mul(0.5), lambda x: Tensor.min(x).mul(0.5)
        )
    ...
    def test_max(self):
        ...
        helper_test_op(
            [(45, 3)], lambda x: x.max().mul(0.5), lambda x: Tensor.max(x).mul(0.5)
        )
        helper_test_op(
            None,
            lambda x: x.max().mul(0.5),
            lambda x: Tensor.max(x).mul(0.5),
            vals=[
                [[1.0, 1.0, 0.0, 1.0]],
            ],
        )
    ...
    def test_std(self):
        ...
        helper_test_op(
            [(45, 65, 85)],
            lambda x: torch.std(x, dim=None, correction=0),
            lambda x: Tensor.std(x, correction=0),
        )
        helper_test_op(
            [(45, 65, 85)],
            lambda x: torch.std(x, dim=None, correction=5),
            lambda x: Tensor.std(x, correction=5),
        )

    def test_std_axis(self):
        helper_test_op(
            [(45, 65, 85)],
            lambda x: torch.std(x, dim=0),
            lambda x: Tensor.std(x, axis=0),
        )
        helper_test_op(
            [(45, 65, 85)],
            lambda x: torch.std(x, dim=2),
            lambda x: Tensor.std(x, axis=2),
        )
        helper_test_op(
            [(45, 65, 85)],
            lambda x: torch.std(x, dim=[1, 2]),
            lambda x: Tensor.std(x, axis=[1, 2]),
        )
        helper_test_op(
            [(45, 65, 85)],
            lambda x: torch.std(x, dim=None),
            lambda x: Tensor.std(x, axis=None),
        )
        helper_test_op(
            [(45, 65, 85)],
            lambda x: torch.std(x, correction=0, dim=0),
            lambda x: Tensor.std(x, axis=0, correction=0),
        )
        helper_test_op(
            [(45, 65, 85)],
            lambda x: torch.std(x, correction=0, dim=2),
            lambda x: Tensor.std(x, axis=2, correction=0),
        )
        helper_test_op(
            [(45, 65, 85)],
            lambda x: torch.std(x, correction=0, dim=[1, 2]),
            lambda x: Tensor.std(x, axis=[1, 2], correction=0),
        )
        helper_test_op(
            [(45, 65, 85)],
            lambda x: torch.std(x, correction=0, dim=None),
            lambda x: Tensor.std(x, axis=None, correction=0),
        )

    def test_std_keepdim(self):
        helper_test_op(
            [(45, 65, 85)],
            lambda x: torch.std(x, dim=None, keepdim=True),
            lambda x: Tensor.std(x, keepdim=True),
        )
        helper_test_op(
            [(45, 65, 85)],
            lambda x: torch.std(x, dim=0, keepdim=True, correction=0),
    ...
    def test_log_softmax(self):
        ...
        helper_test_op(
            [()],
            lambda x: torch.nn.LogSoftmax(dim=0)(x),
            Tensor.log_softmax,
            atol=1e-7,
            grad_atol=1e-7,
        )
    ...
    def test_tanh(self):
        helper_test_op(
            [(45, 65)], lambda x: x.tanh(), Tensor.tanh, atol=1e-6, grad_atol=1e-6
        )
        helper_test_op(
            [(45, 65)],
            lambda x: x.tanh(),
    ...
    def test_hardtanh(self):
        for val in range(10, 30, 5):
            helper_test_op(
                [(45, 65)],
                lambda x: torch.nn.functional.hardtanh(x, -val, val),
                lambda x: x.hardtanh(-val, val),
                atol=1e-6,
                grad_atol=1e-6,
            )
            helper_test_op(
    ...
    def test_flip_eye_crash(self):
        helper_test_op(
            [],
            lambda: (torch.eye(10) @ torch.eye(10).flip(0)),
            lambda: (Tensor.eye(10) @ Tensor.eye(10).flip(0)),
            forward_only=True,
        )
            ...
            for shapes in [
                ...
                with self.subTest(op=torch_op.__name__, shapes=shapes):
                    helper_test_op(
                        ...
                        torch_op,
                        tinygrad_op,
                        a=-0.5 if tinygrad_op != Tensor.pow else 0.0,
                    )
    ...
    def test_slice_in_bounds_multidim(self):
        ...
        helper_test_op(
            [(3, 3, 3)], lambda x: x[1:2, 1:2, 0:-1], lambda x: x[1:2, 1:2, 0:-1]
        )
    ...
    def test_slice_with_none(self):
        ...
        helper_test_op(
            [(3, 3, 3)], lambda x: x[1:2, None, 1:2], lambda x: x[1:2, None, 1:2]
        )
        helper_test_op(
            [(3, 3, 3)],
            lambda x: x[1:2, 1:2, None, -1],
            lambda x: x[1:2, 1:2, None, -1],
        )
        helper_test_op(
            [(3, 3, 3)],
            lambda x: x[None, None, 1, None, 2, 0:2],
            lambda x: x[None, None, 1, None, 2, 0:2],
        )
    ...
    def test_slice_one_endpoint_out_of_bounds(self):
        ...
        helper_test_op(
            [(3, 3, 3)], lambda x: x[1:50, 1:2, -1], lambda x: x[1:50, 1:2, -1]
        )

    def test_slice_stride_gt_one(self):
        helper_test_op(
            [(7, 5, 10)], lambda x: x[::2, ::3, ::4], lambda x: x[::2, ::3, ::4]
        )
        helper_test_op(
            [(7, 5, 10)], lambda x: x[1:5:2, ::3, ::4], lambda x: x[1:5:2, ::3, ::4]
        )
        helper_test_op(
            [(7, 5, 10)], lambda x: x[1:5:2, 3, ::4], lambda x: x[1:5:2, 3, ::4]
        )
        helper_test_op(
            [(7, 5, 10)],
            lambda x: x[1:5:2, None, None, 3, None, ::4],
            lambda x: x[1:5:2, None, None, 3, None, ::4],
        )
    ...
    def test_slice_negative_strides(self):
        ...
        np.testing.assert_allclose(
            a[4:0:-3, 2:0:-1, -1:-5:-2], t[4:0:-3, 2:0:-1, -1:-5:-2].numpy()
        )
        if Device.DEFAULT != "CPU":
            # broken
            np.testing.assert_allclose(
                a[2:5:-1, :, :], t[2:5:-1, :, :].numpy()
            )  # shape = (0, 10, 10)
            np.testing.assert_allclose(
                a[:, 2:5:-1, :], t[:, 2:5:-1, :].numpy()
            )  # shape = (0, 10, 10)
    ...
    def test_slice_both_endpoints_out_of_bounds(self):
        helper_test_op(
            [(3, 3, 3)], lambda x: x[5:10], lambda x: x[5:10], forward_only=True
        )
        helper_test_op(
            [(3, 3, 3)], lambda x: x[-15:-7], lambda x: x[-15:-7], forward_only=True
        )

    def test_slice_start_gt_end(self):
        helper_test_op(
            [(3, 3, 3)], lambda x: x[-2:2], lambda x: x[-2:2], forward_only=True
        )
        helper_test_op(
            [(3, 3, 3)], lambda x: x[-2:-5], lambda x: x[-2:-5], forward_only=True
        )

    def test_slice_empty(self):
        helper_test_op(
            [(10, 10)], lambda x: x[1:1], lambda x: x[1:1], forward_only=True
        )
    ...
    def test_slice_errors(self):
        ...
        with self.assertRaises(IndexError):
            a[
                1, 77, 77, 77
            ]  # IndexError: (finds too many indices before the out of bounds)
        with self.assertRaises(IndexError):
        ...
        with self.assertRaises(ValueError):
            a[::0, 1]  # no 0 strides
        with self.assertRaises(IndexError):
            b[:]  # slice cannot be applied to a 0-dim tensor
    ...
    def test_slice_ellipsis(self):
        ...
        helper_test_op(
            [(3, 3, 3, 3)], lambda x: x[0:3, ..., 2:3], lambda x: x[0:3, ..., 2:3]
        )
        helper_test_op(
            [(3, 3, 3, 3)],
            lambda x: x[None, 0:3, ..., 0, None],
            lambda x: x[None, 0:3, ..., 0, None],
        )
    ...
    def test_pad2d(self):
        ...
        helper_test_op(
            ...
            lambda x: x.pad2d(padding=(1, 2, 3, 4), value=5),
        )
        helper_test_op(
            [(3, 3, 3, 3)],
            lambda x: torch.nn.functional.pad(x, (-1, 2, -3, 4), value=5),
    ...
    def test_pad(self):
        helper_test_op(
            [(3, 3)],
            lambda x: torch.nn.functional.pad(x, (1, 2, 3, 4)),
            lambda x: x.pad(((3, 4), (1, 2))),
        )
        helper_test_op(
            [(3, 3)],
            lambda x: torch.nn.functional.pad(x, (1, 2, 3, 4), value=5),
            lambda x: x.pad(((3, 4), (1, 2)), value=5),
        )
        helper_test_op(
            [(3, 3)],
    ...
    def test_pad_slice(self):
        for value in 0.0, 3.456:
            helper_test_op(
                [(1)],
                lambda x: torch.nn.functional.pad(x, (1, 0), value=value)[0],
                lambda x: x.pad(((1, 0),), value=value)[0],
            )
            helper_test_op(
                [(4)],
                lambda x: torch.nn.functional.pad(x, (1, 0), value=value)[0],
            ...
            helper_test_op(
                [(4)],
                lambda x: torch.nn.functional.pad(x, (3, 0), value=value)[0:1],
                lambda x: x.pad(((3, 0),), value=value)[0:1],
            )
            helper_test_op(
                [(4)],
                lambda x: torch.nn.functional.pad(x, (0, 3), value=value)[6],
                lambda x: x.pad(((0, 3),), value=value)[6],
            )
            helper_test_op(
                [(4)],
                lambda x: torch.nn.functional.pad(x, (0, 3), value=value)[4:6],
                lambda x: x.pad(((0, 3),), value=value)[4:6],
            )
            helper_test_op(
                [(5, 5)],
                lambda x: torch.nn.functional.pad(x, (0, 0, 1, 0), value=value)[0],
                lambda x: x.pad(((1, 0), (0, 0)), value=value)[0],
            )
            helper_test_op(
                [(2, 2)],
                lambda x: torch.nn.functional.pad(x, (0, 1, 0, 0), value=value)[0, 2],
                lambda x: x.pad(((0, 0), (0, 1)), value=value)[0, 2],
            )
            helper_test_op(
                [(4, 4)],
                lambda x: torch.nn.functional.pad(x, (0, 0, 1, 0), value=value)[0, 2],
                lambda x: x.pad(((1, 0), (0, 0)), value=value)[0, 2],
            )
            helper_test_op(
                [(4, 4)],
                lambda x: torch.nn.functional.pad(x, (0, 0, 0, 2), value=value)[5],
                lambda x: x.pad(((0, 2), (0, 0)), value=value)[5],
            )
            helper_test_op(
                [(4, 4)],
                lambda x: torch.nn.functional.pad(x, (0, 0, 0, 2), value=value)[3:5],
                lambda x: x.pad(((0, 2), (0, 0)), value=value)[3:5],
            )
            helper_test_op(
                [(4, 4)],
                lambda x: torch.nn.functional.pad(x, (3, 0, 0, 0), value=value)[1, 0],
                lambda x: x.pad(((0, 0), (3, 0)), value=value)[1, 0],
            )
            helper_test_op(
                [(4, 4)],
                lambda x: torch.nn.functional.pad(x, (3, 0, 0, 0), value=value)[1, 0:4],
                lambda x: x.pad(((0, 0), (3, 0)), value=value)[1, 0:4],
            )
            helper_test_op(
                [(4, 4)],
                lambda x: torch.nn.functional.pad(x, (3, 4, 1, 2), value=value)[0],
                lambda x: x.pad(((1, 2), (3, 4)), value=value)[0],
            )
            helper_test_op(
                [(4, 4)],
                lambda x: torch.nn.functional.pad(x, (3, 4, 1, 2), value=value)[:, 1],
                lambda x: x.pad(((1, 2), (3, 4)), value=value)[:, 1],
            )
            helper_test_op(
                [(4, 4)],
                lambda x: torch.nn.functional.pad(x, (3, 4, 1, 2), value=value)[:, 4],
                lambda x: x.pad(((1, 2), (3, 4)), value=value)[:, 4],
            )
            helper_test_op(
                [(3, 3)],
                lambda x: torch.nn.functional.pad(x, (0, 3, 0, 0), value=value)[:, 4:6],
                lambda x: x.pad(((0, 0), (0, 3)), value=value)[:, 4:6],
            )
            helper_test_op(
                [(3, 3)],
                lambda x: torch.nn.functional.pad(x, (0, 1, 3, 2), value=value)[0:2, :],
                lambda x: x.pad(((3, 2), (0, 1)), value=value)[0:2, :],
            )
            helper_test_op(
                [(3, 3, 3)],
                lambda x: torch.nn.functional.pad(x, (1, 1, 0, 1, 3, 2), value=value)[
                    0:2, :, :
                ],
    ...
    def test_stack_slice(self):
        helper_test_op(
            [(4)],
            lambda x: torch.stack([x for i in range(3)])[0, :],
            lambda x: Tensor.stack([x for i in range(3)])[0, :],
        )
        helper_test_op(
            [(5)],
            lambda x: torch.stack([x for i in range(3)])[0, 0],
            lambda x: Tensor.stack([x for i in range(3)])[0, 0],
        )
        helper_test_op(
            [(4, 4)],
            lambda x: torch.stack([x for i in range(4)])[3],
            lambda x: Tensor.stack([x for i in range(4)])[3],
        )

    def test_transpose(self):
        helper_test_op(
            [(3, 3, 3)], lambda x: x.transpose(1, 2), lambda x: x.transpose(1, 2)
        )
        helper_test_op(
            [(3, 3, 3)], lambda x: x.transpose(0, 2), lambda x: x.transpose(0, 2)
        )
        helper_test_op(
            [(1, 2, 3, 4)],
            lambda x: x.movedim((3, 0, 2, 1), (0, 1, 2, 3)),
            lambda x: x.permute(order=(3, 0, 2, 1)),
        )
        helper_test_op(
            [(3, 4, 5, 6)],
            lambda x: x.movedim((3, 2, 1, 0), (0, 1, 2, 3)),
            lambda x: x.permute(order=(3, 2, 1, 0)),
        )
    ...
    def test_flip(self):
        helper_test_op(
            [(4, 3, 6, 6)], lambda x: torch.flip(x, (0,)), lambda x: x.flip(axis=(0,))
        )
        helper_test_op(
            [(4, 3, 6, 6)],
            lambda x: torch.flip(x, (0, 1)),
            lambda x: x.flip(axis=(0, 1)),
        )
        helper_test_op(
            [(4, 3, 6, 6)],
            lambda x: torch.flip(x, (0, 1, 3)),
            lambda x: x.flip(axis=(0, 1, 3)),
        )
        helper_test_op(
            [(4, 3, 6, 6)], lambda x: torch.flip(x, (3,)), lambda x: x.flip(axis=(3,))
        )
        helper_test_op(
            [(4, 3, 6, 6)],
            lambda x: torch.flip(x, (0, 1, 3)).flip((0,)),
            lambda x: x.flip(axis=(0, 1, 3)).flip(0),
        )
        helper_test_op(
            [(4, 3, 6, 6)], lambda x: torch.flip(x, (3,)), lambda x: x.flip(axis=(-1,))
        )
        helper_test_op([()], lambda x: torch.flip(x, ()), lambda x: x.flip(axis=()))
        helper_test_op([(1,)], lambda x: torch.flip(x, ()), lambda x: x.flip(axis=()))
        helper_test_op(
            [(4, 3, 6, 6)], lambda x: torch.flip(x, ()), lambda x: x.flip(axis=())
        )

    def test_squeeze(self):
        helper_test_op(
            [(1, 3, 6, 6)], lambda x: torch.squeeze(x, 0), lambda x: x.squeeze(dim=0)
        )
        helper_test_op(
            [(4, 3, 1, 6)], lambda x: torch.squeeze(x, 1), lambda x: x.squeeze(dim=1)
        )
        helper_test_op(
            [(4, 3, 6, 6)], lambda x: torch.squeeze(x, 3), lambda x: x.squeeze(dim=3)
        )
        self.helper_test_exception(
            [(4, 3, 6, 6)],
            lambda x: torch.squeeze(x, 50),
            lambda x: x.squeeze(dim=50),
            expected=IndexError,
            exact=True,
        )
        self.helper_test_exception(
        ...
        helper_test_op(
            [(4, 3, 6, 1)], lambda x: torch.squeeze(x, -1), lambda x: x.squeeze(dim=-1)
        )
        helper_test_op(
            [(4, 3, 6, 6)], lambda x: torch.squeeze(x), lambda x: x.squeeze()
        )
        helper_test_op(
            [(1, 3, 6, 6)], lambda x: torch.squeeze(x), lambda x: x.squeeze()
        )
        helper_test_op([(2, 3, 1)], lambda x: torch.squeeze(x), lambda x: x.squeeze())
        helper_test_op(
            [()], lambda x: torch.squeeze(x, -1), lambda x: x.squeeze(dim=-1)
        )
        helper_test_op([()], lambda x: torch.squeeze(x, 0), lambda x: x.squeeze(dim=0))
        self.helper_test_exception(
            [()],
            lambda x: torch.squeeze(x, 10),
            lambda x: x.squeeze(dim=10),
            expected=IndexError,
            exact=True,
        )
    ...
    def test_unsqueeze(self):
        ...
        helper_test_op(
            [(4, 3, 6, 6)],
            lambda x: torch.unsqueeze(x, 4),
            lambda x: x.unsqueeze(dim=4),
        )
        helper_test_op(
            [(4, 3, 6, 6)],
            lambda x: torch.unsqueeze(x, -1),
            lambda x: x.unsqueeze(dim=-1),
        )
        helper_test_op(
            [(4, 3, 6, 6)],
            lambda x: torch.unsqueeze(x, -3),
            lambda x: x.unsqueeze(dim=-3),
        )
        helper_test_op(
            [()], lambda x: torch.unsqueeze(x, 0), lambda x: x.unsqueeze(dim=0)
        )
    ...
    def test_flatten(self):
        for axis in range(3):
            helper_test_op(
                [(4, 3, 6, 6)],
                lambda x: torch.flatten(x, start_dim=axis),
                lambda x: x.flatten(axis),
            )
    ...
    def test_detach(self):
        ...
        helper_test_op(
            [()], lambda x: x.detach(), lambda x: x.detach(), forward_only=True
        )

    def test_expand(self):
        arg = (4, 3, 2, 6)
        helper_test_op(
            [(4, 3, 1, 6)], lambda x: x.expand(arg), lambda x: x.expand(shape=arg)
        )
    ...
    def test_biased_conv2d(self):
        ...
        helper_test_op(
            ...
            lambda x, w, b: torch.nn.functional.conv2d(
                torch.nn.functional.conv2d(x, w, b).relu(), w, b
            ),
    ...
    def test_padded_conv3d(self):
        helper_test_op(
            ...
            lambda x, w: Tensor.conv2d(x, w, padding=[1, 1, 1, 1, 1, 1]).relu(),
            atol=1e-4,
            grad_rtol=1e-5,
        )
    ...
    def test_nested_conv2d(self):
        helper_test_op(
            [(1, 32, 9, 9), (32, 32, 3, 3), (32, 32, 3, 3)],
            lambda x, w1, w2: torch.nn.functional.conv2d(
                torch.nn.functional.conv2d(x, w1).relu(), w2
            ).relu(),
    ...
    def test_simple_conv2d_nhwc(self):
        ...
        helper_test_op(
            ...
            lambda x, w: torch.nn.functional.conv2d(
                x.permute(0, 3, 1, 2), w.permute(3, 2, 0, 1)
            ).relu(),
    ...
    def test_simple_conv_transpose2d(self):
        helper_test_op(
            ...
            lambda x, w: Tensor.conv_transpose2d(x, w).relu(),
            atol=1e-4,
            grad_rtol=1e-5,
        )
    ...
    def test_grouped_conv_transpose2d(self):
        helper_test_op(
            ...
            lambda x, w: Tensor.conv_transpose2d(x, w, groups=2).relu(),
            atol=1e-4,
            grad_rtol=1e-5,
        )
    ...
    def test_padded_conv_transpose2d(self):
        for padding in [(1, 2), (2, 1), 2, 1, 0]:
            helper_test_op(
                ...
                lambda x, w: Tensor.conv_transpose2d(x, w, padding=padding).relu(),
                atol=1e-4,
                grad_rtol=1e-5,
            )
    ...
    def test_dilated_conv_transpose2d(self):
        for dilation in [(1, 2), (2, 1), 2, 1]:
            helper_test_op(
                [(2, 4, 9, 9), (4, 4, 3, 3)],
                lambda x, w: torch.nn.functional.conv_transpose2d(
                    x, w, dilation=dilation
                ).relu(),
                lambda x, w: Tensor.conv_transpose2d(x, w, dilation=dilation).relu(),
                atol=1e-4,
                grad_rtol=1e-5,
            )
    ...
    def test_strided_conv_transpose2d(self):
        for stride in [(2, 1), (1, 2), 1]:
            helper_test_op(
                ...
                lambda x, w: Tensor.conv_transpose2d(x, w, stride=stride).relu(),
                atol=1e-4,
                grad_rtol=1e-5,
            )
    ...
    def test_output_padded_conv_transpose2d(self):
        for output_padding, stride in [((1, 1), (2, 3)), ((2, 1), (3, 2))]:
            helper_test_op(
                [(2, 4, 6, 5), (4, 4, 3, 3), (4,)],
                lambda x, w, b: torch.nn.functional.conv_transpose2d(
                    x, w, b, output_padding=output_padding, stride=stride
                ).relu(),
                lambda x, w, b: Tensor.conv_transpose2d(
                    x, w, b, output_padding=output_padding, stride=stride
                ).relu(),
                atol=1e-4,
                grad_rtol=1e-5,
            ...
            for cin in [1, 3]:
                for H in [1, 2, 5]:
                    for groups in [1, 3] if cin == 3 and H == 5 else [1]:
                        with self.subTest(
                            batch_size=bs, channels=cin, groups=groups, height=H
                        ):
                            helper_test_op(
                                [(bs, cin, 11), (6, cin // groups, H)],
                                lambda x, w: torch.nn.functional.conv1d(
                                    x, w, groups=groups
                                ).relu(),
    ...
    def test_conv2d(self):
        self._test_conv2d(bs=1, cin=3)

    def test_conv2d_bs_4_cin_3(self):
        self._test_conv2d(bs=4, cin=3)

    def test_conv2d_bs_1_cin_1(self):
        self._test_conv2d(bs=1, cin=1)

    def test_conv2d_bs_4_cin_1(self):
        self._test_conv2d(bs=4, cin=1)
    ...
    def test_simple_grouped_conv2d(self):
        ...
        cin = 2
        helper_test_op(
            [(bs, groups * cin, 1, 1), (groups * rcout, cin, 1, 1)],
            lambda x, w: torch.nn.functional.conv2d(x, w, groups=groups).relu(),
            lambda x, w: Tensor.conv2d(x, w, groups=groups).relu(),
            atol=1e-4,
            grad_rtol=1e-5,
    ...
    def test_depthwise_conv2d(self):
        ...
        cin = 1
        helper_test_op(
            [(bs, groups * cin, 32, 32), (groups * rcout, cin, 1, 1)],
            lambda x, w: torch.nn.functional.conv2d(x, w, groups=groups).relu(),
            lambda x, w: Tensor.conv2d(x, w, groups=groups).relu(),
            atol=1e-4,
            grad_rtol=1e-5,
    ...
    def test_grouped_conv2d(self):
        ...
        cin = 3
        helper_test_op(
            [(bs, groups * cin, 5, 5), (groups * rcout, cin, 3, 3)],
            lambda x, w: torch.nn.functional.conv2d(x, w, groups=groups).relu(),
            lambda x, w: Tensor.conv2d(x, w, groups=groups).relu(),
            atol=1e-4,
            grad_rtol=1e-5,
    ...
    def test_fancy_conv2d(self):
        ...
        H, W = 3, 3
        helper_test_op(
            [(bs, cin, 11, 28), (groups * cout, cin // groups, H, W)],
            lambda x, w: torch.nn.functional.conv2d(x, w, groups=groups).relu(),
            lambda x, w: Tensor.conv2d(x, w, groups=groups).relu(),
            atol=1e-4,
            grad_rtol=1e-5,
            ...
            with self.subTest(stride=stride):
                helper_test_op(
                    ...
                    lambda x: torch.nn.functional.max_pool2d(
                        x, kernel_size=(2, 2), stride=stride
                    ),
        ...
        for stride in [(2, 3), (3, 2), 2, 3]:
            with self.subTest(stride=stride):
                helper_test_op(
                    [(32, 2, 110, 28)],
                    lambda x: torch.nn.functional.max_pool2d(
                        x, kernel_size=(5, 5), stride=stride
                    ),
    ...
    def test_maxpool2d_dilation(self):
        for dilation in [(2, 3), (3, 2), 2, 3]:
            helper_test_op(
                [(32, 2, 110, 28)],
                lambda x: torch.nn.functional.max_pool2d(
                    x, kernel_size=(5, 5), dilation=dilation
                ),
    ...
    def test_global_avgpool2d(self):
        helper_test_op(
            ...
            lambda x: Tensor.avg_pool2d(x, kernel_size=(111, 28)),
            rtol=1e-5,
        )
    ...
    def test_cat(self):
        for dim in range(-2, 3):
            helper_test_op(
                [(45, 65, 9), (45, 65, 9), (45, 65, 9)],
                lambda x, y, z: torch.cat((x, y, z), dim),
    ...
    def test_stack(self):
        ...
        for dim in range(-1, 3):
            helper_test_op(
                [(45, 65, 3), (45, 65, 3), (45, 65, 3)],
                lambda x, y, z: torch.stack((x, y, z), dim=dim),
        ...
        a = Tensor(3.14)
        np.testing.assert_allclose(
            Tensor.stack([a, a]).numpy(), Tensor([3.14, 3.14]).numpy()
        )
    ...
    def test_repeat(self):
        ...
        for reps in [[], [4], [2, 1], [3, 2, 2]]:
            ...
            helper_test_op(
                [(4, 6, 3)], lambda x: x.repeat(*repeats), lambda x: x.repeat(repeats)
            )
            helper_test_op(
        ...
        np.testing.assert_allclose(
            x.repeat((2, 0, 4)).numpy(), Tensor.zeros(8, 0, 12).numpy()
        )

    def test_clip(self):
        helper_test_op(
            [(45, 65)], lambda x: x.clip(-2.3, 1.2), lambda x: x.clip(-2.3, 1.2)
        )

    def test_matvecmat(self):
        helper_test_op(
            [(1, 128), (128, 128), (128, 128)],
            lambda x, y, z: (x @ y).relu() @ z,
            atol=1e-4,
        )
    ...
    def _get_index_randoms(self):
        ...
        b = torch.randint(
            high=1, size=(1, 3, 1, 1, 1, 1), dtype=torch.int64, requires_grad=False
        )
        c = torch.randint(
            low=-5,
            high=5,
        ...
        d = torch.randint(
            high=4, size=(2, 1, 1, 5, 1, 1), dtype=torch.int64, requires_grad=False
        )
        e = torch.randint(
            high=1, size=(1, 1, 1, 1, 6, 1), dtype=torch.int64, requires_grad=False
        )
        i, j, k, o, p = [
            Tensor(
                tor.detach().numpy().astype(np.int32),
                dtype=dtypes.int32,
                requires_grad=False,
            )
            for tor in [a, b, c, d, e]
        ]
    ...
    def test_slice_fancy_indexing_no_dim_collapse(self):
        ...
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)], lambda x: x[a, b, c, d, e], lambda x: x[i, j, k, o, p]
        )
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)], lambda x: x[:, b, c, d, :], lambda x: x[:, j, k, o, :]
        )
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)], lambda x: x[a, b, ...], lambda x: x[i, j, ...]
        )
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)], lambda x: x[a, ..., e], lambda x: x[i, ..., p]
        )
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)], lambda x: x[..., c, :, e], lambda x: x[..., k, :, p]
        )
    ...
    def test_slice_fancy_indexing_dim_collapse_int(self):
        ...
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)], lambda x: x[1, b, c, d, e], lambda x: x[1, j, k, o, p]
        )
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)], lambda x: x[a, b, 3, d, e], lambda x: x[i, j, 3, o, p]
        )
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)], lambda x: x[1, b, 2, d, 2], lambda x: x[1, j, 2, o, 2]
        )
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)], lambda x: x[a, 2, 2, 2, e], lambda x: x[i, 2, 2, 2, p]
        )
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)],
            lambda x: x[1, :, 3:11:2, d, 0:2],
            lambda x: x[1, :, 3:11:2, o, 0:2],
        )
    ...
    def test_slice_fancy_indexing_dim_inject_none(self):
        ...
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)],
            lambda x: x[None, b, c, d, e],
            lambda x: x[None, j, k, o, p],
        )
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)],
            lambda x: x[a, b, c, d, None],
            lambda x: x[i, j, k, o, None],
        )
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)],
            lambda x: x[a, b, None, d, e],
            lambda x: x[i, j, None, o, p],
        )
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)],
            lambda x: x[None, b, c, d, None],
            lambda x: x[None, j, k, o, None],
        )
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)],
            lambda x: x[a, :, None, d, e],
            lambda x: x[i, :, None, o, p],
        )
    ...
    def test_slice_fancy_indexing_dim_inject_and_collapse(self):
        ...
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)],
            lambda x: x[1, b, None, d, 1],
            lambda x: x[1, j, None, o, 1],
        )
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)],
            lambda x: x[None, b, 2, d, None],
            lambda x: x[None, j, 2, o, None],
        )
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)],
            lambda x: x[..., 1, d, None],
    ...
    def test_slice_fancy_indexing_with_idx(self):
        ...
        helper_test_op(
            [(2, 3)],
            lambda x: x[torch.tensor([[0, 0, 0], [0, 0, 0]]), torch.tensor(1)],
            lambda x: x[Tensor([[0, 0, 0], [0, 0, 0]]), Tensor(1)],
        )
        helper_test_op(
            [(2, 3)],
            lambda x: x[torch.tensor([1]), torch.tensor([[0, 0, 0], [0, 0, 0]])],
            lambda x: x[Tensor([1]), Tensor([[0, 0, 0], [0, 0, 0]])],
        )
        helper_test_op(
            [(2, 3)],
            lambda x: x[torch.tensor([[0, 0, 0], [0, 0, 0]]), torch.tensor([2, 1, 1])],
            lambda x: x[Tensor([[0, 0, 0], [0, 0, 0]]), Tensor([2, 1, 1])],
        )
        helper_test_op(
            [(2, 3)],
            lambda x: x[torch.tensor([[0, 1, -1], [-1, -2, 0]]), torch.tensor([2, 1, -1])],
    ...
    def test_slice_fancy_indexing_list_indices(self):
        ...
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)],
            lambda x: x[[1, 0], b, c, d, :],
            lambda x: x[[1, 0], j, k, o, :],
        )
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)],
            lambda x: x[a, b, c, [1, 2, 3], ...],
            lambda x: x[i, j, k, [1, 2, 3], ...],
        )
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)],
            lambda x: x[a, [2, 1, 0], c, [2, 1, 0], e],
            lambda x: x[i, [2, 1, 0], k, [2, 1, 0], p],
        )
    ...
    def test_slice_fancy_indexing_tuple_indices(self):
        ...
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)],
            lambda x: x[1, (2, 1, 0), None, c, (2, 1, 0), e],
    ...
    def test_slice_fancy_indexing_list_with_tensors(self):
        ...
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)], lambda x: x[[a, [1, 1]]], lambda x: x[[i, [1, 1]]]
        )
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)], lambda x: x[[a, (1, 1)]], lambda x: x[[i, (1, 1)]]
        )
        helper_test_op(
            [(2, 5, 6, 5, 3, 4)],
            lambda x: x[[a, b, c, d, e]],
            lambda x: x[[i, j, k, o, p]],
        )
    ...
    def test_gather(self):
        ...
        a = Tensor(
            b.detach().numpy().astype(np.int32), dtype=dtypes.int32, requires_grad=False
        )
        helper_test_op(
            [(4, 5, 6)],
            lambda x: x.gather(index=b, dim=0),
            lambda x: x.gather(idx=a, dim=0),
        )
        helper_test_op(
            [(4, 5, 6)],
            lambda x: x.gather(index=b, dim=1),
            lambda x: x.gather(idx=a, dim=1),
        )
        helper_test_op(
            [(4, 5, 6)],
            lambda x: x.gather(index=b, dim=2),
            lambda x: x.gather(idx=a, dim=2),
        )
        helper_test_op(
            [(3, 4, 5)],
            lambda x: x.gather(index=b, dim=0),
        ...
        self.helper_test_exception(
            [(4, 5, 6)],
            lambda x: x.gather(index=torch.tensor([1], dtype=torch.int64), dim=0),
            lambda x: x.gather(idx=Tensor([1], dtype=dtypes.int32), dim=0),
            expected=(RuntimeError, AssertionError),
        )
        self.helper_test_exception(
            [(2, 1, 1)],
    ...
    def test_scaled_product_attention(self):
        helper_test_op(
            [(32, 8, 16, 64), (32, 8, 16, 64), (32, 8, 16, 64)],
        ...
        helper_test_op(
            [(32, 8, 16, 64), (32, 8, 16, 64), (32, 8, 16, 64), (32, 8, 16, 16)],
            lambda x, y, z, m: torch.nn.functional.scaled_dot_product_attention(
                x, y, z, attn_mask=m
            ),
            lambda x, y, z, m: Tensor.scaled_dot_product_attention(
                x, y, z, attn_mask=m
            ),
        )
    ...
    def test_scaled_product_attention_causal(self):
        helper_test_op(
            [(32, 8, 16, 64), (32, 8, 16, 64), (32, 8, 16, 64)],
            lambda x, y, z: torch.nn.functional.scaled_dot_product_attention(
                x, y, z, is_causal=True
            ),
            lambda x, y, z: Tensor.scaled_dot_product_attention(
                x, y, z, is_causal=True
            ),
        )
    ...
    def test_binary_crossentropy(self):
        helper_test_op(
            [(32, 10), (32, 10)],
            lambda x, y: torch.nn.functional.binary_cross_entropy(
                x.sigmoid(), torch.clip(y, 0, 1)
            ),
