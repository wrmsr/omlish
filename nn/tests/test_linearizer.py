...
class TestLinearizer(unittest.TestCase):
    ...
    def test_tensor_cores(self):
        ...
        for tc in tensor_cores[Device.DEFAULT]:
            if tc.arch is not None and tc.arch != os.uname().machine:
                continue
            a, b = Tensor.rand(tc.dims[0], tc.dims[2], dtype=tc.dtype_in), Tensor.rand(
                tc.dims[2], tc.dims[1], dtype=tc.dtype_in
            )
            np_a, np_b = a.numpy(), b.numpy()
            if tc.dtype_out != tc.dtype_in:
                r = (
                    (
                        a.reshape(tc.dims[0], 1, tc.dims[2])
                        * b.permute(1, 0).reshape(1, tc.dims[1], tc.dims[2])
                    )
                    .cast(tc.dtype_out)
                    .sum(axis=2)
                )
            ...
            k.linearize()
            assert (
                len([uop for uop in k.uops if uop.uop == UOps.WMMA]) == 1
    ...
    def test_sum_collapse(self):
        ...
        lin = Linearizer(sched[0].ast)
        assert not any(
            u.uop == UOps.LOOP for u in lin.linearize().uops
        ), "found loop in sum collapse"
    ...
    def test_simplify_uop(self):
        ...
        assert (
            helper_test_simplify(
                UOps.ALU,
                dtypes.bool,
                vin=(UOp(UOps.CONST, dtypes.bool, vin=(), arg=True), c0, c0),
        ...
        assert (
            helper_test_simplify(
                UOps.ALU,
                dtypes.bool,
                vin=(UOp(UOps.CONST, dtypes.bool, vin=(), arg=True), c0, c1),
                arg=TernaryOps.WHERE,
            ).uop
            == UOps.ALU
        )
...
def helper_realized_ast(r: Tensor):
    ...
    output_buffer = Buffer(
        s[-1].out.device,
        prod((s if isinstance(s, int) else s.max for s in s[-1].out.shape)),
        s[-1].out.dtype,
        **s[-1].out._device_extra_args()
...
class TestFloat4(unittest.TestCase):
    def setUp(self):
        if (
            not isinstance(Device[Device.DEFAULT], Compiled)
            or not Device[Device.DEFAULT].linearizer_opts.supports_float4
        ):
        ...
        return (
            len(
                [
                    uop
                    for uop in k.uops
                    if uop.uop == UOps.LOAD and uop.dtype == dtypes.float.vec(4)
                ]
            ),
            len(
                [
                    uop
                    for uop in k.uops
                    if uop.uop == UOps.STORE
                    and len(uop.vin) == 3
                    and uop.vin[2].dtype == dtypes.float.vec(4)
                ]
            ),
        )
...
class TestHandCodedOpts(unittest.TestCase):
    ...
    @unittest.skipIf(
        ...
        "Failing because of custom kernel splitting to circumvent the 8 buffer limit",
    )
    ...
    def test_masked_upcast_wino_full(self):
        ...
        Tensor.wino = True
        x, w = (
            Tensor.rand(1, 4, 9, 9, requires_grad=True).realize(),
...
def helper_linearizer_opt(r: Tensor, opts=[], apply_tc=False):
    ...
    def check_opt(opts, create_k, to_prg):
        ...
        prg = to_prg(k)
        real_bufs[0].copyin(
            np.zeros((real_bufs[0].size,), dtype=real_bufs[0].dtype.np).data
        )  # Zero to check that all values are filled
        prg.exec(real_bufs)
        np.testing.assert_allclose(
            wanna_output, real_bufs[0].toCPU(), atol=1e-4, rtol=1e-4
        )
    ...
    for x in opts:  # Check custom transformations if any.
        check_opt(
            x, lambda: Linearizer(realized_ast), Device[Device.DEFAULT].to_program
        )
...
class TestLinearizerOpts(unittest.TestCase):
    def test_local_and_grouped_reduce(self):
        if (
            not isinstance(Device[Device.DEFAULT], Compiled)
            or not Device[Device.DEFAULT].linearizer_opts.has_local
            or not Device[Device.DEFAULT].linearizer_opts.has_shared
        ):
    ...
    def test_tensor_core_opts(self):
        ...
        helper_linearizer_opt(
            ...
            [
                ...
                ],  # check combo of last unroll and last local
                [
                    Opt(OptOps.UPCAST, 0, 4),
                    Opt(OptOps.UPCAST, 1, 4),
                    Opt(OptOps.UNROLL, 0, 2),
    ...
    def test_padto_matmul(self):
        ...
        if Device.DEFAULT == "CUDA":
            self.skipTest("super slow on CUDA")
        N = 17 * 17
