...
from ...shape.symbolic import create_rednode
from ...shape.symbolic import sym_infer
from ...shape.symbolic import sym_render
...
class TestSymbolic(unittest.TestCase):
    ...
    def test_ge_divides_and(self):
        ...
        self.helper_test_variable(
            expr // 4, 0, 1, "((((FLOAT8_INDEX//4)+idx2)<128) and ((idx1//4)<32))"
        )
    ...
    def test_sum_div_min_max(self):
        self.helper_test_variable(
            Variable.sum([Variable("a", 0, 7), Variable("b", 0, 3)]) // 2,
            0,
            5,
            "((a+b)//2)",
        )
    ...
    def test_sum_div_factor(self):
        self.helper_test_variable(
            Variable.sum([Variable("a", 0, 7) * 4, Variable("b", 0, 3) * 4]) // 2,
            0,
            20,
            "((a*2)+(b*2))",
        )
    ...
    def test_sum_div_some_factor(self):
        self.helper_test_variable(
            Variable.sum([Variable("a", 0, 7) * 5, Variable("b", 0, 3) * 4]) // 2,
            0,
            23,
            "(((a*5)//2)+(b*2))",
        )
    ...
    def test_sum_div_some_partial_factor(self):
        self.helper_test_variable(
            Variable.sum([Variable("a", 0, 7) * 6, Variable("b", 0, 7) * 6]) // 16,
            0,
            5,
            "(((a*3)+(b*3))//8)",
        )
        self.helper_test_variable(
            Variable.sum(
                [NumNode(16), Variable("a", 0, 7) * 6, Variable("b", 0, 7) * 6]
            )
            // 16,
            1,
            6,
    ...
    def test_sum_div_no_factor(self):
        self.helper_test_variable(
            Variable.sum([Variable("a", 0, 7) * 5, Variable("b", 0, 3) * 5]) // 2,
            0,
            25,
            "(((a*5)+(b*5))//2)",
        )
    ...
    def test_mod_factor(self):
        ...
        self.helper_test_variable(
            Variable.sum([Variable("a", 0, 7) * 100, Variable("b", 0, 3) * 50]) % 100,
            0,
            99,
            "((b*50)%100)",
        )
    ...
    def test_mod_to_sub(self):
        ...
        self.helper_test_variable(
            (1 + Variable("a", 1, 2)) % 2, 0, 1, (Variable("a", 1, 2) - 1).render()
        )

    def test_sum_div_const(self):
        self.helper_test_variable(
            Variable.sum([Variable("a", 0, 7) * 4, NumNode(3)]) // 4, 0, 7, "a"
        )

    def test_sum_div_const_big(self):
        self.helper_test_variable(
            Variable.sum([Variable("a", 0, 7) * 4, NumNode(3)]) // 16, 0, 1, "(a//4)"
        )

    def test_sum_lt_fold(self):
        self.helper_test_variable(
            Variable.sum([Variable("a", 0, 7) * 4, Variable("b", 0, 3)]) < 16,
            0,
            1,
            "(a<4)",
        )
        self.helper_test_variable(
            Variable.sum([Variable("a", 0, 7) * 4, Variable("b", 0, 4)]) < 16,
            0,
            1,
            "(((a*4)+b)<16)",
        )
        self.helper_test_variable(
            Variable.sum([Variable("uidx", 0, 3), Variable("a", 0, 1529) * 12])
            < (4 * 67),
            0,
            1,
            "(a<23)",
    ...
    def test_distribute_mul(self):
        self.helper_test_variable(
            Variable.sum([Variable("a", 0, 3), Variable("b", 0, 5)]) * 3,
            0,
            24,
            "((a*3)+(b*3))",
        )
    ...
    def test_mod_mul_sum(self):
        self.helper_test_variable(
            Variable.sum([Variable("b", 0, 2), Variable("a", 0, 5) * 10]) % 9,
            0,
            7,
            "(a+b)",
        )
    ...
    def test_and_fold(self):
        self.helper_test_variable(
            Variable.ands([NumNode(0), Variable("a", 0, 1)]), 0, 0, "0"
        )

    def test_and_remove(self):
        self.helper_test_variable(
            Variable.ands([NumNode(1), Variable("a", 0, 1)]), 0, 1, "a"
        )

    def test_mod_factor_negative(self):
        self.helper_test_variable(
            Variable.sum(
                [NumNode(-29), Variable("a", 0, 10), Variable("b", 0, 10) * 28]
            )
            % 28,
            0,
            27,
        ...
        self.helper_test_variable(
            Variable.sum(
                [NumNode(-29), Variable("a", 0, 100), Variable("b", 0, 10) * 28]
            )
            % 28,
            0,
            27,
    ...
    def test_sum_combine_num(self):
        self.helper_test_variable(
            Variable.sum([NumNode(29), Variable("a", 0, 10), NumNode(-23)]),
            6,
            16,
            "(6+a)",
        )
    ...
    def test_sum_num_hoisted_and_factors_cancel_out(self):
        self.helper_test_variable(
            Variable.sum([Variable("a", 0, 1) * -4 + 1, Variable("a", 0, 1) * 4]),
            1,
            1,
            "1",
        )
    ...
    def test_div_factor(self):
        self.helper_test_variable(
            Variable.sum(
                [NumNode(-40), Variable("a", 0, 10) * 2, Variable("b", 0, 10) * 40]
            )
            // 40,
            -1,
            9,
    ...
    def test_div_remove(self):
        self.helper_test_variable(
            Variable.sum([Variable("idx0", 0, 127) * 4, Variable("idx2", 0, 3)]) // 4,
            0,
            127,
            "idx0",
        )
...
class TestSymbolicSymbolicOps(unittest.TestCase):
    ...
    def test_sumnode_divmod_sumnode(self):
        ...
        assert (idx0 * (i * 4 + 4) + idx1 * (i + 1) + idx2) // (
            i + 1
        ) == idx0 * 4 + idx1
    ...
    def test_sumnode_divmod_sumnode_complex(self):
        ...
        assert (
            (i * 128 + 128) * 2 + gidx0 * 128 + lidx1 * (i * 512 + 512) + ridx2 * 4
        ) // (i * 128 + 128) == 2 + lidx1 * 4
        assert (
            (i * 128 + 128) * 2 + gidx0 * 128 + lidx1 * (i * 512 + 512) + ridx2 * 4
        ) % (i * 128 + 128) == gidx0 * 128 + ridx2 * 4
        assert ((gidx0 * 128 + i * 128 + ridx2 * 4 + 129)) // (i * 128 + 128) == 1
        assert ((gidx0 * 128 + i * 128 + ridx2 * 4 + 129)) % (
            i * 128 + 128
        ) == gidx0 * 128 + ridx2 * 4 + 1
        assert (ridx2 * (i * 4 + 4) + 1 + i + gidx0) // (i * 128 + 128) == 0
        assert (ridx2 * (i * 4 + 4) + 1 + i + gidx0) % (i * 128 + 128) == (
            ridx2 * (i * 4 + 4) + 1 + i + gidx0
        )
    ...
    def test_multi_expand(self):
        ...
        assert s1.expand((a, b)) == [
            NumNode(x + y)
            for x in range(b.min, b.max + 1)
            for y in range(a.min, a.max + 1)
        ]
