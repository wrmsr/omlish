...
from ...helpers import ContextVar
...
class TestContextVars(unittest.TestCase):
    ...
    def test_value_accross_modules(self):
        ...
        exec(
            'from ...helpers import ContextVar;C = ContextVar("C", 13)', {}
    ...
    def test_assignment_across_modules(self):
        ...
        exec(
            'from ...helpers import ContextVar;B = ContextVar("B", 0);B.value = 3;',
            {},
        )  # pylint:disable=exec-used
    ...
    def test_new_var_inside_context_other_module(self):
        ...
        code = """\
from ...helpers import Context, ContextVar
with Context(VARIABLE=1):
...
class TestDtypes(unittest.TestCase):
    def test_dtypes_fields(self):
        ...
        self.assertTrue(
            all(
                issubclass(value.np, np.generic)
                for value in fields.values()
                if value.np is not None
            )
        )
...
class TestFetch(unittest.TestCase):
    ...
    def test_fetch_img(self):
        img = fetch(
            "https://media.istockphoto.com/photos/hen-picture-id831791190",
            allow_caching=False,
        )
