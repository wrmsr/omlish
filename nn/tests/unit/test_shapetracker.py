...
from ...shape.shapetracker import View
...
class CheckingShapeTracker:
    ...
    def flip(self, axis):
        self.st = self.st.stride(
            tuple(-1 if i in axis else 1 for i in range(len(self.shape)))
        )
    ...
    def assert_same(self):
        ...
        if DEBUG >= 1:
            print(
                x, y, self.st.shape, self.shape, idx.render(), valid.render(), self.st
            )
    ...
    def test_1(self):
        self.st = ShapeTracker(
            (
                ...
                View.create(
                    (1, 3, 2, 11, 26, 1, 1, 3), (0, 2464, 0, 112, 1, 0, 0, 29), 0, None
                ),
            )
        )
...
class TestIndexExpressions2d(unittest.TestCase):
    def setUp(self):
        shapes = [
            ...
            (5, 10),
            (5, 1),
        ]  # Make sure dim0 is a multiple of 5, one of the tests divides this dimension by 5
        offsets = [0, 1, 15, 28, 10000]
        self.sts = [
            ShapeTracker((View.create(base_shape, offset=offset),))
    ...
    def tearDown(self):
        for st, offset, shape, node_expr, idxs_expr in zip(
            self.sts, self.offset, self.shapes, self.node_exprs, self.idxs_exprs
        ):
            numel = prod(shape)
            ...
            idx0s = [
                (0, 0),
                (0, min(1, st.shape[0] - 1)),
                (0, st.shape[0] - 1),
                (min(3, st.shape[0] - 1), min(6, st.shape[0] - 1)),
                (st.shape[0] - 1, st.shape[0] - 1),
            ]
            idx1s = [
                (0, 0),
                (0, min(1, st.shape[1] - 1)),
                (0, st.shape[1] - 1),
                (min(3, st.shape[1] - 1), min(6, st.shape[1] - 1)),
                (st.shape[1] - 1, st.shape[1] - 1),
            ]
            idx2s = (
                [
                    (0, 0),
                    (0, min(1, st.shape[2] - 1)),
                    (0, st.shape[2] - 1),
                    (min(3, st.shape[2] - 1), min(6, st.shape[2] - 1)),
                    (st.shape[2] - 1, st.shape[2] - 1),
                ]
                if len(st.shape) == 3
                else [None for _ in idx0s]
            )
            for idx0, idx1, idx2 in product(idx0s, idx1s, idx2s):
                idxs = [
                    Variable(f"idx{i}", idx[0], idx[1])
    ...
    def test_noop(self):
        for st, base_shape, offset in zip(self.sts, self.shapes, self.offset):
            self.node_exprs.append(
                lambda idx, base_shape=base_shape, offset=offset: idx % prod(base_shape)
                + offset
            )
            self.idxs_exprs.append(
                lambda idxs, base_shape=base_shape, offset=offset: idxs[0]
                * base_shape[1]
                + idxs[1]
                + offset
            )
    ...
    def test_permute(self):
        ...
        for st, base_shape, offset in zip(self.sts, self.shapes, self.offset):
            st = st.permute((1, 0))
            self.node_exprs.append(
                lambda idx, base_shape=base_shape, offset=offset: idx
                % base_shape[0]
                * base_shape[1]
                + idx // base_shape[0] % base_shape[1]
                + offset
            )
            self.idxs_exprs.append(
                lambda idxs, base_shape=base_shape, offset=offset: idxs[0]
    ...
    def test_permute_reshape_1(self):  # This tests multiple views
        ...
        for st, base_shape, offset in zip(self.sts, self.shapes, self.offset):
            ...
            self.idxs_exprs.append(
                lambda idxs, base_shape=base_shape, offset=offset: (
                    idxs[0] * (base_shape[1] * 5) + idxs[2]
                )
                % base_shape[0]
                * base_shape[1]
                + (idxs[0] * (base_shape[1] * 5) + idxs[2]) // base_shape[0]
                + offset
            )
            new_st.append(st)
        self.sts = new_st
    ...
    def test_permute_reshape_2(self):
        ...
        for st, base_shape, offset in zip(self.sts, self.shapes, self.offset):
            ...
            self.idxs_exprs.append(
                lambda idxs, base_shape=base_shape, offset=offset: (
                    idxs[1] * (base_shape[1] * 5) + idxs[2]
                )
                % base_shape[0]
                * base_shape[1]
                + (idxs[1] * (base_shape[1] * 5) + idxs[2]) // base_shape[0]
                + offset
            )
            new_st.append(st)
        self.sts = new_st
    ...
    def test_splitting_big(self):
        ...
        assert len(self.st.views) == 1
        v = self.st.views[-1]
        assert v.strides == (15, 5, 1, 75, 15) and v.mask == (
            (0, 1),
            (0, 3),
            (0, 5),
            (0, 1),
    ...
    def test_combining_big(self):
        ...
        v = self.st.views[-1]
        assert (
            v.strides == (0, 0, 0, 1, 0, 0)
            and v.mask == ((0, 1), (0, 1), (0, 1), (30, 75), (0, 1), (0, 1))
            and v.offset == -30
        )
        self.st.assert_same()
...
class TestShapeTracker(unittest.TestCase):
    ...
    def test_permute(self):
        if len(self.st.shape) == 2:
            self.apply(lambda x: x.permute((1, 0)))
        elif len(self.st.shape) == 3:
            self.apply(lambda x: x.permute((2, 0, 1)))
    ...
    def test_stride_int(self):
        self.apply(lambda x: x.stride((1, 2)))

    def test_stride_2(self):
        self.apply(lambda x: x.stride((2, 2)))

    def test_stride_n(self):
        self.apply(lambda x: x.stride((-2, 1)))

    def test_stride_int_n(self):
        self.apply(lambda x: x.stride((-1, 2)))

    def test_stride_2_n(self):
        self.apply(lambda x: x.stride((-2, -2)))
