...
from ..helpers import colored, getenv, CI
from ..jit import TinyJit
import pytest

pytestmark = [
    pytest.mark.exclude_cuda,
    pytest.mark.exclude_gpu,
    pytest.mark.exclude_clang,
]
...
torch_device = torch.device(
    "mps" if getenv("MPS", 0) else ("cuda" if getenv("TORCHCUDA", 0) else "cpu")
)
...
def helper_test_speed(f1, *args):
    ...
    for i in range(CNT):
        ...
        args = [
            ...
            else (None if x is None else (x + 1))
            for x in args
        ]
        ...
        [
            x.numpy()
            if isinstance(x, Tensor) or str(torch_device) == "cpu"
            else x.cpu().numpy()
            for x in args
            if x is not None
        ]
        ...
        if isinstance(args[0], Tensor):
            Device[args[0].device].synchronize()
        else:
            sync()
...
def helper_test_generic_square(name, N, f1, f2, onearg=False):
    ...
    torch_b = (
        (torch.rand(N, N, dtype=torch_dt) - 0.5).to(torch_device)
        if not onearg
        else None
    )
    ...
    helper_test_generic(
        f"{name:30s} {N:5d}x{N:5d}",
        f1,
        (torch_a, torch_b),
        TinyJit(lambda a, b: f2(a, b).realize()),
        (tiny_a, tiny_b),
    )
...
def helper_test_matvec(name, N, M):
    ...
    helper_test_generic(
        f"{name:30s} {N:5d}x{M:5d}",
        lambda a, b: a @ b,
        (torch_a, torch_b),
        TinyJit(lambda a, b: (a @ b).realize()),
        (tiny_a, tiny_b),
    )
...
def helper_test_generic(name, f1, f1_args, f2, f2_args):
    ...
    print(
        ("\r" if not CI else "")
        + f"{name:42s} {et_torch:7.2f} ms ({flops/et_torch:8.2f} GFLOPS {mem/et_torch:8.2f} GB/s) in torch, {et_tinygrad:7.2f} ms ({flops/et_tinygrad:8.2f} GFLOPS {mem/et_tinygrad:8.2f} GB/s) in tinygrad, {colorize_float(et_tinygrad/et_torch)} {desc} {flops:10.2f} MOPS {mem:8.2f} MB"
    )  # noqa: E501
...
def helper_test_conv(bs, in_chans, out_chans, kernel_size, img_size_y, img_size_x):
    torch.manual_seed(0)
    torch_dat = torch.rand(bs, in_chans, img_size_y, img_size_x, dtype=torch_dt).to(
        torch_device
    )
    torch_conv = torch.nn.Conv2d(
        in_chans, out_chans, kernel_size, bias=None, dtype=torch_dt
    ).to(torch_device)
    ...
    helper_test_generic(
        f"conv bs:{bs:3d} chans:{in_chans:3d} -> {out_chans:3d} k:{kernel_size}",
        f1,
        (torch_dat,),
        TinyJit(f2),
        (tiny_dat,),
...
class TestSpeed(unittest.TestCase):
    ...
    def test_cat(self):
        helper_test_generic_square(
            "cat_0",
            256,
            lambda x, y: torch.cat((x, y), dim=0),
            lambda x, y: x.cat(y, dim=0),
        )
        helper_test_generic_square(
            "cat_1",
            256,
            lambda x, y: torch.cat((x, y), dim=1),
            lambda x, y: x.cat(y, dim=1),
        )
    ...
    def test_double_permute(self):
        ...
        helper_test_generic(
            f"double_permute {tiny_a.shape}",
            f,
            (torch_a,),
            TinyJit(lambda a: f(a).realize()),
            (tiny_a,),
        )
    ...
    def test_gemm_unrolled(self):
        ...
        def f2(a, b):
            return (
                a.reshape(N, 1, N).expand(N, N, N) * b.reshape(1, N, N).expand(N, N, N)
            ).sum(axis=2)
    ...
    def test_gemm_unrolled_permute_l(self):
        ...
        def f2(a, b):
            return (
                a.permute(1, 0).reshape(N, 1, N).expand(N, N, N)
                * b.reshape(1, N, N).expand(N, N, N)
            ).sum(axis=2)
    ...
    def test_gemm_unrolled_permute_r(self):
        ...
        def f2(a, b):
            return (
                a.reshape(N, 1, N).expand(N, N, N)
                * b.permute(1, 0).reshape(1, N, N).expand(N, N, N)
            ).sum(axis=2)
    ...
    def test_openpilot_conv2d(self):
        ...
        torch_conv = torch.nn.Conv2d(
            in_chans, out_chans, 3, bias=None, padding=1, dtype=torch_dt
        ).to(torch_device)
        ...
        helper_test_generic(
            f"conv bs:{bs:3d} chans:{in_chans:3d} -> {out_chans:3d} k:3",
            f1,
            (torch_dat,),
            TinyJit(f2),
            (tiny_dat,),
