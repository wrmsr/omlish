...
class FakeAllocator(LRUAllocator):
    def _do_alloc(self, size, dtype, device, **kwargs):
        if size * dtype.itemsize > self._get_cur_free_space(device):
            raise Exception("OOM")
...
class FakeBuffer(RawBuffer):
    def __init__(self, size, dtype, device="0"):
        ...
        super().__init__(
            size, dtype, allocator=FAKE_GLOBAL_ALLOCATOR, **{"device": device}
        )
        assert (
            self._buf.size == size
            and self._buf.dtype == dtype
            and self._buf.device == device
        ), "This allocator requires 100% match of dtype and size."

    @classmethod
    def fromCPU(cls, x: np.ndarray, **kwargs):
        return cls(prod(x.shape), dtypes.from_np(x.dtype), **kwargs)

    def toCPU(self):
        return np.empty(self.size, dtype=self.dtype.np)
...
def cmp_trace_and_buf(buf, trace_ref):
    return trace_ref and trace_ref() == buf._buf
...
class TestAllocators(unittest.TestCase):
    def test_lru_allocator_reusage(self):
        ...
        def test():
            ...
            assert (
                GlobalCounters.mem_cached - mc == 16 * dtypes.float32.itemsize
            ...
            for _ in range(32):
                ...
                __test()
            assert (
                GlobalCounters.mem_used - mu == 16 * dtypes.float32.itemsize
            ), "Only usedbuf is still allocated."
    ...
    def test_lru_allocator_cache_free(self):
        ...
        def test():
            ...
            for sz in range(1, 32):
                ...
                assert (
                    GlobalCounters.mem_used + GlobalCounters.mem_cached - mc - mu <= 128
                ), "Should not allocate on device more than allowed (128)"
            for r in refs:
                assert (
                    r() is None
                ), "All refs should be dead, since buffers were cleared from cache"
    ...
    def test_lru_allocator_multidevice(self):
        def test():
            ...
            for i in range(8):
                refs.append(
                    alloc_free_trace(lru_allocator, 16, dtypes.float32, device=str(i))
                )
            for i in range(64):
                ...
                def __test():
                    ...
                    assert cmp_trace_and_buf(
                        buf, refs[i % 8]
                    ), "Buffer should be reused"

                __test()
            for r in refs:
                assert r() is not None, "All refs should be cached"
    ...
    def test_lru_allocator_failing_alloc_cleans_cache(self):
        def test():
            ...
            assert (
                len(lru_allocator.aging_order["0"]) == 3
            ), "All buffers should be cached"
            assert (
                lru_allocator.free_space["0"] == 128 - 24
            ), "24 bytes to be used by current cached buffers"
            ...
            assert (
                len(lru_allocator.aging_order["0"]) == 0
            ), "All buffers should be freed from cache due to failing alloc"
    ...
    def test_lru_allocator_fail_first_alloc_pass_after_clear_cahce(self):
        def test():
            ...
            cache_length = 3
            assert (
                len(lru_allocator.aging_order["0"]) == cache_length
            ), "All buffers should be cached"
            assert (
                lru_allocator.free_space["0"] == 128 - 24
            ), "24 bytes to be used by current cached buffers"
            ...
            def single_fail_then_pass(*args, **kwargs):
                lru_allocator._do_alloc = (
                    original_do_alloc  # restore the original method
                )
                raise MemoryError("OOM")
            ...
            assert (
                len(lru_allocator.aging_order["0"]) < cache_length
            ), "Some buffers should be cleaned as first alloc failed"
    ...
    def test_lru_allocator_massive_buffer(self):
        with self.assertRaises(AssertionError) as context:
            alloc(allocator := FakeAllocator(), size := 1e13, dtypes.int8)
        self.assertEqual(
            str(context.exception),
            f"out of memory - requested: {size/1e9:5.2f} GB, available: {allocator._get_cur_free_space('0')/1e9:5.2f} GB",
        )

    @unittest.skipIf(Device.DEFAULT != "METAL", "only applies to Metal")
    def test_lru_allocator_metal_max_buffer_length(self):
        from ..runtime.ops_metal import METAL

        with self.assertRaises(AssertionError) as context:
            METAL.allocator._do_alloc(
                buf_len := (max_buf_len := METAL.device.maxBufferLength() + 1),
                dtypes.int8,
                "0",
            )
        self.assertEqual(
