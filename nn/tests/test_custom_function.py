...
from ..lazy import create_lazybuffer
...
def atan2_gpu(ret: LazyBuffer, a: LazyBuffer, b: LazyBuffer):
    ...
    ).build(Device[ret.device].compiler, Device[ret.device].runtime).exec(
        [ret.realized, a.realized, b.realized]
    )


def atan2_cpu(ret: LazyBuffer, a: LazyBuffer, b: LazyBuffer):
    ret.realized._copyin(np.arctan2(a.realized._buf, b.realized._buf))
...
from ..ops import LazyOp, LoadOps, BinaryOps
from ..lazy import LazyBuffer
...
class ATan2(Function):
    def forward(self, a: LazyBuffer, b: LazyBuffer) -> LazyBuffer:
        ...
        return create_lazybuffer(
            a.device,
            ShapeTracker.from_shape(a.shape),
            LoadOps,
            ast,
            max(a.dtype, b.dtype),
        )
    ...
    def backward(
        ...
        denom = (self.a.e(BinaryOps.MUL, self.a)).e(
            BinaryOps.ADD, self.b.e(BinaryOps.MUL, self.b)
        )
        return (
            grad_output.e(BinaryOps.MUL, self.b.e(BinaryOps.DIV, denom))
            if self.needs_input_grad[0]
            else None,
            grad_output.e(
                BinaryOps.MUL,
                self.a.const(0).e(BinaryOps.SUB, self.a).e(BinaryOps.DIV, denom),
            )
