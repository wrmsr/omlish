...
class TestSymbolic(unittest.TestCase):
    ...
    def test_cat_dim0_strides(self):
        ...
        t = (
            Tensor.rand(3, 4)
            .reshape(i, 4)
            .cat(Tensor.rand(3, 4).reshape(j, 4), dim=0)
            .cat(Tensor.rand(3, 4).reshape(k, 4), dim=0)
        )
        st = t.lazydata.st
        ...
        t = (
            Tensor.rand(3, 3)
            .reshape(i, 3)
            .cat(Tensor.rand(3, 3).reshape(i, 3), dim=0)
            .cat(Tensor.rand(3, 3), dim=0)
        )
        st = t.lazydata.st
    ...
    def test_cat_dim1_strides(self):
        ...
        t = (
            Tensor.rand(3, 4)
            .reshape(3, i)
            .cat(Tensor.rand(3, 4).reshape(3, j), dim=1)
            .cat(Tensor.rand(3, 4).reshape(3, k), dim=1)
        )
        st = t.lazydata.st
...
class TestSymbolicVarVals(unittest.TestCase):
    ...
    def test_var_vals_mask(self):
        x = Variable("x", 1, 100).bind(3)
        view = View.create(
            shape=(3, 4), strides=(4, 1), offset=0, mask=((0, x), (0, 4))
        )
...
class TestShapeTrackerUnbind(unittest.TestCase):
    ...
    def test_shrink_unbind(self):
        ...
        assert t.lazydata.st.unbind() == ShapeTracker(
            (View.create(shape=(1, 4), offset=4 * v),)
        )
...
class TestSymbolicReshape(unittest.TestCase):
    ...
    def test_reshape_into_symbols_bad_shape(self):
        ...
        with self.assertRaises(ValueError):
            Tensor.rand(4, 6).reshape(vi, 6).reshape(
                1, 77
            )  # reshape to a different size new shape through symbolic shape
    ...
    def test_symbolic_mask(self):
        ...
        view = View(
            ...
            mask=(
                (0, 1),
                (
                    Variable("start_pos", 1, 128).bind(2),
                    (NumNode(1) + Variable("start_pos", 1, 128).bind(2)),
                ),
                (0, 16),
                (0, 64),
            ),
        ...
        view = View(
            ...
            mask=(
                (1, 2),
                (0, 1),
                (0, (NumNode(1) + Variable("start_pos", 1, 128))),
                (0, 16),
                (0, 64),
            ),
            contiguous=False,
