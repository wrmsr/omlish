...
from ..nn import optim  # noqa: F401
from ..nn import state
...
class BatchNorm2d:
    ...
    def __call__(self, x: Tensor):
        if Tensor.training:
            ...
            batch_invstd = (
                self.running_var.reshape(1, -1, 1, 1)
                .expand(x.shape)
                .add(self.eps)
                .rsqrt()
            )
...
class GroupNorm:
    ...
    def __call__(self, x: Tensor):
        ...
        return x * self.weight.reshape(
            1, -1, *[1] * (len(x.shape) - 2)
        ) + self.bias.reshape(1, -1, *[1] * (len(x.shape) - 2))
