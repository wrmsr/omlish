...
class Tensor:
    ...
    def assign(self, x) -> Tensor:
        ...
        if (
            ...
            x.lazydata.output_buffer = self.lazydata.realized  # noqa: E501
    ...
    def shrink(self, arg: Tuple[Optional[Tuple[sint, sint]], ...]) -> Tensor:
        if not any(x is not None and x != (0, s) for x, s in zip(arg, self.shape)):
            return self
        return mlops.Shrink.apply(
            self,
            arg=tuple(x if x is not None else (0, s) for x, s in zip(arg, self.shape)),
        )
    ...
    # TODO: update docs
    def __getitem__(
        self, indices: Union[int, slice, Tensor, None, List, Tuple]
    ) -> Tensor:  # no ellipsis type...
        ...
        if slice in type_dim and self.ndim == 0:
            raise IndexError("slice cannot be applied to a 0-dim tensor.")
        if len(ellipsis_idx) > 1:
            raise IndexError("an index can only have a single ellipsis ('...')")
        if float in type_dim:
            raise IndexError("float type is not valid index")
        if any(isinstance(i, slice) and i.step == 0 for i in indices):
        ...
        for dim in type_dim[int]:
            if (
                indices_filtered[dim] >= self.shape[dim]
                or indices_filtered[dim] < -self.shape[dim]
            ):
                raise IndexError(
                    f"index {indices_filtered[dim]} is out of bounds for dimension {dim} with size {self.shape[dim]}"
                )
        ...
        for dim in (
            dims_collapsed := [
                dim + sum(1 for d in type_dim[None] if dim >= d)
                for dim in reversed(type_dim[int])
            ]
        ):
            new_shape.pop(dim)
        for dim_sh in new_shape:
            assert isinstance(dim_sh, int), f"does not support symbolic shape {dim_sh}"
        ...
        if type_dim[Tensor]:
            ...
            for tensor_dim in type_dim[Tensor]:
                dims_collapsed_, dims_injected = sum(
                    1 for d in dims_collapsed if tensor_dim >= d
                ...
                idx.append(
                    ((t := indices[tensor_dim + dims_injected]) < 0).where(
                        ret.shape[td], 0
                    )
                    + t
                )
            ...
            arange = [
                ...
                for n, (sd, d) in enumerate(zip(sum_dim, tdim))
            ]  # noqa: E501
            first_idx = [
                idx[0].reshape(
                    *[1] * tdim[0],
            ...
            if (
                tdim[0] != 0
                and len(tdim) != 1
                and tdim != list(range(tdim[0], tdim[-1] + 1))
            ):
                ret_dims = list(range(ret.ndim))
                ret = ret.permute(
                    ret_dims[tdim[0] : tdim[0] + max_dim]
                    + ret_dims[: tdim[0]]
                    + ret_dims[tdim[0] + max_dim :]
                )
        return ret
    ...
    def _reduce(
        ...
        if 0 in self.shape and 0 not in shape:
            return Tensor.full(
                ...
                {mlops.Sum: 0.0, mlops.Max: -float("inf")}[fxn],
            )
        ret = fxn.apply(
            self,
    ...
    def einsum(formula: str, *raw_xs) -> Tensor:
        ...
        all_letter_val = sorted(
            merge_dicts(
                [
                    {letter: dim for letter, dim in zip(letters, tensor.shape)}
        ...
        for x, (order, letters) in zip(xs, [list(zip(*l)) for l in lhs]):
            ...
            xs_.append(
                x.permute(order)
                .reshape(
                    [val if letter in letters else 1 for letter, val in all_letter_val]
                )
                .expand([val for _, val in all_letter_val])
            )

        rhs_order, rhs_letters = tuple(
            zip(*sorted(enumerate(output), key=lambda e: e[1]))
        ) or ([], [])
        # sum over all axes that's not in the output, then permute to the output order
        return (
            reduce(lambda a, b: a * b, xs_)
            .sum(
                axis=[
                    axis
                    for axis, (letter, _) in enumerate(all_letter_val)
                    if letter not in rhs_letters
                ]
            )
            .permute(rhs_order)
        )
    ...
    def conv2d(
        ...
        assert groups * cin == cin_ and len(self.shape) == len(
            weight.shape
        ), f"Input Tensor shape {self.shape} does not match the shape of the weights {weight.shape}. ({groups*cin} vs. {cin_})"  # noqa: E501
        if isinstance(padding, (tuple, list)):
            assert len(padding) == 2 * len(HW) or len(padding) == len(
                HW
            ), f"Expected padding of length {2*len(HW)} or {len(HW)}, but got {len(padding)} for tensor of shape {self.shape}"  # noqa: E501
        padding_ = (
            [padding] * 2 * len(HW)
    ...
    def dot(self, w: Tensor) -> Tensor:
        ...
        ), f"Input Tensor shapes {self.shape} and {w.shape} cannot be multiplied ({self.shape[-1]} != {w.shape[-min(n2, 2)]})"  # noqa: E501
        x = self.reshape(
    ...
    def _broadcasted(
        ...
        if len(y.shape) < len(x.shape):
            y = y.reshape((1,) * (len(x.shape) - len(y.shape)) + y.shape)
        elif len(x.shape) < len(y.shape):
            x = x.reshape((1,) * (len(y.shape) - len(x.shape)) + x.shape)
    ...
    def pow(self, x: Union[Tensor, float], reverse=False) -> Tensor:
        ...
        if not isinstance(x, Tensor) and not reverse:
            ...
            if x in [3, 2, 1, 0]:
                return reduce(
                    lambda acc, _: acc * self, range(int(x)), mlops.Zero.apply(self) + 1
                )
            if x == 0.5:
                return self.sqrt()
    ...
    def xor(self, x: Tensor, reverse=False) -> Tensor:
        return mlops.Xor.apply(*self._broadcasted(x, reverse))
...
if IMAGE:
    ...
    from .features.image import image_conv2d, image_dot
...
def custom_random(out: Buffer):
    ...
    if DEBUG >= 2:
        print(
            f"*** {out.device}   rand  seed {Tensor._seed} size {out.size:<15d} dtype {out.dtype}"
        )
