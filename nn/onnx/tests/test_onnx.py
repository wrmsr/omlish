...
class TestOnnxModel(unittest.TestCase):
    def test_benchmark_openpilot_model(self):
        ...
        for _ in range(7):
            ...
            if not CI:
                print(
                    f"ran openpilot model in {(et - st) * 1000.0:.2f} ms, "
                    f"waited {(mt2 - mt) * 1000.0:.2f} ms for realize, {(et - mt2) * 1000.0:.2f} ms for GPU queue"
                )
        ...
        if not CI:
            ...
            stats.dump_stats(temp_file("net.prof"))
            os.system(f"flameprof {temp_file('net.prof')} > {temp_file('prof.svg')}")
