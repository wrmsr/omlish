...
def Clip(input: Tensor, min=None, max=None):
    return input.clip(float('-inf') if min is None else min, float('inf') if max is None else max)
...
def _axes(axes, noop_with_empty_axes):
    if axes is not None and not (isinstance(axes, Tensor) and axes.shape == (0,)):
        return [int(x) for x in safe_numpy(axes)]
    elif noop_with_empty_axes:
        return []
    else:
        return None
...
def Range(start: Tensor, limit, delta):
    return Tensor.arange(
        start=int(safe_numpy(start)),
        stop=int(safe_numpy(limit)),
        step=int(safe_numpy(delta))
...
def Shape(data: Tensor, end=None, start=0):
    return Tensor(
        ...
        dtype=dtypes.int32 if os.path.isfile( "/TICI") else dtypes.int64,  # TODO: really?
    )
...
def Reshape(data: Tensor, shape: Tensor, allowzero=None):
    return data.reshape([int(x) if x != 0 else data.shape[i] for i, x in enumerate(safe_numpy(shape))])
...
def Trilu(x: Tensor, k: ta.Union[Tensor, int] = 0, upper=1):
...
def Transpose(input: Tensor, perm=None):
    return input.permute(order=list(range(len(input.shape))[::-1]) if perm is None else perm)
...
def Gemm(
    ...
    if C is not None:
        ret += beta * (C if broadcast == 0 else C.reshape([-1 if i < len(C.shape) else 1 for i in range(len(ret.shape))][::-1]))
    return ret
...
def LayerNormalization(x: Tensor, scale, bias, axis=-1, epsilon=1e-05, stash_type=1):
    ...
    return (
        ...
        mean,
        (x.sub(mean)).pow(2).mean(axis=axis, keepdim=True).add(epsilon).sqrt().reciprocal(),
    )


def GroupNormalization(x: Tensor, scale: Tensor, bias: Tensor, num_groups, epsilon=1e-05):
    return x\
        .reshape(x.shape[0], num_groups, -1)\
...
def _format_padding(onnx_pads, ndims=None, axes=None):
    if ndims and len(
        onnx_pads) // 2 != ndims:  onnx_pads = onnx_pads * ndims  # for OnnxBackendPyTorchConvertedModelTest the len(onnx_pads) == 2
    if ndims is None:
...
def _padding(
    ...
    if auto_pad != "NOTSET":
        pads = _auto_pad(X, auto_pad, strides, kernel_shape, dilations, ceil_mode)
    if pads is None:
        return X
...
def _auto_pad(
    ...
    strides = [strides] * len(kernel_shape) if isinstance(strides, int) else \
    ...
    if auto_pad == "SAME_UPPER" or auto_pad == "SAME_LOWER":
        pad_shape = [
            ...
            for sh, st, ks, di in zip(X.shape[-len(kernel_shape):], strides, kernel_shape, dilations)
        ]
        ...
        out_spatial_shape = [
            math.ceil((sh - ((ker - 1) * dil + 1)) / st) + 1 if ceil_mode else math.floor((sh - ((ker - 1) * dil + 1)) / st) + 1
            for sh, st, ker, dil in zip(X.shape[-len(kernel_shape):], strides, kernel_shape, dilations)
        ]
        pad_shape = [
            (osh - 1) * st + ((ks - 1) * dil + 1) - ish
            for osh, st, ks, dil, ish in zip(out_spatial_shape, strides, kernel_shape, dilations, X.shape[-len(kernel_shape):])
        ]
        pad_shape = flatten([[sh // 2, sh - sh // 2] for sh in pad_shape])
...
def Pad(
        x: Tensor,
        pads: ta.Union[Tensor, tuple[int, ...]],
    ...
    if mode == "wrap":
        ...
        shrink_args = [
            (sh - dim[0] % sh if dim[0] % sh != 0 else 0, nsh - (sh - dim[1] % sh) if dim[1] % sh != 0 else nsh)
        ...
        for i, s in enumerate(x.shape):
            if pads[i] == (0, 0):
                ...
                x = (
                        x
                        .flip(i)
                        .shrink(tuple([(0, s_) if i_ != i else (s - pads[i][0] - 1, s_ - 1) for i_, s_ in enumerate(x.shape)]))
                        .pad(tuple([(0, 0) if i_ != i else (0, s) for i_ in range(x.ndim)]))
                ) + x.pad(tuple([(0, 0) if i_ != i else pads[i] for i_ in range(x.ndim)]))
            elif not pads[i][0] and pads[i][1]:
                x = (
                        x
                        .flip(i)
                        .shrink(tuple([(0, s_) if i_ != i else (1, pads[i][1] + 1) for i_, s_ in enumerate(x.shape)]))
                        .pad(tuple([(0, 0) if i_ != i else (s, 0) for i_ in range(x.ndim)]))
                ) + x.pad(tuple([(0, 0) if i_ != i else pads[i] for i_ in range(x.ndim)]))
            else:
                x = (
                        x
                        .flip(i)
                        .shrink(tuple([(0, s_) if i_ != i else (s - pads[i][0] - 1, s_ - 1) for i_, s_ in enumerate(x.shape)]))
                        .pad(tuple([(0, 0) if i_ != i else (0, s + pads[i][1]) for i_ in range(x.ndim)]))
                ) + (
                        x
                        .flip(i)
                        .shrink(tuple([(0, s_) if i_ != i else (1, pads[i][1] + 1) for i_, s_ in enumerate(x.shape)]))
                        .pad(tuple([(0, 0) if i_ != i else (s + pads[i][0], 0) for i_ in range(x.ndim)]))
                ) + x.pad(tuple([(0, 0) if i_ != i else pads[i] for i_ in range(x.ndim)]))
        return x
        ...
        for i, s in enumerate(x.shape):
            if pads[i] == (0, 0):
                ...
                x = (
                        x
                        .shrink(tuple([(0, s_) if i_ != i else (0, 1) for i_, s_ in enumerate(x.shape)]))
                        .expand([pads[i][0] if i_ == i else s_ for i_, s_ in enumerate(x.shape)])
                        .pad(tuple([(0, 0) if i_ != i else (0, s) for i_ in range(x.ndim)]))
                ) + x.pad(tuple([(0, 0) if i_ != i else pads[i] for i_ in range(x.ndim)]))
            elif not pads[i][0] and pads[i][1]:
                x = (
                        x
                        .shrink(tuple([(0, s_) if i_ != i else (s_ - 1, s_) for i_, s_ in enumerate(x.shape)]))
                        .expand([pads[i][0] if i_ == i else s_ for i_, s_ in enumerate(x.shape)])
                        .pad(tuple([(0, 0) if i_ != i else (s + pads[i][0], 0) for i_ in range(x.ndim)]))
                ) + x.pad(tuple([(0, 0) if i_ != i else pads[i] for i_ in range(x.ndim)]))
            else:
                x = (
                        ...
                        x
                        .shrink(tuple([(0, s_) if i_ != i else (s_ - 1, s_) for i_, s_ in enumerate(x.shape)]))
                        .expand([pads[i][1] if i_ == i else s_ for i_, s_ in enumerate(x.shape)])
                        .pad(tuple([(0, 0) if i_ != i else (s + pads[i][0], 0) for i_ in range(x.ndim)]))
                ) + x.pad(tuple([(0, 0) if i_ != i else pads[i] for i_ in range(x.ndim)]))
        return x
...
def AveragePool(
    ...
    if dilations != 1:
        raise NotImplementedError(f"dilations != 1 not supported, dilations:{dilations}")
    ...
    padding_included = _padding(
        ...
        dilations=dilations
    ).avg_pool2d(kernel_shape, stride=strides)
...
def MaxPool(
    ...
    if ceil_mode and auto_pad == "NOTSET":
        auto_pad = "VALID"
    ret = _padding(
        X,
        pads,
        auto_pad, constant_value=float("-inf"),
        axes=tuple(range(len(X.shape)))[-len(kernel_shape):],
        strides=strides,
        kernel_shape=kernel_shape,
        dilations=dilations,
        ceil_mode=ceil_mode,
    ...
    indices = (
            (ret.flatten().unsqueeze(1).expand(ret_len, X_len) == X.flatten().reshape(1, X_len).expand(ret_len, X_len))
            * Tensor.arange(X_len).reshape(1, X_len).expand(ret_len, X_len)
    ).sum(1).reshape(ret.shape).cast(dtypes.int64)
    if storage_order:
...
def Conv(
    ...
    if auto_pad != "NOTSET":
        ...
        padding = [
            p
            for ps in zip(pads[:len(pads) // 2][::-1], pads[len(pads) // 2:][::-1])
            for p in ps
        ] if pads is not None else 0  # reorder padding
    return X.conv2d(
        W,
        B,
        stride=strides,
...
def ConvTranspose(
    ...
    if pads is None and auto_pad != "NOTSET":
        pads = _auto_pad(X, auto_pad, strides, kernel_shape, dilations)

    elif pads is None and auto_pad == "NOTSET":
        pads = [0, 0] * (X.ndim - 2)
    ...
    if output_shape and not output_padding:
        out_sh = [
            st * (xs - 1) + (ks - 1) * di + 1 if n < 2 else st * (xs - 1) + (ks - 1) * di + 1 - pads[n - 2] - pads[ n - 1]
            for n, (st, xs, ks, di) in enumerate(zip(strides_, X.shape, kernel_shape, dilations_))
        ]
...
def Dropout(data: Tensor, ratio=0.5, training_mode=False, seed=None):
    if isinstance(ratio, Tensor) and not ratio.shape:
        ratio = safe_numpy(ratio)  # ratio and tensor is passed in as Tensor with shape: ()
...
def LRN(input: Tensor, size, alpha=1e-4, beta=0.75, bias=1.0):
    ...
    return input / (
        input
        .mul(input)
        .reshape(bs, 1, c, iy * ix)
        .pad2d((0, 0, (size - 1) // 2, size // 2))
        .avg_pool2d((size, 1), 1)
        .reshape(bs, c, iy, ix)
        .mul(alpha)
        .add(bias)
...
def SoftmaxCrossEntropyLoss(scores: Tensor, labels: Tensor, weights=None, ignore_index=None, reduction="mean"):
    ...
    if weights is not None:
        weights = weights.__getitem__(tuple([labels, *[slice(None)] * (weights.ndim - 1)]))

    loss = (mask * -y).sum(1) if weights is None else (mask * -y).sum(1) * weights
    if reduction == "mean":
        loss = loss.sum() / (loss == 0).where(0, 1).sum() if weights is None else loss.sum() / weights.sum()
    elif reduction == "sum":
        loss = loss.sum()
    return loss, y
    ...
    if mode == "nearest":
        ...
        for y in safe_numpy(y_out):
            for x in safe_numpy(x_out):
                ...
                y_shrink = (0, X.shape[2]) if X.shape[2] == 1 else (y_floor, y_floor + 2) if y != y_floor else (
                y_floor, y_floor + 1)
                ...
                if x == x_floor and y == y_floor:  # TODO https://en.wikipedia.org/wiki/Bilinear_interpolation#Weighted_mean maybe do weighted mean?
                    ...
                    ret.append((corners[0, 0, 0, 0] * (x2 - x) * (y2 - y) + corners[0, 0, 0, 1] * (x - x1) * (y2 - y) + corners[0, 0, 1, 0] * (x2 - x) * (y - y1) + corners[0, 0, 1, 1] * (x - x1) * (
                                            y - y1)) / ((x2 - x1) * (y2 - y1)))
...
def CenterCropPad(input: Tensor, shape: Tensor, axes=None):
    ...
    for s, x in zip(shape, axes):
        if s < input.shape[x]:
            shrink_arg[x] = (input.shape[x] // 2 - s // 2, input.shape[x] // 2 + s // 2) if s % 2 == 0 else (
            input.shape[x] // 2 - s // 2 - 1, input.shape[x] // 2 + s // 2)
        elif s > input.shape[x]:
            pad_arg[x] = ((s - input.shape[x]) // 2, (s - input.shape[x]) // 2) if (s - input.shape[x]) % 2 == 0 else ((s - input.shape[x]) // 2, (s - input.shape[x]) // 2 + 1)
...
def DequantizeLinear(x: Tensor, x_scale: Tensor, x_zero_point: ta.Union[Tensor, int] = 0, axis=1):
    axis = axis + x.ndim if axis < 0 else axis
    ...
    x_zer = x_zero_point.reshape(*[1] * axis, *x_scale.shape, *[1] * (x.ndim - axis - x_scale.ndim)) if isinstance(
        x_zero_point, Tensor) else x_zero_point
...
def AffineGrid(theta: Tensor, size: Tensor, align_corners=0):
    ...
    for dim, dim_sz in enumerate(data_sz):
        ...
        if dim == 0:
            stackable = [a.reshape(dim_sz, *[1] * (len(data_sz) - 1)) + size_zeros, *stackable]
        elif dim == 1:
            stackable = [a.reshape(1, dim_sz, *[1] * (len(data_sz) - 2)) + size_zeros, *stackable]
        else:
...
def FastGelu(x: Tensor, bias: ta.Optional[Tensor] = None):
    x = x + bias
...
def EmbedLayerNormalization(
        input_ids: Tensor,
        segment_ids: ta.Optional[Tensor] = None,
        word_embedding: Tensor = None,
        position_embedding: Tensor = None,
        segment_embedding: ta.Optional[Tensor] = None,
        gamma=None,
        beta=None,
        mask: ta.Optional[Tensor] = None,
        position_ids: ta.Optional[Tensor] = None,
        epsilon=None,
...
def Attention(
        ...
        weights,
        bias: ta.Optional[Tensor] = None,
        mask_index: ta.Optional[Tensor] = None,
        past: ta.Optional[Tensor] = None,
        relative_position_bias: ta.Optional[Tensor] = None,
        past_sequence_length: ta.Optional[Tensor] = None,
        do_rotary=None,
