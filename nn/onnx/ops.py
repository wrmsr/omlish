...
def _padding(
    ...
    if auto_pad != "NOTSET":
        ...
        out_spatial_shape = [
            math.ceil((sh - dil * (ker - 1) - 1) / st + 1) if ceil_mode else math.floor((sh - dil * (ker - 1) - 1) / st + 1)
...
def ConvTranspose(
    ...
    ] if output_shape is not None or auto_pad != "NOTSET" else []
    if pads is None:
        ...
        if auto_pad == "NOTSET":
            ...
            total_padding = [
                ...
                for st, ish, pad, ks, dil, osh in
                zip(strides, X.shape[2:], output_padding, kernel_shape, dilations, output_shape)
            ]
