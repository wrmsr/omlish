...
def arch():
    return "sm_" + "".join(
        [str(x) for x in pycuda.driver.Context.get_device().compute_capability()]
    )
...
if getenv("CUDACPU", 0) == 1:
    ...
    class cuda:
            ...
            def __call__(self, *args, block, grid, shared):
                lib.ptx_run(
                    ...
                    (ctypes.c_void_p * len(args))(
                        *[ctypes.cast(x, ctypes.c_void_p) for x in args]
                    ),
                    *block,
                    *grid,
                    shared,
        ...
        module_from_buffer = lambda src: cuda.module(
            src
        )  # pylint: disable=unnecessary-lambda # noqa: E731
    ...
    class RawCUDABuffer(RawBufferCopyInOut):  # type: ignore
        ...
        def _copyin(self, x: np.ndarray, stream: Optional[cuda.Stream] = None):
...
def compile_cuda(prg) -> bytes:
    return cuda_compile(
        prg, target="ptx", no_extern_c=True, options=["-Wno-deprecated-gpu-targets"]
    )
        ...
        if DEBUG >= 6:
            try:
                fn = (
                    ...
                    / f"tinycuda_{hashlib.md5(prg.encode('utf-8')).hexdigest()}"
                ).as_posix()
                with open(fn + ".ptx", "wb") as f:
                    f.write(prg.encode("utf-8"))
                subprocess.run(
                    ["ptxas", f"-arch={arch()}", "-o", fn, fn + ".ptx"], check=True
                )
                print(subprocess.check_output(["nvdisasm", fn]).decode("utf-8"))
            except Exception as e:
                print("failed to generate SASS", str(e))
        # TODO: name is wrong, so we get it from the ptx using hacks
        self.prg = cuda.module_from_buffer(prg.encode("utf-8")).get_function(
            prg.split(".visible .entry ")[1].split("(")[0]
        )
    ...
    def __call__(
        ...
        *args,
        ...
        self.prg(
            *[
                x._buf
                if isinstance(x, RawCUDABuffer)
                else np.int32(x)
                if (isinstance(x, int) and not getenv("CUDACPU"))
                else x
                for x in args
            ],
...
CUDADevice = Compiled(
    RawCUDABuffer,
    LinearizerOptions(
        supports_float4=False if getenv("PTX") else True,
