...
class RawTorchBuffer(RawBuffer):
    ...
    def _copyin(self, x):
        buf = (
            torch.from_numpy(x if all(s >= 0 for s in x.strides) else x.copy())
    ...
    def _get_buf(self):
        return (
            ...
            if self._buf is not None
                ...
                [self.size], device=device, dtype=inverse_type_map[self.dtype]
            )
        )
...
torch_fxn_for_op: Dict[Op, Callable] = {
    ...
    BufferOps.LOAD: lambda x: x._get_buf(),
    BufferOps.STORE: lambda x: RawTorchBuffer(prod(x.shape), type_map[x.dtype], x),
