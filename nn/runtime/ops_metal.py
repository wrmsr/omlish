...
class MetalAllocator(LRUAllocator):
    def _do_alloc(self, size, dtype, device, **kwargs):
        ...
        buf = METAL.device.newBufferWithLength_options_(
            buf_len, Metal.MTLResourceStorageModeShared
        )
...
class MetalGraph:
    def __init__(
        ...
        for j, ji in enumerate(self.jit_cache):
            ...
            pipeline_state = unwrap(
                METAL.device.newComputePipelineStateWithDescriptor_options_reflection_error_(
            ...
            for i, v in enumerate(prg.vars):
                icb_command.setKernelBuffer_offset_atIndex_(
                    self.int_buf._buf, var_vals_keys.index(v) * 4, len(ji.rawbufs) + i
                )
        ...
        self.read_resources, self.write_resources = dedup(read_resources), dedup(
            write_resources
        )
        self.command_buffer: Any = None
        self.int_buf_view = (
            self.int_buf.toCPU()
        )  # TODO: this is metal syncing when it doesn't need to
    ...
    def __call__(
        ...
        if (
            ...
            and self.command_buffer in METAL.mtl_buffers_in_flight
        ):
        ...
        encoder.useResources_count_usage_(
            all_read_resources, len(all_read_resources), Metal.MTLResourceUsageRead
        )
        encoder.useResources_count_usage_(
