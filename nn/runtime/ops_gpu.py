...
from ..helpers import DEBUG, getenv, prod, ImageDType, OSX, fromimport, diskcache
    ...
    def __init__(self):
        ...
        platform_devices: List[List[cl.Device]] = [
            ...
            for y in (
                ...
                + [x.get_devices(device_type=cl.device_type.CPU) for x in cl_platforms]
            )
            if y
        ]
        self.devices = [
...
class CLBuffer(RawBufferCopyInOut, RawBufferTransfer):
    ...
    def _copyout(self, x: np.ndarray):
        ...
        CL.cl_allocator.ensure_has_free_space(
            self.size * self.dtype.itemsize, self._device
        )
        buf = cl.Buffer(
        ...
        with mapped.base:
            cl.enqueue_copy(
                ...
                is_blocking=True,
                wait_for=[event]
                + ([evt] if (evt := getattr(self, "event", None)) else []),
            )
    ...
    def _transfer(self, x):
        if "gfx" in CL.cl_ctxs[x._buf.device].devices[0].name:
            ...
            raise NotImplementedError(
                "p2p transfer between devices not implemented on non-amd"
            )
...
class CLProgram:
    def __init__(self, name: str, prg: bytes, argdtypes=None, options=None):
        self.name, self.clprograms = name, [
            cl.Program(ctx, ctx.devices, [prg] * len(ctx.devices)) for ctx in CL.cl_ctxs
        ]
        self._clprgs = [
