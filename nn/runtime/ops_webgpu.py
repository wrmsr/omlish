...
from ..renderer.cstyle import WGSLRenderer

wgpu_device = get_default_device()


def create_uniform(val: int) -> wgpu.GPUBuffer:
    buf = wgpu_device.create_buffer(
        size=4, usage=wgpu.BufferUsage.UNIFORM | wgpu.BufferUsage.COPY_DST
    )
        ...
        bindings = [
            {
                "binding": i,
                "resource": {
                    "buffer": create_uniform(x) if i >= len(bufs) else x,
                    "offset": 0,
                    "size": 4 if i >= len(bufs) else x.size,
                },
            }
            for i, x in enumerate(bufs + vals)
        ]  # noqa: E501
