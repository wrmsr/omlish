...
from .. import nn
from ..datasets import cifar_mean
from ..datasets import cifar_std
...
def train_cifar():
    ...
    def pad_reflect(X, size=2) -> Tensor:
        ...
        X_lr = (
                X[..., :, 1 : 1 + p[0]].flip(3).pad(((0, 0), (0, 0), (0, 0), (0, s + p[0]))) +
                X[..., :, -1 - p[1] : -1].flip(3).pad(((0, 0), (0, 0), (0, 0), (s + p[1], 0)))
        )
        ...
        s = X.shape[2]
        X_lr = (
                X[..., 1 : 1 + p[0], :].flip(2).pad(((0, 0), (0, 0), (0, s + p[0]), (0, 0))) +
                X[..., -1 - p[1] : -1, :].flip(2).pad(((0, 0), (0, 0), (s + p[1], 0), (0, 0)))
        )
...
if __name__ == "__main__":
    if not getenv("DIST"):
        ...
        if getenv("HIP"):
            from ..runtime.ops_hip import HIP
