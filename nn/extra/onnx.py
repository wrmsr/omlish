...
from onnx import AttributeProto  # onnx 1.50 uses serialized file (see onnx/onnx-ml.proto) as descriptors
...
def get_run_onnx(onnx_model: ModelProto):
    def type_parse(type_proto: TypeProto):
        ...
        while True:
            ...
            if attr == "tensor_type":
                if "dim_value" not in getattr(type_proto, attr).shape.dim.__dir__():
                    return ()  # variable type, unable to determine shape
                elif not ret:
                    return tuple(
                        [x.dim_value for x in getattr(type_proto, attr).shape.dim]
                    )
                else:
                    ret.extend(
                        [(x.dim_value,) for x in getattr(type_proto, attr).shape.dim]
                ...
                raise NotImplementedError(
                    f"opaque_type is not implemented: {type_proto}"
                )
    ...
    def attribute_parse(
        a: AttributeProto,
    ) -> float | int | str | Tensor | tuple[float] | tuple[int]:
    ...
    for inp in onnx_model.graph.initializer:
        if len(inp.raw_data) > 0:
            ...
            tensors[inp.name] = Tensor(
                np.array([], dtype=np.float32), requires_grad=False
            )
        else:
    ...
    def run_onnx(inputs={}, debug=0):
        ...
        for inp in onnx_model.graph.input:
            ...
            if inp.name in inputs:
                if isinstance(inputs[inp.name], Tensor):
                    ...
                    input_tensors[inp.name] = [
                        Tensor(i, requires_grad=False) for i in inputs[inp.name]
                    ]
                elif (
                    domain == "ai.onnx.preview.training"
                ):  # not sure if in real use the domain is "ai.onnx.preview.training"
                    input_tensors[inp.name] = Tensor(
                        inputs[inp.name], requires_grad=True
                    )  # TODO there isn't a good way to parse which inp requires_grad, some are manually turned off in optimizer ops
                else:
                    input_tensors[inp.name] = Tensor(
                        inputs[inp.name], requires_grad=False
                    )
                if shape:  # if only input_tensor is not variable type
                    input_shape = (
                        input_tensors[inp.name].shape
                        if isinstance(input_tensors[inp.name], Tensor)
                        else (1, *[i.shape for i in input_tensors[inp.name]])
                    )
                    assert (
                        input_shape == shape
                    ), f"wrong shape for input {inp.name}, {input_shape} isn't {shape}"
            else:
        ...
        for num, n in enumerate(onnx_model.graph.node):
            ...
            if debug >= 1:
                print(
                    f"{num}: op {n.op_type} shape {[x.shape if isinstance(x, Tensor) else x for x in inp]} opt {opt}"
                )
            ...
            # need to check onnx_model_version
                ...
                if onnx_model_version < 10:
                    axes, ends, starts, steps = (
                        list(opt.get("axes", range(inp[0].ndim))),
                        list(opt["ends"]),
                        list(opt["starts"]),
                        [1] * inp[0].ndim,
                    )
                    ...
                    starts, ends = inp[1:3]
                    axes = safe_numpy(
                        Tensor.arange(inp[0].ndim, dtype=dtypes.int32)
                        if len(inp) <= 3
                        else inp[3]
                    ).tolist()
                    steps = safe_numpy(inp[4]) if len(inp) > 4 else [1] * inp[0].ndim
                    starts, ends = (
                        safe_numpy(starts.ceil().cast(dtypes.int32)).tolist(),
                        safe_numpy(ends.ceil().cast(dtypes.int32)).tolist(),
                    )
                arg = [(0, x, 1) for x in inp[0].shape]
                for i, axis in enumerate(axes):
                    axis = int(axis) + inp[0].ndim if axis < 0 else int(axis)
                    starts[i], ends[i] = (
                        starts[i] + inp[0].shape[axis] if starts[i] < 0 else starts[i],
                        ends[i] + inp[0].shape[axis] if ends[i] < 0 else ends[i],
                    )
                    starts[i], ends[i] = max(
                        0, min(starts[i], inp[0].shape[axis])
                    ), max(0, min(ends[i], inp[0].shape[axis]))
                    if starts[i] > ends[i] and steps[i] >= 0:
                        steps[i] = -steps[i]
                    arg[axis] = (starts[i], ends[i], steps[i])
                new_shape = tuple(
                    (s, e) if st > 0 else (e + 1, s + 1) for s, e, st in arg
                )
                if any(s == e for s, e in new_shape):
                    ret = inp[0].shrink(new_shape)
                else:
                    ret = inp[0].__getitem__(
                        tuple([slice(s, e, st) for s, e, st in arg])
                    )

            # need to call backward on intermediate_tensors
            elif n.op_type == "Gradient":
                assert len(opt["xs"]) == len(
                    inp
                ), f"len(opt['xs']):{len(opt['xs'])}, len(inp):{len(inp)} output and input has to match"
                y = opt["y"]
            ...
            if debug >= 2:
                print([x.shape if isinstance(x, Tensor) else None for x in ret])
            if debug >= 2:
                print("outputs:")
            for i in range(len(n.output)):
                if debug >= 2:
                    print(f"\t{n.output[i]} - {ret[i]}")
