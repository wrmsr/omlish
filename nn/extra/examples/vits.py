...
class Synthesizer:
    ...
    def infer(
        ...
        max_y_length_estimate_scale=None,
        pad_length=-1,
    ):
        x, m_p, logs_p, x_mask = self.enc_p.forward(
            x.realize(),
            x_lengths.realize(),
            emotion_embedding.realize()
            if emotion_embedding is not None
            else emotion_embedding,
        )
        g = (
            self.emb_g(sid.reshape(1, 1)).squeeze(1).unsqueeze(-1)
            if self.n_speakers > 0
            else None
        )
        logw = self.dp.forward(
            x,
            x_mask.realize(),
            g=g.realize(),
            reverse=self.use_sdp,
            noise_scale=noise_scale_w if self.use_sdp else 1.0,
        )
    ...
    def generate(
        ...
        max_y_length = (
            y_lengths.max().numpy()
            if max_y_length_estimate_scale is None
            else max(15, x.shape[-1]) * max_y_length_estimate_scale
        )
        ...
        m_p_2 = (
            attn.squeeze(1).matmul(m_p.transpose(1, 2)).transpose(1, 2)
        )  # [b, t', t], [b, t, d] -> [b, d, t']
        logs_p_2 = (
            attn.squeeze(1).matmul(logs_p.transpose(1, 2)).transpose(1, 2)
        )  # [b, t', t], [b, t, d] -> [b, d, t']
        z_p = (
            m_p_2
            + Tensor.randn(*m_p_2.shape, dtype=m_p_2.dtype)
            * logs_p_2.exp()
            * noise_scale
        )
        ...
        if pad_length > -1:
            ...
            y_mask = y_mask.pad(((0, 0), (0, 0), (0, pad_length - row_len)), 0).cast(
                z_p.dtype
            )
            ...
            y_mask = Tensor(
                ...
                dtype=y_mask.dtype,
                requires_grad=y_mask.requires_grad,
            )
            z_p = (
                z_p.squeeze(0)
                .pad(((0, 0), (0, pad_length - z_p.shape[2])), 1)
                .unsqueeze(0)
            )
        z = self.flow.forward(
            z_p.realize(), y_mask.realize(), g=g.realize(), reverse=True
        )
        ...
        if max_y_length_estimate_scale is not None:
            ...
            if real_max_y_length > max_y_length:
                logging.warning(
                    f"Underestimated max length by {(((real_max_y_length / max_y_length) * 100) - 100):.2f}%, recomputing inference without estimate..."
                )
                return self.generate(
                    g,
                    logs_p,
            ...
            if real_max_y_length < max_y_length:
                ...
                if overestimation > 10:
                    logging.warning(
                        "Warning: max length overestimated by more than 10%"
                    )
...
class StochasticDurationPredictor:
    def __init__(
        ...
        self.post_pre, self.post_proj = nn.Conv1d(1, filter_channels, 1), nn.Conv1d(
        ...
        self.pre, self.proj = nn.Conv1d(in_channels, filter_channels, 1), nn.Conv1d(
            filter_channels, filter_channels, 1
        )
        self.convs = DDSConv(
            filter_channels, kernel_size, n_layers=3, p_dropout=p_dropout
    ...
    @TinyJit
    def forward(
        self, x: Tensor, x_mask, w=None, g=None, reverse=False, noise_scale=1.0
    ):
        x = self.pre(x.detach())
        if g is not None:
        ...
        if not reverse:
            ...
            log_det_tot_q = 0
            h_w = (
                self.post_proj(self.post_convs.forward(self.post_pre(w), x_mask))
                * x_mask
            )
            e_q = (
                Tensor.randn(w.size(0), 2, w.size(2), dtype=x.dtype).to(device=x.device)
                * x_mask
            )
            z_q = e_q
            ...
            z0 = (w - u) * x_mask
            log_det_tot_q += Tensor.sum(
                (z_u.logsigmoid() + (-z_u).logsigmoid()) * x_mask, [1, 2]
            )
            log_q = (
                Tensor.sum(-0.5 * (math.log(2 * math.pi) + (e_q**2)) * x_mask, [1, 2])
                - log_det_tot_q
            )
            log_det_tot = 0
            ...
            nll = (
                Tensor.sum(0.5 * (math.log(2 * math.pi) + (z**2)) * x_mask, [1, 2])
                - log_det_tot
            )
        ...
        z = (
            Tensor.randn(x.shape[0], 2, x.shape[2], dtype=x.dtype).to(device=x.device)
            * noise_scale
        )
        for flow in flows:
...
class DurationPredictor:
    def __init__(
        ...
        (
            ...
            self.p_dropout,
            self.gin_channels,
        ) = (in_channels, filter_channels, kernel_size, p_dropout, gin_channels)
        self.conv_1, self.norm_1 = nn.Conv1d(
            in_channels, filter_channels, kernel_size, padding=kernel_size // 2
        ), LayerNorm(filter_channels)
        self.conv_2, self.norm_2 = nn.Conv1d(
...
class PosteriorEncoder:
    def __init__(
        ...
        self.pre, self.proj = nn.Conv1d(in_channels, hidden_channels, 1), nn.Conv1d(
            hidden_channels, out_channels * 2, 1
        )
        self.enc = WN(
            hidden_channels,
            kernel_size,
...
class Generator:
    def __init__(
        ...
        self.num_kernels, self.num_upsamples = len(resblock_kernel_sizes), len(
            upsample_rates
        )
    ...
    def forward(self, x: Tensor, g=None):
        ...
        for i in range(self.num_upsamples):
            ...
            xs = sum(
                self.resblocks[i * self.num_kernels + j].forward(x)
...
class LayerNorm(nn.LayerNorm):
    def __init__(self, channels, eps=1e-5):
        super().__init__(channels, eps, elementwise_affine=True)

    def forward(self, x: Tensor):
        return self.__call__(x.transpose(1, -1)).transpose(1, -1)
...
class DDSConv:  # Dilated and Depth-Separable Convolution
    def __init__(self, channels, kernel_size, n_layers, p_dropout=0.0):
        self.channels, self.kernel_size, self.n_layers, self.p_dropout = (
            channels,
            kernel_size,
            n_layers,
            p_dropout,
...
class ConvFlow:
    def __init__(
        ...
        (
            ...
            self.num_bins,
            self.tail_bound,
        ) = (in_channels, filter_channels, kernel_size, n_layers, num_bins, tail_bound)
    ...
    def forward(self, x, x_mask, g=None, reverse=False):
        ...
        un_normalized_heights = h[..., self.num_bins : 2 * self.num_bins] / math.sqrt(
            self.filter_channels
        )
        un_normalized_derivatives = h[..., 2 * self.num_bins :]
        x1, log_abs_det = piecewise_rational_quadratic_transform(
            x1,
            un_normalized_widths,
            un_normalized_heights,
            un_normalized_derivatives,
            inverse=reverse,
            tails="linear",
...
class ResidualCouplingLayer:
    def __init__(
        ...
        (
            ...
            self.n_layers,
            self.mean_only,
        ) = (channels, hidden_channels, kernel_size, dilation_rate, n_layers, mean_only)
    ...
    def forward(self, x, x_mask, g=None, reverse=False):
        ...
        if not reverse:
            return x0.cat((m + x1 * logs.exp() * x_mask), dim=1)
    ...
    def forward(self, x: Tensor, *args, reverse=False, **kwargs):
        return (
            ...
            if reverse
                ...
                x.flip([1]),
                Tensor.zeros(x.shape[0], dtype=x.dtype).to(device=x.device),
            )
        )
...
class ElementwiseAffine:
    ...
    ):  # x if reverse else y, logdet
        return (
            (x - self.m) * Tensor.exp(-self.logs) * x_mask
            if reverse
            else (
                (self.m + Tensor.exp(self.logs) * x) * x_mask,
                Tensor.sum(self.logs * x_mask, [1, 2]),
            )
        )
...
class MultiHeadAttention:
    def __init__(
        ...
        if window_size is not None:
            self.emb_rel_k, self.emb_rel_v = [
                Tensor.randn(
                    1 if heads_share else n_heads, window_size * 2 + 1, self.k_channels
                )
                * (self.k_channels**-0.5)
                for _ in range(2)
            ]
    ...
    def attention(
        self, query: Tensor, key: Tensor, value: Tensor, mask=None
        ...
        output = (
            output.transpose(2, 3).contiguous().reshape(b, d, t_t)
    ...
    def _matmul_with_relative_keys(self, x, y):
        return x.matmul(
            y.unsqueeze(0).transpose(-2, -1)
        )  # x: [b, h, l, d], y: [h or 1, m, d], re, : [b, h, l, m]

    def _get_relative_embeddings(self, relative_embeddings, length):
        pad_length, slice_start_position = max(length - (self.window_size + 1), 0), max(
            (self.window_size + 1) - length, 0
        )
        padded_relative_embeddings = (
            relative_embeddings
            if pad_length <= 0
            else relative_embeddings.pad(
    ...
    def _relative_position_to_absolute_position(
        ...
        x_flat = x.reshape([batch, heads, length * 2 * length]).pad(
            convert_pad_shape([[0, 0], [0, 0], [0, length - 1]])
        )
        return x_flat.reshape([batch, heads, length + 1, 2 * length - 1])[
            :, :, :length, length - 1 :
        ]
    ...
    def _absolute_position_to_relative_position(
        ...
        x_flat = x.reshape([batch, heads, length**2 + length * (length - 1)]).pad(
...
class FFN:
    def __init__(
        ...
        self.conv_1, self.conv_2 = nn.Conv1d(
            in_channels, filter_channels, kernel_size
        ), nn.Conv1d(filter_channels, out_channels, kernel_size)
    ...
    def _causal_padding(self, x):
        return (
            ...
            else x.pad(convert_pad_shape([[0, 0], [0, 0], [self.kernel_size - 1, 0]]))
        )
...
def piecewise_rational_quadratic_transform(
    ...
    if tails is None:
        spline_fn, spline_kwargs = rational_quadratic_spline, {}
    else:
        spline_fn, spline_kwargs = unconstrained_rational_quadratic_spline, {
            "tails": tails,
            "tail_bound": tail_bound,
        }
    return spline_fn(
        inputs=inputs,
        un_normalized_widths=un_normalized_widths,
        un_normalized_heights=un_normalized_heights,
        un_normalized_derivatives=un_normalized_derivatives,
        inverse=inverse,
...
def unconstrained_rational_quadratic_spline(
    ...
    output, log_abs_det = rational_quadratic_spline(
        inputs=inputs.squeeze(dim=0).squeeze(dim=0),
        unnormalized_widths=un_normalized_widths.squeeze(dim=0).squeeze(dim=0),
        unnormalized_heights=un_normalized_heights.squeeze(dim=0).squeeze(dim=0),
        unnormalized_derivatives=un_normalized_derivatives.squeeze(dim=0).squeeze(
            dim=0
        ),
        inverse=inverse,
        left=-tail_bound,
    ...
    return output.unsqueeze(dim=0).unsqueeze(dim=0), log_abs_det.unsqueeze(
        dim=0
    ).unsqueeze(dim=0)
...
def rational_quadratic_spline(
    ...
    widths = min_bin_width + (
        1 - min_bin_width * num_bins
    ) * unnormalized_widths.softmax(axis=-1)
    cum_widths = cat_lr(
        ((right - left) * widths[..., :-1].cumsum(axis=1) + left),
        left,
        right + 1e-6 if not inverse else right,
    )
    widths = cum_widths[..., 1:] - cum_widths[..., :-1]
    derivatives = min_derivative + (unnormalized_derivatives.exp() + 1).log()
    heights = min_bin_height + (
        1 - min_bin_height * num_bins
    ) * unnormalized_heights.softmax(axis=-1)
    cum_heights = cat_lr(
        ((top - bottom) * heights[..., :-1].cumsum(axis=1) + bottom),
        bottom,
        top + 1e-6 if inverse else top,
    )
    heights = cum_heights[..., 1:] - cum_heights[..., :-1]
    bin_idx = (
        (inputs[..., None] >= (cum_heights if inverse else cum_widths)).sum(axis=-1) - 1
    )[..., None]
    ...
    if inverse:
        a = (inputs - input_cum_heights) * (
            input_derivatives + input_derivatives_plus_one - 2 * input_delta
        ) + input_heights * (input_delta - input_derivatives)
        b = input_heights * input_derivatives - (inputs - input_cum_heights) * (
            input_derivatives + input_derivatives_plus_one - 2 * input_delta
        )
        ...
        return root * input_bin_widths + input_cum_widths, -(
            derivative_numerator.log() - 2 * denominator.log()
        )
    ...
    return (
        input_cum_heights + numerator / denominator,
        derivative_numerator.log() - 2 * denominator.log(),
    )


def sequence_mask(length: Tensor, max_length):
...
def generate_path(
    duration: Tensor, mask: Tensor
):  # duration: [b, 1, t_x], mask: [b, 1, t_y, t_x]
    b, _, t_y, t_x = mask.shape
    path = (
        sequence_mask(duration.cumsum(axis=2).reshape(b * t_x), t_y)
        .cast(mask.dtype)
        .reshape(b, t_x, t_y)
    )
...
def fused_add_tanh_sigmoid_multiply(input_a: Tensor, input_b: Tensor, n_channels: int):
    ...
    t_act, s_act = (
        in_act[:, :n_channels_int, :].tanh(),
        in_act[:, n_channels_int:, :].sigmoid(),
    )
    return t_act * s_act


def cat_lr(t, left, right):
    return (
        Tensor.full(get_shape(t), left)
        .cat(t, dim=-1)
        .cat(Tensor.full(get_shape(t), right), dim=-1)
    )
...
def convert_pad_shape(pad_shape):
    return tuple(tuple(x) for x in pad_shape)
...
def split(
    ...
    assert (
        sum(split_sizes) == tensor.shape[dim]
    ), "Sum of split_sizes must equal the dimension size of tensor along the given dimension."
    start, slices = 0, []
    for size in split_sizes:
        slice_range = [
            (start, start + size) if j == dim else None
...
def gather(x, indices, axis):
    indices = (
        (indices < 0).where(indices + x.shape[axis], indices).transpose(ax1=axis, ax2=0)
    )
    ...
    return (
        (
            (
                indices.unsqueeze(indices.ndim).expand(*indices.shape, x.shape[-1])
                == Tensor.arange(x.shape[-1])
                .reshape(*reshape_arg)
                .expand(*indices.shape, x.shape[-1])
            )
            * x
        )
        .sum(indices.ndim)
        .transpose(ax1=0, ax2=axis)
    )
...
def norm_except_dim(v, dim):
    ...
    return np.transpose(
        norm_except_dim(transposed_v, 0),
        (dim,) + tuple(i for i in range(v.ndim) if i != dim),
    )
...
class HParams:
    def __init__(self, **kwargs):
        for k, v in kwargs.items():
            self[k] = v if type(v) != dict else HParams(**v)
...
def load_checkpoint(checkpoint_path, model: Synthesizer, optimizer=None, skip_list=[]):
    ...
    for key, v in saved_state_dict.items():
        if any(layer in key for layer in skip_list):
            continue
        try:
            ...
            for k in key.split("."):
                if k.isnumeric():
                    ...
                    if isinstance(obj, (LayerNorm, nn.LayerNorm)) and k in [
                        "gamma",
                        "beta",
                    ]:
            ...
            raise e
    logging.info(
        f"Loaded checkpoint '{checkpoint_path}' (iteration {iteration}) in {time.time() - start_time:.4f}s"
    )
...
class TextMapper:  # Based on https://github.com/keithito/tacotron
    def __init__(self, symbols, apply_cleaners=True):
        ...
        self._symbol_to_id, _id_to_symbol = {s: i for i, s in enumerate(symbols)}, {
            i: s for i, s in enumerate(symbols)
        }
        self._whitespace_re, self._abbreviations = re.compile(r"\s+"), [
    ...
    def text_to_sequence(self, text, cleaner_names):
        if self.apply_cleaners:
            for name in cleaner_names:
                ...
                if not cleaner:
                    raise ModuleNotFoundError("Unknown cleaner: %s" % name)
                text = cleaner(text)
        else:
            text = text.strip()
    ...
    def get_text(self, text, add_blank=False, cleaners=("english_cleaners2",)):
        ...
        return Tensor(
            self.intersperse(text_norm, 0) if add_blank else text_norm,
            dtype=dtypes.int64,
        )
    ...
    def phonemize(self, text, strip=True):
        return _phonemize(
            self.phonemizer, text, default_separator, strip, 1, False, False
        )

    def filter_oov(self, text):
        return "".join(list(filter(lambda x: x in self._symbol_to_id, text)))

    def base_english_cleaners(self, text):
        return self.collapse_whitespace(
            self.phonemize(self.expand_abbreviations(unidecode(text.lower())))
        )

    def english_cleaners2(self, text):
        return self.base_english_cleaners(text)

    def transliteration_cleaners(self, text):
        return self.collapse_whitespace(unidecode(text.lower()))

    def cjke_cleaners(self, text):
        return re.sub(
            r"([^\.,!\?\-…~])$",
            r"\1.",
            re.sub(
    ...
    def cjke_cleaners2(self, text):
        return re.sub(
            r"([^\.,!\?\-…~])$", r"\1.", re.sub(r"\s+$", "", self.english_to_ipa2(text))
        )

    def cjks_cleaners(self, text):
        return re.sub(
            r"([^\.,!\?\-…~])$",
            r"\1.",
            re.sub(r"\s+$", "", self.english_to_lazy_ipa(text)),
        )
    ...
    def english_to_ipa2(self, text):
        ...
        return reduce(
            lambda t, rx: re.sub(rx[0], rx[1], t),
            _ipa_to_ipa2,
            self.mark_dark_l(self.english_to_ipa(text)),
        ).replace("...", "…")
        ...
        return self.collapse_whitespace(
            ipa.convert(
                self.normalize_numbers(
                    self.expand_abbreviations(unidecode(text).lower())
                )
            )
        )
    ...
    def english_to_lazy_ipa(self, text):
        ...
        return reduce(
            lambda t, rx: re.sub(rx[0], rx[1], t), _lazy_ipa, self.english_to_ipa(text)
        )

    def expand_abbreviations(self, text):
        return reduce(
            lambda t, abbr: re.sub(abbr[0], abbr[1], t), self._abbreviations, text
        )
    ...
    def _remove_commas(self, m):
        return m.group(1).replace(",", "")  # george won't like this
    ...
    def _expand_dollars(self, m):
        ...
        if dollars and cents:
            return "%s %s, %s %s" % (
                dollars,
                "dollar" if dollars == 1 else "dollars",
                cents,
...
MODELS = {  # config_url, weights_url
    "ljs": (
        ...
        "https://drive.google.com/uc?export=download&id=1q86w74Ygw2hNzYP9cWkeClGT5X25PvBT&confirm=t",
    ),
    "vctk": (
        "https://raw.githubusercontent.com/jaywalnut310/vits/main/configs/vctk_base.json",
        "https://drive.google.com/uc?export=download&id=11aHOlhnxzjpdWDpsz1vFDCzbeEfoIxru&confirm=t",
    ),
    "mmts-tts": (
        "https://huggingface.co/facebook/mms-tts/raw/main/full_models/eng/config.json",
        "https://huggingface.co/facebook/mms-tts/resolve/main/full_models/eng/G_100000.pth",
    ),
    "uma_trilingual": (
        "https://huggingface.co/spaces/Plachta/VITS-Umamusume-voice-synthesizer/raw/main/configs/uma_trilingual.json",
    ...
    "voistock": (
        "https://huggingface.co/spaces/skytnt/moe-tts/resolve/main/saved_model/15/config.json",
        "https://huggingface.co/spaces/skytnt/moe-tts/resolve/main/saved_model/15/model.pth",
    ),
}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--out_path",
        default=None,
        help="Specify the full output path. Overrides the --out_dir and --name parameter.",
    )
    parser.add_argument(
        "--out_dir",
        default=str(Path(__file__).parents[1] / "temp"),
        help="Specify the output path.",
    )
    parser.add_argument(
        "--base_name",
        default="test",
        help="Specify the base of the output file name. Default is 'test'.",
    )
    parser.add_argument(
        "--text_to_synthesize",
        default="""Hello person. If the code you are contributing isn't some of the highest quality code you've written in your life, either put in the effort to make it great, or don't bother.""",
    ...
    if model_has_multiple_speakers:
        ...
        if args.speaker_id >= hps.data.n_speakers:
            raise ValueError(f"Speaker ID {args.speaker_id} is invalid for this model.")
        speaker_name = "?"
        if hps.__contains__("speakers"):  # maps speaker ids to names
            ...
            speaker_name = next(
                (key for key, value in speakers.items() if value == args.speaker_id),
                None,
            )
    ...
    if args.emotion_path is not None:
        if args.emotion_path.endswith(".npy"):
            emotion_embedding = Tensor(
                np.load(args.emotion_path), dtype=dtypes.int64
            ).unsqueeze(0)
        else:
    ...
    if hps.__contains__("symbols"):
        symbols = hps.symbols
    elif args.model_to_use == "mmts-tts":
        symbols = [
            x.replace("\n", "")
            for x in fetch(
    ...
    x_tst, x_tst_lengths = stn_tst.unsqueeze(0), Tensor(
        [stn_tst.shape[0]], dtype=dtypes.int64
    )
    sid = (
        Tensor([args.speaker_id], dtype=dtypes.int64)
        if model_has_multiple_speakers
        else None
    )
    ...
    audio_tensor = net_g.infer(
        ...
        max_y_length_estimate_scale=Y_LENGTH_ESTIMATE_SCALARS[args.model_to_use]
        if args.estimate_max_y_length
        else None,
    )[0, 0].realize()
    ...
    out_path = Path(
        args.out_path
        or Path(args.out_dir)
        / f"{args.model_to_use}{f'_sid_{args.speaker_id}' if model_has_multiple_speakers else ''}_{args.base_name}.wav"
    )
