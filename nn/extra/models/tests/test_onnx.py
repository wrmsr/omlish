...
class TestOnnxModel(unittest.TestCase):
    def test_benchmark_openpilot_model(self):
        ...
        def get_inputs():
            np_inputs = {
                ...
                "traffic_convention": np.array([[1.0, 0.0]]),
                "nav_features": np.zeros((1, 256)),
                "features_buffer": np.zeros((1, 99, 128)),
            }
            inputs = {
                k: Tensor(v.astype(np.float32), requires_grad=False)
        ...
        for _ in range(7):
            ...
            if not CI:
                print(
                    f"ran openpilot model in {(et-st)*1000.0:.2f} ms, waited {(mt2-mt)*1000.0:.2f} ms for realize, {(et-mt2)*1000.0:.2f} ms for GPU queue"
                )
    ...
    def test_efficientnet(self):
        ...
        self._test_model(
            fetch(
                "https://github.com/onnx/models/raw/main/archive/vision/classification/efficientnet-lite4/model/efficientnet-lite4-11.onnx"
            ),
            input_name,
            input_new,
        )  # noqa: E501
    ...
    def test_shufflenet(self):
        ...
        self._test_model(
            fetch(
                "https://github.com/onnx/models/raw/main/archive/vision/classification/shufflenet/model/shufflenet-9.onnx"
            ),
            input_name,
            input_new,
        )  # noqa: E501
    ...
    def test_resnet(self):
        ...
        self._test_model(
            fetch(
                "https://github.com/onnx/models/raw/main/archive/vision/classification/resnet/model/resnet18-v2-7.onnx"
            ),
            input_name,
            input_new,
        )  # noqa: E501
    ...
    def _test_model(self, fn, input_name, input_new, debug=False):
        ...
        print("onnx loaded")
        from test.models.test_efficientnet import (
            chicken_img,
            car_img,
            preprocess,
            _LABELS,
